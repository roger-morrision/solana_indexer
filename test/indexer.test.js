...
import { decodePumpBondingCurveInitializations, decodePumpMigrations, decodePumpSwapEvents, decodePumpSwapPoolInitializations, decodePumpTradeEvents, decodeRaydiumClmmSwapEvents, decodeRaydiumCpmmPoolInitializations, decodeRaydiumSwapEvents, parseBlock } from "../src/parser.js";
...
test("decodes Raydium CLMM SwapEvent with exact price state and explicit unavailable reserves", async () => {
  ...
  const block = parseBlock(input), swap = block.swaps[0]; assert.equal(swap.registryVersion, 5); assert.equal(swap.decoderVersion, 1); assert.equal(swap.inputVaultBeforeRaw, null); assert.equal(swap.sqrtPriceX64, sqrtPriceX64.toString());
...
test("decodes Pump create-v2 bonding-curve lifecycle with canonical quote identity", async () => {
  ...
  const block = parseBlock({ slot: 902, blockhash: "block-902", previousBlockhash: "block-901", parentSlot: 901, blockTime: 1_700_000_002, transactions: [entry], provenance: { commitment: "finalized" } }), store = new IndexStore("unused"); await store.load(); store.apply(block); assert.equal(block.poolLifecycleEvents[0].decoderVersion, 3); assert.equal(store.pool("curve-account-2").summary.venueType, "bonding_curve"); assert.equal(store.pool("curve-account-2").summary.lifecycle.symbol, "TERM"); store.rebuildAggregates(); assert.equal(store.pool("curve-account-2").summary.lifecycle.creator, decoded[0].creator);
...
test("links Pump bonding-curve migration to its canonical PumpSwap pool", async () => {
  ...
  const block = parseBlock({ slot: 903, blockhash: "block-903", previousBlockhash: "block-902", parentSlot: 902, blockTime: 1_700_000_003, transactions: [v2Entry], provenance: { commitment: "finalized" } }), store = new IndexStore("unused"); await store.load(); store.apply(block); assert.equal(block.poolLifecycleEvents[0].type, "pool_migrated"); assert.equal(block.poolLifecycleEvents[0].decoderVersion, 3); assert.equal(store.pool("v2-10").summary.destinationProtocol, "pump-swap"); assert.equal(store.pool("v2-10").summary.lifecycle.sourcePool, "v2-4"); store.rebuildAggregates(); assert.equal(store.pool("v2-10").summary.lifecycle.migrationVersion, 2);
...
test("internal evidence API exposes missing fields and immutable provenance contract", async (t) => {
  ...
  const registry = await (await fetch(`${base}/internal/registry`)).json(); assert.equal(registry.version, 5); assert.ok(registry.programs.some((row) => row.protocol === "pump-bonding-curve" && row.eventTypes.includes("pool_created") && row.eventTypes.includes("pool_migrated"))); assert.ok(registry.programs.some((row) => row.protocol === "raydium-clmm")); assert.ok(registry.programs.some((row) => row.protocol === "pump-swap" && row.eventTypes.includes("pool_created")));
});
