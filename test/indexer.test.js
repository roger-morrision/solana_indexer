...
import { decodeOrcaWhirlpoolPoolInitializations, decodeOrcaWhirlpoolSwapEvents, decodePumpBondingCurveInitializations, decodePumpCompletionEvents, decodePumpMigrations, decodePumpSwapEvents, decodePumpSwapPoolInitializations, decodePumpTradeEvents, decodeRaydiumClmmSwapEvents, decodeRaydiumCpmmPoolInitializations, decodeRaydiumSwapEvents, parseBlock } from "../src/parser.js";
...
test("finalized Raydium CLMM snapshots decode canonical pool state and vault balances", async () => {
  ...
  const store = new IndexStore("unused"); await store.load(); store.applyPoolSnapshot(snapshot); const pool = store.pool("pool-a").summary; assert.equal(pool.pairIdentitySource, "protocol_account"); assert.equal(pool.accountSnapshot.commitment, "finalized"); assert.equal(store.stats().poolSnapshots, 1); assert.equal(store.state.events.at(-1).type, "clmm_pool_snapshot_applied"); assert.deepEqual(compileWarehouseBatch(store.state, { lastSequence: 0 }).events.map((row) => row.event_type), ["clmm_pool_snapshot_applied"]); const sequence = store.state.eventSequence; store.applyPoolSnapshot(snapshot); assert.equal(store.state.eventSequence, sequence); assert.throws(() => store.applyPoolSnapshot({ ...snapshot, pools: [{ ...snapshot.pools[0], vault0AmountRaw: "999" }] }), /conflicting finalized pool snapshot/); store.rebuildAggregates(); assert.equal(store.pool("pool-a").summary.accountSnapshot.balanceSlot, 501);
...
test("finalized Orca snapshots bind canonical Whirlpool state and vault balances", async () => {
  ...
  const client = { call: async (method, params) => { assert.equal(method, "getMultipleAccounts"); if (params[1].encoding === "base64") return { context: { slot: 600 }, value: [account] }; assert.equal(params[1].minContextSlot, 600); return { context: { slot: 601 }, value: [{ data: { parsed: { info: { mint: mint0, tokenAmount: { amount: "3000" } } } } }, { data: { parsed: { info: { mint: mint1, tokenAmount: { amount: "4000" } } } } }] }; } };
  const snapshot = await createOrcaPoolSnapshot({ client, pools: ["orca-pool"], genesisHash: MAINNET_GENESIS_HASH, observedAt: "2026-08-22T00:00:00.000Z" }); assert.deepEqual({ stateSlot: snapshot.stateSlot, balanceSlot: snapshot.balanceSlot, vault0AmountRaw: snapshot.pools[0].vault0AmountRaw, vault1AmountRaw: snapshot.pools[0].vault1AmountRaw }, { stateSlot: 600, balanceSlot: 601, vault0AmountRaw: "3000", vault1AmountRaw: "4000" });
  const store = new IndexStore("unused"); await store.load(); store.applyOrcaPoolSnapshot(snapshot); const pool = store.pool("orca-pool").summary; assert.equal(pool.protocol, "orca-whirlpool"); assert.equal(pool.baseMint, mint0); assert.equal(pool.accountSnapshot.sqrtPriceX64, ((1n << 80n) + 5n).toString()); assert.equal(store.state.events.at(-1).type, "orca_pool_snapshot_applied"); assert.deepEqual(compileWarehouseBatch(store.state, { lastSequence: 0 }).events.map((row) => row.event_type), ["orca_pool_snapshot_applied"]); const sequence = store.state.eventSequence; store.applyOrcaPoolSnapshot(snapshot); assert.equal(store.state.eventSequence, sequence); assert.throws(() => store.applyOrcaPoolSnapshot({ ...snapshot, pools: [{ ...snapshot.pools[0], vault0AmountRaw: "999" }] }), /conflicting finalized Orca pool snapshot/); assert.equal(store.poolRisk("orca-pool", 120_000, Date.parse(snapshot.observedAt) + 60_000).liquidity.assessable, true);
  assert.throws(() => store.applyOrcaPoolSnapshot({ ...snapshot, genesisHash: "devnet" }), /invalid finalized mainnet Orca pool snapshot/); const wrongVault = { call: async (method, params) => params[1].encoding === "base64" ? { context: { slot: 600 }, value: [account] } : { context: { slot: 601 }, value: [{ data: { parsed: { info: { mint: "wrong", tokenAmount: { amount: "1" } } } } }, { data: { parsed: { info: { mint: mint1, tokenAmount: { amount: "1" } } } } }] } }; await assert.rejects(createOrcaPoolSnapshot({ client: wrongVault, pools: ["orca-pool"], genesisHash: MAINNET_GENESIS_HASH }), /vault identity mismatch/);
});
...
test("decodes Raydium CLMM SwapEvent with exact price state and explicit unavailable reserves", async () => {
  ...
  const block = parseBlock(input), swap = block.swaps[0]; assert.equal(swap.registryVersion, 8); assert.equal(swap.decoderVersion, 1); assert.equal(swap.inputVaultBeforeRaw, null); assert.equal(swap.sqrtPriceX64, sqrtPriceX64.toString());
...
test("decodes Orca Whirlpool Traded events only with matching swap accounts and token evidence", async () => {
  ...
  const input = JSON.parse(await fs.readFile(fixture, "utf8")); input.dexEvents = []; input.transactions[0] = { transaction: { signatures: ["orca-signature"], message: entry.transaction.message }, meta: entry.meta }; const swap = parseBlock(input).swaps[0]; assert.equal(swap.protocol, "orca-whirlpool"); assert.equal(swap.registryVersion, 8); assert.equal(swap.decoderVersion, 2); assert.equal(swap.reserveTiming, "unavailable"); assert.equal(swap.inputVaultBeforeRaw, null);
...
test("backup and restore tooling verifies integrity and gates destructive restore", async () => {
  ...
  assert.match(backup, /sha256sum .*SHA256SUMS/); assert.match(backup, /SELF_HOSTED_ARCHIVE_URL/); assert.match(backup, /BACKUP_WRITERS_QUIESCED/); assert.match(backup, /pg_dump/); assert.match(backup, /FORMAT Native/); assert.match(backup, /backup-preflight\.js/); assert.match(fetchBackup, /sha256sum --check SHA256SUMS/); assert.match(fetchBackup, /SELF_HOSTED_ARCHIVE_URL must be loopback HTTP/); assert.match(restore, /--confirm-empty-target/); assert.match(restore, /sha256sum --check SHA256SUMS/); assert.match(restore, /backup-preflight\.js/); assert.match(restore, /pg_restore --clean --if-exists/); assert.match(restore, /TRUNCATE TABLE/);
});
...
test("backup preflight proves complete fresh checksummed artifacts without authorizing restore", async (t) => {
  ...
  const tar = Buffer.concat([entry("data/index.json", "{}"), entry("data/exporter-status.json", "{}"), Buffer.alloc(1_024)]), files = { "postgres.dump": "pg", "clickhouse-instructions.native": "i", "clickhouse-swaps.native": "s", "clickhouse-balance_changes.native": "b", "clickhouse-dead_letters.native": "d", "redis.rdb": "r", "indexer-state.tar": tar, "inbox-manifest.json": "{}", "manifest.json": JSON.stringify({ schemaVersion: 1, createdAt: "2026-08-22T00:00:00.000Z", chain: "solana", scope: "postgres-clickhouse-redis-indexer-state" }) };
  const sums = []; for (const [name, value] of Object.entries(files)) { const bytes = Buffer.isBuffer(value) ? value : Buffer.from(value); await fs.writeFile(path.join(root, name), bytes); sums.push(`${crypto.createHash("sha256").update(bytes).digest("hex")}  ${name}`); } await fs.writeFile(path.join(root, "SHA256SUMS"), `${sums.join("\n")}\n`);
  const result = await preflightBackup(root, { now: Date.parse("2026-08-22T01:00:00.000Z") }); assert.equal(result.restoreAuthorized, false); assert.equal(result.checksumFiles, 9); assert.deepEqual(result.invariants, { completeInventory: true, checksums: true, rpo: true, safeTar: true, canonicalStatePresent: true }); await assert.rejects(preflightBackup(root, { now: Date.parse("2026-08-24T00:00:00.000Z") }), /outside RPO/); await fs.writeFile(path.join(root, "redis.rdb"), "corrupt"); await assert.rejects(preflightBackup(root, { now: Date.parse("2026-08-22T01:00:00.000Z") }), /checksum mismatch/);
});
...
test("warehouse sync is ordered, retry-safe, and checkpoints only after both sinks", async () => {
  ...
  const sinks = validateWarehouseSinkSequences(3, { clickhouse: "3\n", postgres: "3", redis: "3" }); assert.deepEqual(sinks, { clickhouse: 3, postgres: 3, redis: 3 }); assert.throws(() => validateWarehouseSinkSequences(3, { clickhouse: "2", postgres: "3", redis: "3" }), /clickhouse warehouse sequence mismatch/); assert.throws(() => validateWarehouseSinkSequences(3, { clickhouse: "3", postgres: "", redis: "3" }), /postgres warehouse sequence unavailable/);
  const probeCalls = [], probeSpawn = (command, args) => { probeCalls.push({ command, args }); const listeners = {}, stdout = { on(name, handler) { if (name === "data") queueMicrotask(() => { handler(Buffer.from("3\n")); listeners.close(0); }); } }, stderr = { on() {} }; return { stdout, stderr, on(name, handler) { listeners[name] = handler; } }; }; assert.deepEqual(await probeWarehouseSinks(3, probeSpawn, {}), sinks); assert.deepEqual(probeCalls.map((row) => row.command).sort(), ["clickhouse-client", "psql", "redis-cli"]); assert.match(probeCalls.find((row) => row.command === "psql").args.at(-1), /warehouse-canonical-events/); assert.deepEqual(probeCalls.find((row) => row.command === "redis-cli").args, ["--raw", "GET", "terminal_dex:hot:current"]);
  const reconciliation = validateWarehouseReconciliation({ sequence: 3, events: 3, instructions: 1, swaps: 2, balanceChanges: 4, deadLetters: 0, tokens: 5, pools: 6 }, { clickhouse: "3\n1\n2\n4\n0", postgres: "3\n5", redis: "3\n6\n5" }); assert.equal(reconciliation.verified, true); assert.throws(() => validateWarehouseReconciliation(reconciliation, { clickhouse: "3\n1\n1\n4\n0", postgres: "3\n5", redis: "3\n6\n5" }), /mismatch/);
  const outputs = { "clickhouse-client": "3\n1\n2\n4\n0", psql: "3\n5", "redis-cli": "3\n6\n5" }, reconciliationCalls = [], reconciliationSpawn = (command, args) => { reconciliationCalls.push({ command, args }); const listeners = {}, stdout = { on(name, handler) { if (name === "data") queueMicrotask(() => { handler(Buffer.from(outputs[command])); listeners.close(0); }); } }, stderr = { on() {} }; return { stdout, stderr, on(name, handler) { listeners[name] = handler; } }; }; assert.deepEqual(await probeWarehouseReconciliation(reconciliation, reconciliationSpawn, {}), reconciliation); assert.match(reconciliationCalls.find((row) => row.command === "clickhouse-client").args.at(-1), /uniqExact\(event_id\)/);
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "warehouse-checkpoint-")), filename = path.join(root, "nested", "checkpoint.json"); await writeWarehouseCheckpoint(filename, 3, sinks, reconciliation); const persisted = JSON.parse(await fs.readFile(filename, "utf8")); assert.deepEqual({ schemaVersion: persisted.schemaVersion, consumer: persisted.consumer, lastSequence: persisted.lastSequence, sinks: persisted.sinks, reconciliation: persisted.reconciliation }, { schemaVersion: 1, consumer: "warehouse-canonical-events", lastSequence: 3, sinks, reconciliation }); await assert.rejects(writeWarehouseCheckpoint(filename, 3, sinks), /reconciliation/);
});

test("warehouse materializes touched canonical slots after synchronous fork cleanup", async () => {
  const store = new IndexStore("unused"); await store.load(); store.apply(parseBlock(JSON.parse(await fs.readFile(fixture, "utf8")))); store.state.deadLetters = [{ id: "bad.ndjson:hash", filename: "bad.ndjson", fingerprint: "a".repeat(64), error: "invalid record", attempts: 2, firstObservedAt: "2026-08-22T00:00:00.000Z", lastObservedAt: "2026-08-22T00:01:00.000Z", resolved: false }]; const batch = compileWarehouseBatch(store.state, { lastSequence: 0 }), facts = compileWarehouseFacts(store.state, batch); assert.deepEqual(facts.slots, [100]); assert.equal(facts.instructions.length, 1); assert.equal(facts.swaps.length, 1); assert.equal(facts.balanceChanges.length, 0); assert.equal(facts.deadLetters.length, 1); assert.equal(facts.instructions[0].commitment, "finalized"); assert.equal(facts.swaps[0].input_amount_raw, "12500000");
  const projections = compileWarehouseProjections(store), metadataSql = compileWarehouseMetadataSql(store.state, batch.toSequence, projections), redisInput = compileRedisHotSync(store.state, batch), redisText = redisInput.toString(); assert.ok(projections.candidates.length > 0); assert.equal(projections.candidates[0].state, "review_required"); assert.ok(projections.jobs.some((row) => row.type === "account_snapshot")); assert.match(metadataSql, /^BEGIN;/); assert.match(metadataSql, /INSERT INTO tokens/); assert.match(metadataSql, /INSERT INTO candidates/); assert.match(metadataSql, /UPDATE operational_jobs SET status = 'cancelled'/); assert.match(metadataSql, /INSERT INTO operational_jobs/); assert.match(metadataSql, /operational_jobs\.status = 'running' THEN operational_jobs\.payload/); assert.match(metadataSql, /ON CONFLICT \(job_key\) WHERE job_key IS NOT NULL/); assert.match(metadataSql, /ON CONFLICT \(chain, mint\) DO UPDATE/); assert.match(metadataSql, /warehouse-canonical-events/); assert.ok(metadataSql.endsWith("COMMIT;\n")); assert.match(redisText, /terminal_dex:hot:current/); assert.match(redisText, /terminal_dex:hot:\d+:pools/); assert.match(redisText, /terminal_dex:canonical_events/); assert.match(redisText, /MULTI/); assert.match(redisText, /EXEC/); const calls = [], spawnProcess = (command, args) => { const listeners = {}; return { stderr: { on() {} }, on(name, handler) { listeners[name] = handler; }, stdin: { end(input) { calls.push({ command, args, input }); queueMicrotask(() => listeners.close(0)); } } }; }; await syncWarehouseBatch(batch, spawnProcess, {}, facts, metadataSql, redisInput); assert.equal(calls[0].args.at(-1), "INSERT INTO terminal_dex.canonical_events FORMAT JSONEachRow"); const slotDeletes = calls.filter((row) => row.args.includes("--multiquery") && row.args.at(-1).includes("slot IN (100)")); assert.equal(slotDeletes.length, 3); assert.ok(slotDeletes.every((row) => row.args.at(-1).includes("mutations_sync = 2"))); assert.ok(calls.find((row) => row.args.at(-1) === "INSERT INTO terminal_dex.canonical_instructions FORMAT JSONEachRow")); assert.ok(calls.find((row) => row.args.at(-1) === "INSERT INTO terminal_dex.canonical_swaps FORMAT JSONEachRow")); assert.ok(calls.find((row) => row.args.at(-1) === "INSERT INTO terminal_dex.canonical_dead_letters FORMAT JSONEachRow")); const postgresCall = calls.find((row) => row.command === "psql"); assert.equal(postgresCall.input, metadataSql); assert.equal(calls.at(-1).command, "redis-cli"); assert.deepEqual(calls.at(-1).args, ["--pipe"]); assert.equal(calls.at(-1).input, redisInput);
...
test("internal evidence API exposes missing fields and immutable provenance contract", async (t) => {
  ...
  const registry = await (await fetch(`${base}/internal/registry`)).json(); assert.equal(registry.version, 8); assert.ok(registry.programs.some((row) => row.protocol === "pump-bonding-curve" && row.eventTypes.includes("curve_completed") && row.eventTypes.includes("migration_completed"))); assert.ok(registry.programs.some((row) => row.protocol === "raydium-clmm")); assert.ok(registry.programs.some((row) => row.protocol === "orca-whirlpool" && row.eventTypes.includes("pool_created"))); assert.ok(registry.programs.some((row) => row.protocol === "pump-swap" && row.eventTypes.includes("pool_created")));
});

test("WebSocket isolates replayable snapshot updates from block subscriptions", async (t) => {
  const store = new IndexStore("unused"); await store.load(); store.applyAccountSnapshot({ schemaVersion: 1, chain: "solana", genesisHash: MAINNET_GENESIS_HASH, commitment: "finalized", slot: 700, observedAt: new Date().toISOString(), mints: [{ mint: "snapshot-mint", mintInfo: {}, accounts: [] }] }); const server = createServer({ webSocketHeartbeatMs: 60_000 }, store); await new Promise((resolve) => server.listen(0, "127.0.0.1", resolve)); t.after(() => new Promise((resolve) => server.close(resolve))); const base = `ws://127.0.0.1:${server.address().port}/ws?cursor=0`;
