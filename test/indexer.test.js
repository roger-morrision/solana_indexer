...
import { decodeOrcaWhirlpoolSwapEvents, decodePumpBondingCurveInitializations, decodePumpCompletionEvents, decodePumpMigrations, decodePumpSwapEvents, decodePumpSwapPoolInitializations, decodePumpTradeEvents, decodeRaydiumClmmSwapEvents, decodeRaydiumCpmmPoolInitializations, decodeRaydiumSwapEvents, parseBlock } from "../src/parser.js";
...
test("decodes Raydium CLMM SwapEvent with exact price state and explicit unavailable reserves", async () => {
  ...
  const block = parseBlock(input), swap = block.swaps[0]; assert.equal(swap.registryVersion, 7); assert.equal(swap.decoderVersion, 1); assert.equal(swap.inputVaultBeforeRaw, null); assert.equal(swap.sqrtPriceX64, sqrtPriceX64.toString());
...
test("decodes Orca Whirlpool Traded events only with matching swap accounts and token evidence", async () => {
  ...
  const input = JSON.parse(await fs.readFile(fixture, "utf8")); input.dexEvents = []; input.transactions[0] = { transaction: { signatures: ["orca-signature"], message: entry.transaction.message }, meta: entry.meta }; const swap = parseBlock(input).swaps[0]; assert.equal(swap.protocol, "orca-whirlpool"); assert.equal(swap.registryVersion, 7); assert.equal(swap.reserveTiming, "unavailable"); assert.equal(swap.inputVaultBeforeRaw, null);
...
test("internal evidence API exposes missing fields and immutable provenance contract", async (t) => {
  ...
  const registry = await (await fetch(`${base}/internal/registry`)).json(); assert.equal(registry.version, 7); assert.ok(registry.programs.some((row) => row.protocol === "pump-bonding-curve" && row.eventTypes.includes("curve_completed") && row.eventTypes.includes("migration_completed"))); assert.ok(registry.programs.some((row) => row.protocol === "raydium-clmm")); assert.ok(registry.programs.some((row) => row.protocol === "orca-whirlpool")); assert.ok(registry.programs.some((row) => row.protocol === "pump-swap" && row.eventTypes.includes("pool_created")));
});
