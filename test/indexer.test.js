...
test("canonical finalized account snapshots persist complete holder and authority evidence", async () => {
  const client = { call: async (method, params) => { if (method === "getSlot") return 500; assert.equal(params[1].minContextSlot, 500); if (method === "getMultipleAccounts") return { context: { slot: 500 }, value: [{ data: { parsed: { info: { decimals: 6, supply: "100", mintAuthority: null, freezeAuthority: null, extensions: [] } } } }] }; if (method === "getProgramAccounts") { assert.equal(params[1].withContext, true); return { context: { slot: 500 }, value: params[0].startsWith("Tokenkeg") ? [{ pubkey: "token-a", account: { data: { parsed: { info: { mint: "mint-a", owner: "wallet-a", state: "initialized", tokenAmount: { amount: "100", decimals: 6 } } } } } }] : [] }; } throw new Error(method); } };
...
test("account snapshots reject mixed finalized RPC contexts", async () => {
  ...
  const mixedPrograms = { call: async (method) => method === "getSlot" ? 500 : method === "getMultipleAccounts" ? { context: { slot: 500 }, value: [null] } : { context: { slot: 501 }, value: [] } };
