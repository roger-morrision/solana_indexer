...
test("warehouse sync is ordered, retry-safe, and checkpoints only after both sinks", async () => {
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "warehouse-checkpoint-")), filename = path.join(root, "nested", "checkpoint.json"); await writeWarehouseCheckpoint(filename, 3, sinks); const persisted = JSON.parse(await fs.readFile(filename, "utf8")); assert.deepEqual({ schemaVersion: persisted.schemaVersion, consumer: persisted.consumer, lastSequence: persisted.lastSequence, sinks: persisted.sinks }, { schemaVersion: 1, consumer: "warehouse-canonical-events", lastSequence: 3, sinks });
});
