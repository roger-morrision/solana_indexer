...
import { claimOperationalJobSql, finishOperationalJobSql, runOperationalJobCycle, validateOperationalJob } from "../src/operational-job-worker.js";
...
test("operational worker claims leases, retries safely, and is never auto-enabled", async () => {
  ...
  const service = await fs.readFile(path.join(rootDir, "validator/solana-indexer-operational-worker.service"), "utf8"), timer = await fs.readFile(path.join(rootDir, "validator/solana-indexer-operational-worker.timer"), "utf8"), installer = await fs.readFile(path.join(rootDir, "validator/install-indexer-services.sh"), "utf8"); assert.match(service, /npm run work:operational/); assert.match(service, /NoNewPrivileges=true/); assert.match(service, /ReadWritePaths=\/home\/sol\/solana-indexer\/data/); assert.match(timer, /OnUnitActiveSec=1min/); assert.match(installer, /solana-indexer-operational-worker\.timer/); assert.doesNotMatch(installer, /^systemctl enable --now/m);
});
