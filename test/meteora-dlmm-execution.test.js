...
function hookedConfiguredMintEvidence(mint, hookProgram, discriminator, config) { const evidence = token2022MintEvidence(mint), validationAccount = deriveTransferHookValidationAccount(mint, hookProgram).address, data = Buffer.alloc(51); Buffer.from([105, 37, 101, 197, 75, 251, 102, 26]).copy(data); data.writeUInt32LE(39, 8); data.writeUInt32LE(1, 12); data[16] = discriminator; config.copy(data, 17); data[49] = 1; data[50] = 1; evidence.extensionTypes.push("transferHook"); evidence.transferHookEvidence = { schemaVersion: 1, commitment: "finalized", slot: 103, authority: null, programId: hookProgram, programExecutable: true, program: rawEvidence(address(15), Buffer.alloc(0)), validationAccount, validation: { ...rawEvidence(hookProgram, data), extraAccountMetaList: decodeTransferHookExtraAccountMetaList(data.toString("hex")) } }; return evidence; }
function finalizedRouteAccount(addressValue, owner, slot, data) { return { schemaVersion: 1, address: addressValue, owner, commitment: "finalized", slot, dataLength: data.length, rawHex: data.toString("hex"), rawPayloadHash: crypto.createHash("sha256").update(data).digest("hex") }; }
...
test("Meteora swap2 binds account-data hook resolution to hashed finalized evidence", () => {
  const args = fixture(), hookProgram = address(12), vaultData = Buffer.alloc(32), config = Buffer.alloc(32); vaultData.set([9, 8, 7, 6], 11); config.set([4, 2, 11, 4]); args.pool.tokenProgram0 = METEORA_DLMM_EXECUTION_CONSTANTS.token2022Program; args.pool.mint0Evidence = hookedConfiguredMintEvidence(args.pool.tokenMint0, hookProgram, 1, config); args.quote.inputTransferFeeRaw = "10"; args.quote.grossOutputRaw = "900"; const routeData = { [args.pool.tokenVault0]: finalizedRouteAccount(args.pool.tokenVault0, METEORA_DLMM_EXECUTION_CONSTANTS.token2022Program, 105, vaultData) };
  const instruction = buildMeteoraDlmmSwapInstruction({ ...args, minimumOutputRaw: "850", transferHookAccountData: routeData }), expectedPda = findProgramAddress(hookProgram, [Buffer.from([9, 8, 7, 6])]).address;
  assert.deepEqual(instruction.accounts.slice(-4), [{ address: expectedPda, signer: false, writable: true }, { address: args.pool.mint0Evidence.transferHookEvidence.validationAccount, signer: false, writable: false }, { address: hookProgram, signer: false, writable: false }, { address: args.pool.binArrays[0].address, signer: false, writable: true }]); assert.equal(instruction.evidence.transferHookAccountDataSlot, 105); assert.equal(instruction.evidence.transferHookAccountData[0].rawPayloadHash, routeData[args.pool.tokenVault0].rawPayloadHash);
  const preparation = prepareMeteoraDlmmSwapSimulation({ ...args, minimumOutputRaw: "850", inputPreAmountRaw: "2000", outputPreAmountRaw: "100", transferHookAccountData: routeData }); assert.equal(preparation.minContextSlot, 105);
  const corrupt = structuredClone(routeData); corrupt[args.pool.tokenVault0].rawPayloadHash = "0".repeat(64); assert.throws(() => buildMeteoraDlmmSwapInstruction({ ...args, minimumOutputRaw: "850", transferHookAccountData: corrupt }), /source-account evidence/);
});
