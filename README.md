...
| `INDEXER_WAREHOUSE_CHECKPOINT_FILE` | `data/warehouse-checkpoint.json` | Atomic mainnet/genesis-bound checkpoint with version-3 reconciliation evidence, advanced only after ClickHouse, PostgreSQL, and Redis reconcile canonical state including the exact Redis hot-token content digest |
