...
Ingestion rejects malformed provenance timestamps and network identities, unsafe or regressing source tips, negative lags, and tip/lag combinations that do not exactly match the block slot. Verified producers bind every block artifact to their genesis hash. Exporter cursor/status resume evidence is read only from stable bounded non-link regular files before any RPC or inbox mutation. The shared evidence reader allocates only the admitted byte length, reads through the verified file handle, probes for growth, and rechecks both handle and path identity before returning bytes. Legacy inputs may omit provenance and remain explicitly `unknown`, but missing or non-mainnet block identity fails index health and bot readiness; malformed supplied evidence is never silently downgraded.
...
never removes the requirement for fresh per-order evidence. Warehouse publication
...
rows carry source-derived SHA-256 hashes, and reconciliation version 11 verifies
...
| `INDEXER_WAREHOUSE_CHECKPOINT_FILE` | `data/warehouse-checkpoint.json` | Atomic mainnet/genesis-bound checkpoint with version-11 reconciliation evidence, advanced only after the canonical-event chain plus exact ClickHouse derived-fact, PostgreSQL token/pool/candidate, and Redis hot-state content digests reconcile |
| `INDEXER_WAREHOUSE_STATUS_FILE` | `data/warehouse-status.json` | Aggregate-only version-2 failed-sync evidence with a bounded stage and optional token/pool/candidate invalid-preimage counts; immediately gates warehouse REST, metrics, and bot readiness when bound to the last successful checkpoint, while any present malformed, unsafe, or wrong-network document fails closed |
...
| `LOCAL_VALIDATOR_RPCS` | unset | Comma-separated 2-4 unique loopback Agave RPC endpoints. Overrides `LOCAL_VALIDATOR_RPC`; every node must pass the configured genesis check before failover is enabled, while a previously verified node inside a Retry-After circuit retains only its already-established identity until the circuit can be probed again. |
...
| `LOCAL_RPC_COOLDOWN_MS` | `30000` | Private-node circuit cooldown; HTTP 429/503 `Retry-After` hints, including zero-padded delta-seconds, are normalized and clamped to 1 second through 1 hour so zero, stale, or excessive hints cannot hot-loop or stall ingestion |
...
| `INDEXER_STREAM_MAX_MESSAGE_BYTES` | `67108864` | Maximum UTF-8 byte length accepted for one validator PubSub message |
...
- `GET /api/v1/holders/:mint?limit=100` (allowlisted finalized snapshot and exact holder concentration facts; exclusions disclosed)
- `GET /api/v1/token-account/:address` (exact allowlisted balance/account identity with snapshot withheld evidence; mutable provenance internals are not echoed)
...
`/internal/evidence/:mint` (allowlisted evidence envelope, latest swap, and compact pool facts); `/internal/trending`; `/internal/new-pairs` (newest-first compact pool catalog rows, without raw persisted pool or snapshot internals);
...
evidence fails closed. `getIndexedTokenMetadata` accepts `{ mint }` or `[mint]`
...
histogram buckets so the documented 500 ms read p99 can alert without mixing
metrics, static, or write traffic and without request-path cardinality.
...
standard deviation, and round-open slot. Legacy Switchboard V1 parse-optimized
accounts enforce the exact type byte and SBF layout, preserve the result/range
floating-point bits and decimal representation, and use OpenBook's exact
maximum-minus-minimum deviation. The snapshot applies OpenBook's
confidence, freshness, mixed/dual-oracle, market-decimal, lot conversion,
signed overflow, and side-specific peg-limit rules. Other legacy Switchboard
account forms and mutable stub sources remain unpriced and fail closed.
...
the exact Agave version output through a deadline-bound, output-bounded probe
with redacted diagnostics, and requires a reviewed qualification recorded
...
- `npm run retention:inbox` previews old raw inbox files eligible for deletion. Archive completion first admits its manifest through a stable non-link regular-file snapshot bounded to 16 MiB, then installs only a stable bounded receipt that is byte-identical to both its generated completion object and archived copy. Retention reads bounded stable non-link state, receipt, and canonical inbox files; only parser-v2 checkpointed files whose SHA-256 matches both the checkpoint and the last successfully uploaded self-hosted archive receipt are selected, unresolved dead letters are excluded, and content is revalidated immediately before confirmed deletion. Any dead-letter capacity overflow refuses both preview and deletion because omitted failure identities make eligibility unknowable. Restore preflight applies before/open/after identity checks to its bounded control files and fixed-memory streaming hashes of every large artifact. `ops/backup.sh` installs the receipt only after the manifest and archive uploads succeed. Rerun with `-- --confirm-delete`; deletion is never implicit.
- `npm run retention:audit` validates every JSONL audit record and previews tenant-aware expiration with an exact `contentSha256`. Stop the API writer, then rerun with `-- --confirm-delete --writer-quiesced --expected-sha256=<reviewed-digest>`; changed content, missing quiescence, malformed input, or a source that no longer satisfies the same stable bounded-file contract blocks deletion, and the retained log is replaced durably.
