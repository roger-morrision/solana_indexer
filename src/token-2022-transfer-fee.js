...
function maybeNullAddress(bytes, offset) { const address = bytes.subarray(offset, offset + 32); return address.every((byte) => byte === 0) ? null : encodeBase58(address); }
...
export function decodeRawToken2022MintEvidence(account, currentEpoch, slot) {
  ...
  const mintInfo = { mintAuthority: coptionAddress(bytes, 0, "mint authority"), supply: bytes.readBigUInt64LE(36).toString(), decimals: bytes[44], freezeAuthority: coptionAddress(bytes, 46, "freeze authority") }, extensions = [], seen = new Set(); let transferFeeConfig = null;
  if (bytes.length > 82) { if (bytes.length < 170 || bytes.subarray(82, 165).some((byte) => byte !== 0) || bytes[165] !== 1) throw new Error("raw Token-2022 mint extension envelope is invalid"); let offset = 166; while (offset < bytes.length) { if (offset + 4 > bytes.length) throw new Error("truncated raw Token-2022 mint extension"); const type = bytes.readUInt16LE(offset), length = bytes.readUInt16LE(offset + 2); offset += 4; if (type === 0 && length === 0) { if (bytes.subarray(offset).some((byte) => byte !== 0)) throw new Error("raw Token-2022 mint extension padding is invalid"); break; } const name = MINT_EXTENSION_NAMES.get(type); if (!name || seen.has(type) || offset + length > bytes.length) throw new Error("raw Token-2022 mint extension is invalid"); seen.add(type); let state = null; if (type === 1) { if (length !== 108) throw new Error("raw Token-2022 transfer fee config is invalid"); transferFeeConfig = normalizeTransferFeeConfig({ withheldAmountRaw: bytes.readBigUInt64LE(offset + 64), olderTransferFee: rawFee(bytes, offset + 72), newerTransferFee: rawFee(bytes, offset + 90) }); state = transferFeeConfig; } else if (type === 14) { if (length !== 64) throw new Error("raw Token-2022 transfer hook is invalid"); state = { authority: maybeNullAddress(bytes, offset), programId: maybeNullAddress(bytes, offset + 32) }; } extensions.push(state == null ? { extension: name } : { extension: name, state }); offset += length; } }
  mintInfo.extensions = extensions;
