...
export async function runOperationalJobCycle({ workerId = crypto.randomUUID(), leaseSeconds = 300, maxAttempts = 5, baseBackoffSeconds = 30, query = (sql) => run("psql", ["--no-psqlrc", "--set", "ON_ERROR_STOP=1", "--tuples-only", "--no-align", "--quiet", "--command", sql], null, spawn, true), execute = (job) => { const script = job.type === "account_snapshot" ? "account-snapshot.js" : job.type === "cpmm_pool_snapshot" ? "cpmm-pool-snapshot.js" : job.type === "pump_swap_pool_snapshot" ? "pump-swap-pool-snapshot.js" : job.type === "pump_bonding_curve_snapshot" ? "pump-bonding-curve-snapshot.js" : job.type === "orca_pool_snapshot" ? "orca-pool-snapshot.js" : job.type === "meteora_dlmm_pool_snapshot" ? "meteora-dlmm-pool-snapshot.js" : job.type === "offchain_metadata_snapshot" ? "offchain-metadata-snapshot.js" : "clmm-pool-snapshot.js", extra = job.type === "offchain_metadata_snapshot" ? [job.onchainMetadataHash, job.uri] : []; return run(process.execPath, [path.join(path.dirname(fileURLToPath(import.meta.url)), script), "--artifact-only", job.target, ...extra]); }, schedule = setInterval, cancel = clearInterval } = {}) {
