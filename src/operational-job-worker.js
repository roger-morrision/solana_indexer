...
export function validateOperationalJob(row) {
  if (!Number.isSafeInteger(row?.id) || row.id < 1 || !["account_snapshot", "clmm_pool_snapshot", "orca_pool_snapshot"].includes(row.job_type) || row.payload?.schemaVersion !== 1 || row.payload?.chain !== "solana-mainnet") throw new Error("invalid claimed operational job"); const target = row.job_type === "account_snapshot" ? row.payload.mint : row.payload.pool; if (!ADDRESS.test(target ?? "")) throw new Error("invalid operational job target"); return { id: row.id, attempts: row.attempts, type: row.job_type, target };
}
...
export async function runOperationalJobCycle({ workerId = crypto.randomUUID(), leaseSeconds = 300, maxAttempts = 5, baseBackoffSeconds = 30, query = (sql) => run("psql", ["--no-psqlrc", "--set", "ON_ERROR_STOP=1", "--tuples-only", "--no-align", "--quiet", "--command", sql], null, spawn, true), execute = (job) => { const script = job.type === "account_snapshot" ? "account-snapshot.js" : job.type === "orca_pool_snapshot" ? "orca-pool-snapshot.js" : "clmm-pool-snapshot.js"; return run(process.execPath, [path.join(path.dirname(fileURLToPath(import.meta.url)), script), "--artifact-only", job.target]); }, schedule = setInterval, cancel = clearInterval } = {}) {
