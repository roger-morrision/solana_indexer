...
function signedTransactionIdentity(transactionBase64) { if (typeof transactionBase64 !== "string" || !/^[A-Za-z0-9+/]+={0,2}$/.test(transactionBase64)) throw new Error("landed transaction encoding is invalid"); const bytes = Buffer.from(transactionBase64, "base64"), signatures = decodeShortVec(bytes), messageOffset = signatures.offset + signatures.value * 64; if (bytes.toString("base64").replace(/=+$/, "") !== transactionBase64.replace(/=+$/, "") || bytes.length > MAX_TRANSACTION_BYTES || signatures.value < 1 || signatures.value > MAX_SIGNATURES || messageOffset >= bytes.length) throw new Error("landed transaction signatures are invalid"); const firstSignature = bytes.subarray(signatures.offset, signatures.offset + 64); if (firstSignature.every((byte) => byte === 0)) throw new Error("landed transaction signatures are invalid"); return { messageHash: crypto.createHash("sha256").update(bytes.subarray(messageOffset)).digest("hex"), firstSignature: base58(firstSignature) }; }
...
export async function verifyFinalizedLandedTransaction(client, { signature, simulationReceipt, expectedGenesisHash, genesisHash }) {
  ...
  const landed = await client.call("getTransaction", [signature, { commitment: "finalized", encoding: "base64", maxSupportedTransactionVersion: 0 }]); if (!landed || landed.slot !== status.slot || landed.meta?.err !== null || !Array.isArray(landed.transaction) || landed.transaction[1] !== "base64") throw new Error("finalized transaction does not match the simulated message"); const identity = signedTransactionIdentity(landed.transaction[0]); if (identity.messageHash !== simulationReceipt.messageHash || identity.firstSignature !== signature) throw new Error("finalized transaction does not match the simulated message and signature");
