...
export function compileWarehouseProjections(store, staleAfterMs = 120_000, limit = 500) {
  ...
  for (const [pool, row] of Object.entries(store.state.pools).sort(([left], [right]) => left.localeCompare(right))) if (row.protocol === "orca-whirlpool" && (!row.accountSnapshot || stale(row.accountSnapshot.observedAt))) jobs.push({ key: `orca-pool-snapshot:${pool}`, type: "orca_pool_snapshot", payload: { schemaVersion: 1, chain: CHAIN, pool, reason: !row.accountSnapshot ? "missing" : "stale", canonicalAsOf: new Date(now).toISOString(), sourceSlot: row.accountSnapshot?.stateSlot ?? null } });
