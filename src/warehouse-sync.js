...
export function compileWarehouseFacts(state, batch) {
  ...
  const nativeTransfers = selected(state.nativeTransfers).map((row) => { const base = factBase(row); if (typeof row.transferId !== "string" || !row.transferId || !Number.isInteger(row.instructionIndex) || row.instructionIndex < 0 || (row.innerIndex != null && (!Number.isInteger(row.innerIndex) || row.innerIndex < 0)) || row.programId !== "11111111111111111111111111111111" || typeof row.source !== "string" || !row.source || typeof row.destination !== "string" || !row.destination || !/^\d+$/.test(row.lamportsRaw ?? "") || BigInt(row.lamportsRaw) > 18_446_744_073_709_551_615n || row.decoderVersion !== 1 || !/^([0-9a-f]{64})$/.test(row.rawPayloadHash ?? "")) throw new Error("invalid canonical native transfer fact"); unique(`native-transfer:${row.transferId}`); return { ...base, transfer_id: row.transferId, instruction_index: row.instructionIndex, inner_index: row.innerIndex, program_id: row.programId, source: row.source, destination: row.destination, lamports_raw: row.lamportsRaw, decoder_version: row.decoderVersion, raw_payload_hash: row.rawPayloadHash, payload: JSON.stringify(row) }; });
