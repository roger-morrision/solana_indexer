...
export function compileWarehouseFacts(state, batch) {
  ...
  const nativeTransfers = selected(state.nativeTransfers).map((row) => { const base = factBase(row), accountCreation = row.transferKind === "account_creation"; if (typeof row.transferId !== "string" || !row.transferId || !Number.isInteger(row.instructionIndex) || row.instructionIndex < 0 || (row.innerIndex != null && (!Number.isInteger(row.innerIndex) || row.innerIndex < 0)) || row.programId !== "11111111111111111111111111111111" || !["transfer", "account_creation"].includes(row.transferKind) || typeof row.source !== "string" || !row.source || typeof row.destination !== "string" || !row.destination || !/^\d+$/.test(row.lamportsRaw ?? "") || BigInt(row.lamportsRaw) > 18_446_744_073_709_551_615n || (accountCreation && (!/^\d+$/.test(row.allocatedSpaceRaw ?? "") || BigInt(row.allocatedSpaceRaw) > 18_446_744_073_709_551_615n || typeof row.ownerProgram !== "string" || !row.ownerProgram)) || (!accountCreation && (row.allocatedSpaceRaw != null || row.ownerProgram != null)) || row.decoderVersion !== 2 || !/^([0-9a-f]{64})$/.test(row.rawPayloadHash ?? "")) throw new Error("invalid canonical native transfer fact"); unique(`native-transfer:${row.transferId}`); return { ...base, transfer_id: row.transferId, instruction_index: row.instructionIndex, inner_index: row.innerIndex, program_id: row.programId, transfer_kind: row.transferKind, source: row.source, destination: row.destination, lamports_raw: row.lamportsRaw, allocated_space_raw: row.allocatedSpaceRaw, owner_program: row.ownerProgram, decoder_version: row.decoderVersion, raw_payload_hash: row.rawPayloadHash, payload: JSON.stringify(row) }; });
...
export function compileWarehouseProjections(store, staleAfterMs = 120_000, limit = 500) {
  const blockTimes = Object.values(store?.state?.blocks ?? {}).map((row) => Number(row?.blockTime) * 1_000).filter(Number.isFinite), now = blockTimes.length ? blockTimes.reduce((latest, value) => Math.max(latest, value), 0) : Date.parse(store?.state?.updatedAt ?? ""); if (!Number.isFinite(now) || !Number.isInteger(staleAfterMs) || staleAfterMs < 1_000 || !Number.isInteger(limit) || limit < 1 || limit > 10_000) throw new Error("invalid warehouse projection context");
  ...
  const stale = (observedAt) => { const observed = Date.parse(observedAt ?? ""); return !Number.isFinite(observed) || observed > now || now - observed > staleAfterMs; }, jobs = [];
  for (const mint of Object.keys(store.state.mints).sort()) { const snapshot = store.state.holderSnapshots[mint]; if (!snapshot || stale(snapshot.observedAt)) jobs.push({ key: `account-snapshot:${mint}`, type: "account_snapshot", payload: { schemaVersion: 1, chain: CHAIN, mint, reason: !snapshot ? "missing" : "stale", canonicalAsOf: new Date(now).toISOString(), sourceSlot: snapshot?.slot ?? null } }); }
  for (const [mint, row] of Object.entries(store.state.mints).sort(([left], [right]) => left.localeCompare(right))) { const canonical = row.metadata, enriched = row.offchainMetadata, enrichmentAge = now - Date.parse(enriched?.observedAt ?? ""), refresh = !enriched || !Number.isFinite(enrichmentAge) || enrichmentAge < 0 || enrichmentAge > 86_400_000; if (canonical?.uri?.startsWith("https://") && /^[0-9a-f]{64}$/.test(canonical.rawPayloadHash ?? "") && refresh) jobs.push({ key: `offchain-metadata-snapshot:${mint}`, type: "offchain_metadata_snapshot", payload: { schemaVersion: 1, chain: CHAIN, mint, uri: canonical.uri, onchainMetadataHash: canonical.rawPayloadHash, reason: !enriched ? "missing" : "stale", canonicalAsOf: new Date(now).toISOString(), sourceSlot: row.authoritySourceSlot ?? row.lastSlot ?? null } }); }
  for (const [pool, row] of Object.entries(store.state.pools).sort(([left], [right]) => left.localeCompare(right))) if (row.protocol === "raydium-clmm" && (!row.accountSnapshot || stale(row.accountSnapshot.observedAt))) jobs.push({ key: `clmm-pool-snapshot:${pool}`, type: "clmm_pool_snapshot", payload: { schemaVersion: 1, chain: CHAIN, pool, reason: !row.accountSnapshot ? "missing" : "stale", canonicalAsOf: new Date(now).toISOString(), sourceSlot: row.accountSnapshot?.stateSlot ?? null } });
  for (const [pool, row] of Object.entries(store.state.pools).sort(([left], [right]) => left.localeCompare(right))) if (row.protocol === "orca-whirlpool") { const snapshot = row.accountSnapshot, incomplete = snapshot?.tickArrayCoverage !== "finalized_program_account_snapshot"; if (!snapshot || incomplete || stale(snapshot.observedAt)) jobs.push({ key: `orca-pool-snapshot:${pool}`, type: "orca_pool_snapshot", payload: { schemaVersion: 1, chain: CHAIN, pool, reason: !snapshot ? "missing" : incomplete ? "incomplete_tick_array_coverage" : "stale", canonicalAsOf: new Date(now).toISOString(), sourceSlot: snapshot?.stateSlot ?? null } }); }
...
export function compileWarehouseMetadataSql(state, sequence, projections = { candidates: [], security: [], jobs: [] }) {
  ...
  const candidateRows = projections.candidates.map((row) => { if (!row.mint || !row.scoreVersion || !Number.isSafeInteger(row.score) || !["review_required", "eligible", "rejected"].includes(row.state)) throw new Error("invalid warehouse candidate row"); return `(${sqlLiteral(CHAIN)}, ${sqlLiteral(row.mint)}, ${sqlLiteral(row.scoreVersion)}, ${jsonbLiteral(row.evidence)}, ${row.score}, ${sqlLiteral(row.state)})`; });
  const securityRows = projections.security.map((row) => { if (!row.mint || !row.ruleVersion || !Number.isSafeInteger(row.sourceSlot) || row.sourceSlot < 0 || !["unknown", "low", "medium", "high"].includes(row.severity) || typeof row.blocksResearch !== "boolean" || typeof row.blocksEntry !== "boolean" || typeof row.blocksExit !== "boolean" || !Number.isFinite(Date.parse(row.observedAt)) || !Number.isFinite(Date.parse(row.expiresAt))) throw new Error("invalid warehouse security row"); return `(${sqlLiteral(CHAIN)}, ${sqlLiteral(row.mint)}, ${sqlLiteral(row.ruleVersion)}, ${row.sourceSlot}, NULL, ${jsonbLiteral(row.evidence)}, ${sqlLiteral(row.severity)}, ${row.blocksResearch}, ${row.blocksEntry}, ${row.blocksExit}, ${sqlLiteral(row.observedAt)}, ${sqlLiteral(row.expiresAt)})`; });
  const jobRows = (projections.jobs ?? []).map((row) => { if (!/^[a-z0-9_-]+:[^\u0000-\u001f]+$/.test(row.key ?? "") || !["account_snapshot", "clmm_pool_snapshot", "orca_pool_snapshot", "offchain_metadata_snapshot"].includes(row.type) || row.payload?.schemaVersion !== 1 || row.payload?.chain !== CHAIN) throw new Error("invalid warehouse operational job"); return `(${sqlLiteral(row.key)}, ${sqlLiteral(row.type)}, ${jsonbLiteral(row.payload)}, 'pending', 0, now())`; });
  const activeJobKeys = `ARRAY[${(projections.jobs ?? []).map((row) => sqlLiteral(row.key)).join(", ")}]::text[]`;
  return `BEGIN;\n${rows.length ? `INSERT INTO tokens (chain, mint, decimals, metadata, authorities, extensions, source_slot) VALUES\n${rows.join(",\n")}\nON CONFLICT (chain, mint) DO UPDATE SET decimals = EXCLUDED.decimals, metadata = EXCLUDED.metadata, authorities = EXCLUDED.authorities, extensions = EXCLUDED.extensions, source_slot = EXCLUDED.source_slot, updated_at = now();\n` : ""}${candidateRows.length ? `INSERT INTO candidates (chain, mint, score_version, evidence_snapshot, score, state) VALUES\n${candidateRows.join(",\n")}\nON CONFLICT (chain, mint, score_version) DO UPDATE SET evidence_snapshot = EXCLUDED.evidence_snapshot, score = EXCLUDED.score, state = EXCLUDED.state, updated_at = now();\n` : ""}${securityRows.length ? `INSERT INTO security_snapshots (chain, mint, rule_version, source_slot, source_signature, evidence, severity, blocks_research, blocks_entry, blocks_exit, observed_at, expires_at) VALUES\n${securityRows.join(",\n")}\nON CONFLICT (chain, mint, rule_version, source_slot) WHERE source_slot IS NOT NULL DO UPDATE SET evidence = EXCLUDED.evidence, severity = EXCLUDED.severity, blocks_research = EXCLUDED.blocks_research, blocks_entry = EXCLUDED.blocks_entry, blocks_exit = EXCLUDED.blocks_exit, observed_at = EXCLUDED.observed_at, expires_at = EXCLUDED.expires_at;\n` : ""}UPDATE operational_jobs SET status = 'cancelled', updated_at = now() WHERE job_type IN ('account_snapshot', 'clmm_pool_snapshot', 'orca_pool_snapshot', 'offchain_metadata_snapshot') AND status = 'pending' AND NOT (job_key = ANY(${activeJobKeys}));\n${jobRows.length ? `INSERT INTO operational_jobs (job_key, job_type, payload, status, attempts, available_at) VALUES\n${jobRows.join(",\n")}\nON CONFLICT (job_key) WHERE job_key IS NOT NULL DO UPDATE SET job_type = CASE WHEN operational_jobs.status = 'running' THEN operational_jobs.job_type ELSE EXCLUDED.job_type END, payload = CASE WHEN operational_jobs.status = 'running' THEN operational_jobs.payload ELSE EXCLUDED.payload END, status = CASE WHEN operational_jobs.status = 'running' THEN operational_jobs.status ELSE 'pending' END, attempts = CASE WHEN operational_jobs.status = 'running' THEN operational_jobs.attempts ELSE 0 END, available_at = CASE WHEN operational_jobs.status = 'running' THEN operational_jobs.available_at ELSE now() END, updated_at = now();\n` : ""}${checkpointSql(sequence)}COMMIT;\n`;
}
...
export function assessWarehouseCheckpoint(checkpoint, eventSequence, oldestSequence, staleAfterMs = 300_000, maxLagEvents = 1_000, now = Date.now()) {
  ...
  const reconciliation = checkpoint.reconciliation; if (!reconciliation || reconciliation.schemaVersion !== 1 || reconciliation.verified !== true || reconciliation.sequence !== sequence || ["events", "instructions", "swaps", "balanceChanges", "nativeTransfers", "deadLetters", "tokens", "pools"].some((key) => !Number.isSafeInteger(reconciliation[key]) || reconciliation[key] < 0)) return { ...unavailable("content_reconciliation_unavailable"), sequence, sinks };
...
export function validateWarehouseReconciliation(expected, values) {
  const keys = ["sequence", "events", "instructions", "swaps", "balanceChanges", "nativeTransfers", "deadLetters", "tokens", "pools"]; if (!expected || keys.some((key) => !Number.isSafeInteger(expected[key]) || expected[key] < 0)) throw new Error("invalid expected warehouse reconciliation");
  const clickhouse = numericLines(values?.clickhouse, 6, "clickhouse"), postgres = numericLines(values?.postgres, 2, "postgres"), redis = numericLines(values?.redis, 3, "redis"), actual = { sequence: postgres[0], events: clickhouse[0], instructions: clickhouse[1], swaps: clickhouse[2], balanceChanges: clickhouse[3], nativeTransfers: clickhouse[4], deadLetters: clickhouse[5], tokens: postgres[1], pools: redis[1] };
...
export async function probeWarehouseReconciliation(expected, spawnProcess = spawn, env = process.env) {
  const [clickhouse, postgres, redis] = await Promise.all([
    captureProcess("clickhouse-client", ["--query", "SELECT uniqExact(sequence) FROM terminal_dex.canonical_events UNION ALL SELECT uniqExact(event_id) FROM terminal_dex.canonical_instructions UNION ALL SELECT uniqExact(swap_id) FROM terminal_dex.canonical_swaps UNION ALL SELECT uniqExact(tuple(signature, account_index)) FROM terminal_dex.canonical_balance_changes UNION ALL SELECT uniqExact(transfer_id) FROM terminal_dex.canonical_native_transfers UNION ALL SELECT uniqExact(identity) FROM terminal_dex.canonical_dead_letters FORMAT TabSeparatedRaw"], spawnProcess, env),
