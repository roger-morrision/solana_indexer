...
export async function preflightBackup(directory, { now = Date.now(), maximumAgeMs = 24 * 60 * 60 * 1_000, expectedBackupId = null } = {}) {
  ...
  if (manifest.schemaVersion !== 2 || manifest.chain !== "solana" || manifest.scope !== "postgres-clickhouse-redis-indexer-state" || manifest.writersQuiesced !== true || !BACKUP_ID.test(manifest.backupId ?? "") || (expectedBackupId !== null && manifest.backupId !== expectedBackupId) || !Number.isFinite(createdAtMs) || new Date(createdAtMs).toISOString() !== manifest.createdAt || createdAtMs > now || now - createdAtMs > maximumAgeMs || JSON.stringify(artifactNames) !== JSON.stringify([...ARTIFACTS].sort())) throw new Error("backup manifest is invalid, future-dated, outside RPO, or not quiesced");
  ...
  const members = await tarInventory(path.join(root, "indexer-state.tar")); return { schemaVersion: 2, kind: "backup_restore_preflight", backupId: manifest.backupId, backupManifestSha256: evidence.get("manifest.json").sha256, createdAt: manifest.createdAt, ageMs: now - createdAtMs, checksumFiles: sums.size, artifactBytes: [...ARTIFACTS].reduce((sum, name) => sum + evidence.get(name).bytes, 0), tarMembers: members, restoreAuthorized: false, invariants: { completeInventory: true, checksums: true, manifestEvidence: true, writersQuiesced: true, inboxIdentity: true, rpo: true, safeTar: true, canonicalStatePresent: true } };
}

export async function createBackupManifest(directory, { backupId, createdAt = new Date().toISOString() } = {}) { const createdAtMs = Date.parse(createdAt); if (!path.isAbsolute(directory) || !BACKUP_ID.test(backupId ?? "") || !Number.isFinite(createdAtMs) || new Date(createdAtMs).toISOString() !== createdAt) throw new Error("invalid backup manifest parameters"); const root = path.resolve(directory); if (root === path.parse(root).root) throw new Error("invalid backup manifest parameters"); const artifacts = {}; for (const name of ARTIFACTS) artifacts[name] = await artifactEvidence(root, name); const manifest = { schemaVersion: 2, backupId, createdAt, chain: "solana", scope: "postgres-clickhouse-redis-indexer-state", writersQuiesced: true, artifacts }; await writeAtomic(path.join(root, "manifest.json"), manifest); return manifest; }
