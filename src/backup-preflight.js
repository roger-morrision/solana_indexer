...
export async function preflightBackup(directory, { now = Date.now(), maximumAgeMs = 24 * 60 * 60 * 1_000, expectedBackupId = null } = {}) {
  ...
  const members = await tarInventory(path.join(root, "indexer-state.tar")); return { schemaVersion: 2, kind: "backup_restore_preflight", backupId: manifest.backupId, createdAt: manifest.createdAt, ageMs: now - createdAtMs, checksumFiles: sums.size, artifactBytes: [...ARTIFACTS].reduce((sum, name) => sum + evidence.get(name).bytes, 0), tarMembers: members, restoreAuthorized: false, invariants: { completeInventory: true, checksums: true, manifestEvidence: true, writersQuiesced: true, inboxIdentity: true, rpo: true, safeTar: true, canonicalStatePresent: true } };
}
