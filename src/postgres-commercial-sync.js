...
export function buildCommercialSyncSql(registry, auditText) {
  ...
  for (const [index, line] of auditText.split(/\r?\n/).filter(Boolean).entries()) { let row; try { row = JSON.parse(line); } catch { throw new Error(`invalid audit JSON at line ${index + 1}`); } const observed = Date.parse(row?.observedAt ?? ""), statusClass = Math.floor(Number(row?.statusCode) / 100), quotaUnits = row?.quotaUnits ?? 1; if (row?.schemaVersion !== 1 || !row.tenantId || !tenantIds.has(row.tenantId) || !Number.isFinite(observed) || typeof row.path !== "string" || !row.path.startsWith("/") || /[\u0000-\u001f]/.test(row.path) || !Number.isFinite(row.durationMs) || row.durationMs < 0 || !Number.isInteger(quotaUnits) || quotaUnits < 1 || quotaUnits > 100 || statusClass < 1 || statusClass > 5) throw new Error(`invalid tenant usage audit at line ${index + 1}`); const bucket = new Date(Math.floor(observed / 3_600_000) * 3_600_000).toISOString(), key = `${row.tenantId}\u0000${bucket}\u0000${row.path}\u0000${statusClass}`, current = usage.get(key) ?? { tenantId: row.tenantId, bucket, route: row.path, statusClass, requests: 0, durationMs: 0 }; current.requests += quotaUnits; current.durationMs += row.durationMs; usage.set(key, current); }
