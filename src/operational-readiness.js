...
export function assessProviderConfiguration(env = process.env) {
  ...
  if (localValue || localWsValue) { try { const endpoints = String(localValue ?? "").split(",").map((value) => value.trim()).filter(Boolean), wsEndpoints = String(localWsValue ?? "").split(",").map((value) => value.trim()).filter(Boolean); if (!endpoints.length || endpoints.length > 4 || endpoints.length !== wsEndpoints.length || new Set(endpoints).size !== endpoints.length || new Set(wsEndpoints).size !== wsEndpoints.length) throw new Error("invalid local provider topology"); endpoints.forEach(validateLocalRpcUrl); wsEndpoints.forEach(validateLocalWsUrl); return { available: true, healthy: true, reason: null, mode: "local_validator" }; } catch { return { available: true, healthy: false, reason: "provider_configuration_invalid", mode: null }; } }
