...
export class IndexStore {
  ...
  evidence(mint, staleAfterMs = 120_000, now = Date.now()) {
    const token = this.mint(mint, 100), security = this.tokenSecurity(mint), usdReference = this.referencePrice(mint, staleAfterMs, now); const swaps = this.state.swaps.filter((row) => row.inputMint === mint || row.outputMint === mint).sort((a, b) => b.slot - a.slot); const pools = [...new Set(swaps.map((row) => row.pool))].map((address) => this.pool(address).summary).filter(Boolean); const latest = swaps[0] ?? null; const ageMs = latest?.blockTime == null ? null : now - latest.blockTime * 1_000; const observedInFuture = ageMs != null && ageMs < 0; const missing = [];
