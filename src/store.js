...
export class IndexStore {
  ...
  applyPoolSnapshot(snapshot) {
    if (snapshot?.schemaVersion !== 1 || snapshot.type !== "raydium_clmm_pool_snapshot" || snapshot.commitment !== "finalized" || !Number.isSafeInteger(snapshot.stateSlot) || !Number.isSafeInteger(snapshot.balanceSlot) || snapshot.balanceSlot < snapshot.stateSlot || !Array.isArray(snapshot.pools)) throw new Error("invalid finalized CLMM pool snapshot");
