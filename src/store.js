...
export class IndexStore {
  ...
  applyPoolSnapshot(snapshot) {
    if (snapshot?.schemaVersion !== 1 || snapshot.type !== "raydium_clmm_pool_snapshot" || snapshot.commitment !== "finalized" || typeof snapshot.genesisHash !== "string" || !snapshot.genesisHash || !Number.isFinite(Date.parse(snapshot.observedAt ?? "")) || !Number.isSafeInteger(snapshot.stateSlot) || !Number.isSafeInteger(snapshot.balanceSlot) || snapshot.balanceSlot < snapshot.stateSlot || !Array.isArray(snapshot.pools)) throw new Error("invalid finalized CLMM pool snapshot");
    for (const row of snapshot.pools) { if (!row.address || row.programId !== "CAMMCzo5YL8w4VFF8KVHrK22GGUsp5VTaW7grrKgrWqK" || !row.tokenMint0 || !row.tokenMint1 || !row.tokenVault0 || !row.tokenVault1 || !/^\d+$/.test(row.vault0AmountRaw) || !/^\d+$/.test(row.vault1AmountRaw) || !/^\d+$/.test(row.liquidityRaw) || !/^\d+$/.test(row.sqrtPriceX64) || !Number.isInteger(row.tick) || !Number.isInteger(row.tickSpacing)) throw new Error("invalid CLMM pool snapshot row"); const prior = this.state.poolSnapshots[row.address]; if (prior && prior.balanceSlot > snapshot.balanceSlot) continue; this.state.poolSnapshots[row.address] = { ...row, commitment: "finalized", stateSlot: snapshot.stateSlot, balanceSlot: snapshot.balanceSlot, observedAt: snapshot.observedAt, genesisHash: snapshot.genesisHash }; }
  ...
  evidence(mint, staleAfterMs = 120_000, now = Date.now()) {
    const token = this.mint(mint, 100), security = this.tokenSecurity(mint), usdReference = this.referencePrice(mint, staleAfterMs, now); const swaps = this.state.swaps.filter((row) => row.inputMint === mint || row.outputMint === mint).sort((a, b) => b.slot - a.slot); const pools = [...new Set(swaps.map((row) => row.pool))].map((address) => this.pool(address).summary).filter(Boolean); const latest = swaps[0] ?? null; const ageMs = latest?.blockTime == null ? null : Math.max(0, now - latest.blockTime * 1_000); const missing = [];
  ...
  referencePrice(mint, staleAfterMs = 120_000, now = Date.now()) {
    ...
    const eligible = this.state.swaps.filter((row) => row.provenance?.commitment === "finalized" && Number.isInteger(row.inputDecimals) && Number.isInteger(row.outputDecimals) && /^\d+$/.test(row.inputAmountRaw) && /^\d+$/.test(row.outputAmountRaw) && row.blockTime != null && now - row.blockTime * 1_000 <= staleAfterMs && BigInt(row.inputAmountRaw) > 0n && BigInt(row.outputAmountRaw) > 0n).sort((a, b) => b.slot - a.slot || (b.eventIndex ?? 0) - (a.eventIndex ?? 0)); const edges = new Map();
