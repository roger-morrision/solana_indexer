...
export class IndexStore {
  ...
  rebuildAggregates() {
    ...
    for (const [mint, snapshot] of Object.entries(this.state.holderSnapshots)) { const row = this.state.mints[mint] ?? { transferCount: 0, swapCount: 0, lastSlot: snapshot.slot, lastBlockTime: null }; row.mintInfo = this.state.mints[mint]?.mintInfo ?? snapshot.mintInfo ?? null; row.authoritySourceSlot = snapshot.slot; this.state.mints[mint] = row; }
  }
  prune() {
  ...
  poolRisk(address, staleAfterMs = 120_000, now = Date.now()) {
    ...
    const summary = this.state.pools[address] ?? null, mints = [...new Set(swaps.flatMap((row) => [row.baseMint, row.quoteMint]).filter(Boolean))], security = mints.map((mint) => this.tokenSecurity(mint)), holders = mints.map((mint) => this.holders(mint, 10)); const manipulationAssessable = swaps.length >= 20 && decodedTraderCount * 10 >= swaps.length * 9, holderAssessable = holders.length === 2 && holders.every((row) => row.complete && row.exclusionsApplied), securityPass = security.length === 2 && security.every((row) => row.assessable && !row.findings.some((finding) => finding.blocksAutomation)); const liquidity = { assessable: Boolean(summary && (summary.inputVaultBeforeRaw != null || summary.realTokenReservesRaw != null)), venueType: summary?.venueType ?? null, reserveTiming: summary?.reserveTiming ?? null, inputVaultBeforeRaw: summary?.inputVaultBeforeRaw ?? null, outputVaultBeforeRaw: summary?.outputVaultBeforeRaw ?? null, realTokenReservesRaw: summary?.realTokenReservesRaw ?? null, realQuoteReservesRaw: summary?.realQuoteReservesRaw ?? null };
