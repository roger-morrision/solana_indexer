...
export class IndexStore {
  ...
  apply(block) {
    ...
    for (const event of block.poolLifecycleEvents ?? []) this.state.pools[event.pool] = { ...(this.state.pools[event.pool] ?? {}), ...poolLifecycleProjection(event) };
  ...
  rebuildAggregates() {
    ...
    for (const event of this.state.programEvents) if (event.type === "pool_created" && event.pool) this.state.pools[event.pool] = poolLifecycleProjection(event);
