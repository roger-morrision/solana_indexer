...
function poolLifecycleProjection(event) { return { protocol: event.protocol, destinationProtocol: event.destinationProtocol ?? null, venueType: event.venueType ?? "amm", baseMint: event.tokenMint0, quoteMint: event.tokenMint1, pairIdentitySource: "protocol_instruction", lifecycle: { type: event.type, slot: event.slot, eventIndex: event.innerIndex ?? 0, blockTime: event.blockTime, signature: event.signature, creator: event.creator ?? null, migrator: event.migrator ?? null, user: event.user ?? null, sourcePool: event.sourcePool ?? null, migrationVersion: event.migrationVersion ?? null, completedAtUnix: event.completedAtUnix ?? null, migratedBaseAmountRaw: event.migratedBaseAmountRaw ?? null, migratedQuoteAmountRaw: event.migratedQuoteAmountRaw ?? null, poolMigrationFeeRaw: event.poolMigrationFeeRaw ?? null, poolAuthority: event.poolAuthority ?? null, ammConfig: event.ammConfig ?? null, globalConfig: event.globalConfig ?? null, payer: event.payer ?? null, lpMint: event.lpMint ?? null, tokenVault0: event.tokenVault0 ?? null, tokenVault1: event.tokenVault1 ?? null, observationKey: event.observationKey ?? null, poolIndex: event.poolIndex ?? null, initialAmount0Raw: event.initialAmount0Raw ?? null, initialAmount1Raw: event.initialAmount1Raw ?? null, requestedOpenTime: event.requestedOpenTime ?? null, coinCreator: event.coinCreator ?? null, mayhemMode: event.mayhemMode ?? null, cashbackCoin: event.cashbackCoin ?? null, name: event.name ?? null, symbol: event.symbol ?? null, uri: event.uri ?? null, baseTokenProgram: event.baseTokenProgram ?? null, quoteTokenProgram: event.quoteTokenProgram ?? null } }; }
...
export class IndexStore {
  ...
  rebuildAggregates() {
    ...
    for (const event of this.state.programEvents) if (["pool_created", "pool_migrated", "curve_completed", "migration_completed"].includes(event.type) && event.pool) this.state.pools[event.pool] = mergePoolLifecycle(this.state.pools[event.pool], event);
  ...
  holders(mint, limit = 100, staleAfterMs = null, now = Date.now()) {
    ...
    return { mint, coverage: complete ? "finalized_program_account_snapshot_plus_changes" : snapshotComplete ? "stale_finalized_snapshot_plus_observed_changes" : "observed_changes_only", complete, snapshotComplete, concentrationAssessable: complete, safeForAutomation: false, exclusionsApplied: false, freshness: { ageMs, staleAfterMs, observedInFuture, stale: Boolean(snapshot) && !fresh }, snapshot, observedTokenAccounts: accounts.length, observedHolders: ranked.length, observedRaw: observedRaw.toString(), top10ObservedRaw: top10Raw.toString(), concentration: observedRaw > 0n ? { numeratorRaw: top10Raw.toString(), denominatorRaw: observedRaw.toString() } : null, holders: ranked.slice(0, limit).map((row) => ({ ...row, amountRaw: row.amountRaw.toString() })) };
  }
