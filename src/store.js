...
export class IndexStore {
  ...
  applyPoolSnapshot(snapshot) {
    ...
    for (const row of snapshot.pools) { const tickArrays = row.tickArrays ?? [], tickArraySlot = row.tickArraySlot ?? null; if (!row.address || seenPools.has(row.address) || row.programId !== "CAMMCzo5YL8w4VFF8KVHrK22GGUsp5VTaW7grrKgrWqK" || !row.tokenMint0 || !row.tokenMint1 || !row.tokenVault0 || !row.tokenVault1 || !/^\d+$/.test(row.vault0AmountRaw) || !/^\d+$/.test(row.vault1AmountRaw) || !/^\d+$/.test(row.liquidityRaw) || !/^\d+$/.test(row.sqrtPriceX64) || !Number.isInteger(row.tick) || !Number.isInteger(row.tickSpacing) || !Array.isArray(tickArrays) || (tickArrays.length ? !Number.isSafeInteger(tickArraySlot) || tickArraySlot < snapshot.stateSlot || tickArraySlot > snapshot.balanceSlot : tickArraySlot !== null)) throw new Error("invalid CLMM pool snapshot row"); const tickAddresses = new Set(); for (const array of tickArrays) { if (!array.address || tickAddresses.has(array.address) || array.pool !== row.address || !Number.isInteger(array.startTickIndex) || !Number.isInteger(array.initializedTickCount) || array.initializedTickCount < 0 || array.initializedTickCount > 60 || !/^\d+$/.test(array.recentEpoch) || !/^[0-9a-f]{64}$/.test(array.rawPayloadHash)) throw new Error("invalid CLMM tick array snapshot row"); tickAddresses.add(array.address); } seenPools.add(row.address); }
