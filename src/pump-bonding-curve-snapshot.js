...
export async function createPumpBondingCurveSnapshot({ client, mints, automaticMintEvidence = false, genesisHash, observedAt = new Date().toISOString() }) {
  ...
  const curves = mints.map((mint, index) => decodePumpBondingCurveAccount(mint, addresses[index], stateResponse.value[index])), mintAddresses = [...new Set(curves.flatMap((curve) => [curve.mint, ...(curve.quoteMint === DEFAULT_PUBLIC_KEY ? [] : [curve.quoteMint])]))], mintResponse = await getMultipleAccountsBatched(client, mintAddresses, { commitment: "finalized", encoding: "jsonParsed", minContextSlot: stateSlot }, { label: "Pump base and quote mint" }), mintSlot = mintResponse?.context?.slot;
  if (!Number.isSafeInteger(mintSlot) || mintSlot < stateSlot || mintResponse.value?.length !== mintAddresses.length) throw new Error("invalid Pump mint response"); const mintEvidence = new Map(mintAddresses.map((mint, index) => { const account = mintResponse.value[index], info = account?.data?.parsed?.info; if (![SPL_TOKEN_PROGRAM, TOKEN_2022_PROGRAM].includes(account?.owner) || !/^\d+$/.test(info?.supply ?? "") || !Number.isInteger(info?.decimals) || info.decimals < 0 || info.decimals > 255) throw new Error(`invalid Pump mint account ${mint}`); return [mint, { programId: account.owner, decimals: info.decimals, supplyRaw: info.supply }]; }));
  for (const curve of curves) { const base = mintEvidence.get(curve.mint), quote = curve.quoteMint === DEFAULT_PUBLIC_KEY ? { programId: SPL_TOKEN_PROGRAM, decimals: 9, supplyRaw: null } : mintEvidence.get(curve.quoteMint); curve.baseTokenProgram = base.programId; curve.baseDecimals = base.decimals; curve.baseMintSupplyRaw = base.supplyRaw; curve.quoteTokenProgram = quote.programId; curve.quoteDecimals = quote.decimals; curve.executionQuoteMint = curve.quoteMint === DEFAULT_PUBLIC_KEY ? WRAPPED_SOL_MINT : curve.quoteMint; }
  const globalAddress = derivePumpGlobal().address, feeConfigAddress = derivePumpFeeConfig(PUMP_PROGRAM).address, configResponse = await getMultipleAccountsBatched(client, [globalAddress, feeConfigAddress], { commitment: "finalized", encoding: "base64", minContextSlot: mintSlot }, { label: "Pump configuration" }), configSlot = configResponse?.context?.slot;
  if (!Number.isSafeInteger(configSlot) || configSlot < mintSlot || configResponse.value?.length !== 2) throw new Error("invalid Pump configuration response");
  if (automaticMintEvidence) {
    const adapters = curves.map((curve) => ({ address: curve.address, tokenMint0: curve.mint, tokenMint1: curve.executionQuoteMint, tokenProgram0: curve.baseTokenProgram, tokenProgram1: curve.quoteTokenProgram, mintDecimals0: curve.baseDecimals, mintDecimals1: curve.quoteDecimals })), evidence = await acquirePoolMintEvidence(client, adapters, configSlot);
    for (let index = 0; index < curves.length; index++) { const adapter = bindPoolMintEvidence(adapters[index], evidence), curve = curves[index]; curve.mintEvidenceSlot = adapter.mintEvidenceSlot; curve.epoch = adapter.epoch; curve.mint0Evidence = adapter.mint0Evidence; curve.mint1Evidence = adapter.mint1Evidence; }
  }
  return { schemaVersion: 1, type: "pump_bonding_curve_snapshot", chain: "solana", genesisHash, commitment: "finalized", stateSlot, mintSlot, configSlot, observedAt, global: decodePumpGlobalAccount(globalAddress, configResponse.value[0]), feeConfig: decodePumpSwapFeeConfigAccount(feeConfigAddress, configResponse.value[1]), curves };
}
...
async function main() { const config = loadConfig(), store = new IndexStore(config.dataFile, config.maxTransactions, config.retentionSeconds); await store.load(); const artifactOnly = process.argv.includes("--artifact-only"), requested = process.argv.slice(2).filter((value) => value !== "--artifact-only"), mints = requested.length ? requested : [...new Set(Object.values(store.state.pools).filter((row) => row.protocol === "pump-bonding-curve" && row.baseMint).map((row) => row.baseMint))]; if (!mints.length) throw new Error("no Pump bonding-curve mints supplied or discovered"); const client = new LocalValidatorClient(process.env.LOCAL_VALIDATOR_RPC || "http://127.0.0.1:8899"), expected = process.env.INDEXER_EXPECTED_GENESIS_HASH || MAINNET_GENESIS_HASH, genesisHash = await client.assertGenesis(expected), snapshot = await createPumpBondingCurveSnapshot({ client, mints, automaticMintEvidence: true, genesisHash }); if (!artifactOnly) { store.applyPumpBondingCurveSnapshot(snapshot); await store.save(); } await atomicWrite(config.pumpBondingCurveSnapshotFile, snapshot); console.log(JSON.stringify({ stateSlot: snapshot.stateSlot, configSlot: snapshot.configSlot, curves: snapshot.curves.length, artifactOnly })); }
