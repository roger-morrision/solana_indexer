...
export class ExternalRpcPool {
  ...
  async call(method, params = []) { const errors = []; for (const provider of this.providers) { if (provider.openUntil > this.now()) continue; provider.calls++; const requestId = ++this.id; try { const response = await this.fetchImpl(provider.endpoint, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ jsonrpc: "2.0", id: requestId, method, params }), signal: AbortSignal.timeout(this.timeoutMs) }); if (!response.ok) { const wait = response.status === 429 ? retryAfterMs(response.headers?.get?.("retry-after"), this.now()) : null; if (response.status === 429) provider.openUntil = this.now() + (wait ?? this.cooldownMs); throw new Error(`HTTP ${response.status}`); } const payload = await response.json(), hasResult = Object.hasOwn(payload ?? {}, "result"), hasError = Object.hasOwn(payload ?? {}, "error"); if (payload?.jsonrpc !== "2.0" || payload.id !== requestId || hasResult === hasError) throw new Error("invalid JSON-RPC response envelope"); if (hasError) throw new Error(`RPC ${payload.error?.code ?? "unknown"}`); provider.failures = 0; provider.openUntil = 0; this.lastProvider = provider.name; return payload.result; } catch (error) { provider.errors++; provider.failures++; if (provider.openUntil <= this.now() && provider.failures >= this.failureThreshold) provider.openUntil = this.now() + this.cooldownMs; errors.push(`${provider.name}:${error.name ?? "Error"}`); } } throw new Error(`all external RPC providers unavailable (${errors.join(",") || "circuits_open"})`); }
