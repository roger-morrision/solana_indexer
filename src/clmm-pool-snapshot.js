...
export async function createClmmPoolSnapshot({ client, pools, tickArrays = {}, bitmapExtensions = {}, automaticTickCoverage = false, genesisHash, observedAt = new Date().toISOString() }) {
  ...
  const vaults = decoded.flatMap((row) => [row.tokenVault0, row.tokenVault1]), dependencySlot = decoded.reduce((slot, row) => Math.max(slot, row.bitmapExtensionSlot ?? slot, row.tickArraySlot ?? slot), stateSlot);
