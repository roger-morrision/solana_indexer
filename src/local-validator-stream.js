...
export class LocalValidatorStream {
  constructor({ endpoint = "ws://127.0.0.1:8900", endpoints = null, rpcClient, inbox, statusFile, WebSocketClass = globalThis.WebSocket, reconnectMinMs = 500, reconnectMaxMs = 30_000, connectTimeoutMs = 10_000, idleTimeoutMs = 90_000, maxMessageBytes = 67_108_864, expectedGenesisHash = MAINNET_GENESIS_HASH, scheduleReconnect = setTimeout, cancelReconnect = scheduleReconnect === setTimeout ? clearTimeout : () => {}, scheduleConnectTimeout = setTimeout, cancelConnectTimeout = scheduleConnectTimeout === setTimeout ? clearTimeout : () => {}, scheduleIdleTimeout = setTimeout, cancelIdleTimeout = scheduleIdleTimeout === setTimeout ? clearTimeout : () => {} }) {
    ...
    this.endpoints = configured; this.endpointIndex = 0; this.rpcClient = rpcClient; this.inbox = inbox; this.statusFile = statusFile; this.WebSocketClass = WebSocketClass; this.reconnectMinMs = reconnectMinMs; this.reconnectMaxMs = reconnectMaxMs; this.connectTimeoutMs = connectTimeoutMs; this.idleTimeoutMs = idleTimeoutMs; this.maxMessageBytes = maxMessageBytes; this.scheduleReconnect = scheduleReconnect; this.cancelReconnect = cancelReconnect; this.scheduleConnectTimeout = scheduleConnectTimeout; this.cancelConnectTimeout = cancelConnectTimeout; this.scheduleIdleTimeout = scheduleIdleTimeout; this.cancelIdleTimeout = cancelIdleTimeout;
...
async function main() {
  ...
  const stream = new LocalValidatorStream({ endpoints: wsEndpoints, rpcClient, inbox: config.inbox, statusFile: config.exporterStatusFile, reconnectMinMs: config.streamReconnectMinMs, reconnectMaxMs: config.streamReconnectMaxMs, connectTimeoutMs: config.streamConnectTimeoutMs, idleTimeoutMs: config.streamIdleTimeoutMs, maxMessageBytes: config.streamMaxMessageBytes, expectedGenesisHash: process.env.INDEXER_EXPECTED_GENESIS_HASH || MAINNET_GENESIS_HASH });
