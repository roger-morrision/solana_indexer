...
export function evaluateOpenBookOraclePolicy(market, oracleSlot) {
  ...
  if (!evidence.length || evidence.some((row) => !["pyth_legacy", "raydium_clmm", "switchboard_v1", "switchboard_v2"].includes(row.provider)) || evidence.length !== (market.oracleB ? 2 : 1) || !Number.isSafeInteger(oracleSlot) || oracleSlot < 0) return unsupported;
  ...
  const values = evidence.map((row) => row.provider === "pyth_legacy" ? { price: Number(BigInt(row.priceRaw)) * (10 ** row.exponent), deviation: Number(BigInt(row.confidenceRaw)) * (10 ** row.exponent), slot: BigInt(row.lastUpdateSlotRaw) } : row.provider === "switchboard_v2" ? { price: Number(BigInt(row.resultMantissaRaw)) / (10 ** row.resultScale), deviation: Number(BigInt(row.standardDeviationMantissaRaw)) / (10 ** row.standardDeviationScale), slot: BigInt(row.lastUpdateSlotRaw) } : row.provider === "switchboard_v1" ? { price: f64Bits(row.resultBitsRaw), deviation: f64Bits(row.maximumResponseBitsRaw) - f64Bits(row.minimumResponseBitsRaw), slot: BigInt(row.lastUpdateSlotRaw) } : { price: (Number(BigInt(row.squaredPriceX64Raw)) / (2 ** 64)) * (10 ** row.decimalExponent), deviation: 0, slot: (1n << 64n) - 1n });
