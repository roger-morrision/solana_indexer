...
export function quoteOpenBookSnapshotExactInput({ snapshot, poolAddress, inputMint, amountIn, now = Date.now(), staleAfterMs = 60_000, currentSlot = snapshot?.balanceSlot }) {
  ...
  let availableLots = buy ? amount * scale / (scale + takerFee) / quoteLot : amount / baseLot, baseLots = 0n, quoteLots = 0n;
  ...
  if (!baseLots || !quoteLots) throw new Error("OpenBook executable depth is unavailable");
  if (consumed > amount || output <= 0n || consumed > U64_MAX || output > U64_MAX) throw new Error("OpenBook quote arithmetic overflow or zero output");
  return { schemaVersion: 1, protocol: "openbook-v2", status: left === 0n ? "quoted" : "partial", pool: poolAddress, inputMint, outputMint: buy ? market.tokenMint0 : market.tokenMint1, side: buy ? "bid" : "ask", amountInRaw: amount.toString(), consumedInRaw: consumed.toString(), amountLeftRaw: left.toString(), amountOutRaw: output.toString(), takerFeeRaw: fee.toString(), baseLotsFilled: baseLots.toString(), quoteLotsFilled: quoteLots.toString(), baseLotSizeRaw: baseLot.toString(), quoteLotSizeRaw: quoteLot.toString(), takerFeeMillionths: takerFee.toString(), stateSlot: snapshot.stateSlot, bookSlot: snapshot.bookSlot, oracleSlot: snapshot.oracleSlot, balanceSlot: snapshot.balanceSlot, currentSlot, levels, executable: false, safeForAutomation: false, executionBoundary: "analysis_only_quote", missing: ["local_simulation", "external_signer_approval", "landed_transaction_confirmation"] };
}
