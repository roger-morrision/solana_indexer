...
export async function acquirePoolMintEvidence(client, pools, minContextSlot) {
  ...
  for (let index = 0; index < mints.length; index++) {
    ...
    const parsedHook = account.owner === TOKEN_2022_PROGRAM ? transferHookState(info, mint) : null, hook = rawMint ? transferHookState(rawMint.mintInfo, mint) : null;
    if (rawMint && JSON.stringify(parsedHook) !== JSON.stringify(hook)) throw new Error(`pool mint ${mint} parsed and raw transfer hook differ`); if (hook) hooks.push({ mint, ...hook });
