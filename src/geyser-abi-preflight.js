...
export async function preflightGeyser({ manifestFile, agaveBinary, pluginLibrary, versionProbe = runAgaveVersionProbe }) {
  ...
  const [agaveBinarySha256, pluginBinarySha256] = await Promise.all([sha256(agaveBinary), sha256(pluginLibrary)]), agaveVersionOutput = await versionProbe(agaveBinary), [agaveAfterProbe, pluginAfterProbe] = await Promise.all([sha256(agaveBinary), sha256(pluginLibrary)]); if (agaveAfterProbe !== agaveBinarySha256 || pluginAfterProbe !== pluginBinarySha256) throw new Error("activation binaries changed during version verification"); const result = validateGeyserCompatibility(manifest, { agaveVersionOutput, agaveBinarySha256, pluginBinarySha256 }); if (!result.activationAllowed) throw new Error(`Yellowstone activation refused: ${result.reason}`); return result;
}
