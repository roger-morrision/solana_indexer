...
export async function createPumpSwapPoolSnapshot({ client, pools, genesisHash, observedAt = new Date().toISOString() }) {
  ...
  const feeConfigAddress = derivePumpSwapFeeConfig().address, feeResponse = await getMultipleAccountsBatched(client, [feeConfigAddress], { commitment: "finalized", encoding: "base64", minContextSlot: balanceSlot }, { label: "PumpSwap FeeConfig" }), configSlot = feeResponse?.context?.slot; if (!Number.isSafeInteger(configSlot) || configSlot < balanceSlot || feeResponse.value?.length !== 1) throw new Error("invalid PumpSwap FeeConfig response"); const feeConfig = decodePumpSwapFeeConfigAccount(feeConfigAddress, feeResponse.value[0]); decoded.forEach((row) => { row.feeConfig = feeConfig; row.feeConfigSlot = configSlot; });
