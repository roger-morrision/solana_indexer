...
const BACKUP_INVARIANTS = ["completeInventory", "checksums", "manifestEvidence", "writersQuiesced", "inboxIdentity", "rpo", "safeTar", "canonicalStatePresent"];
...
export function compileRecoveryQualification({ backup, indexHealth, warehouse, exporter, eventSequence, startedAt, completedAt, maximumRtoMs = 4 * 60 * 60 * 1_000 }) {
  ...
  if (backup?.schemaVersion !== 2 || backup.kind !== "backup_restore_preflight" || !/^[0-9]{8}T[0-9]{6}Z$/.test(backup.backupId ?? "") || !SHA256.test(backup.backupManifestSha256 ?? "") || backup.restoreAuthorized !== false || !backup.invariants || BACKUP_INVARIANTS.some((key) => backup.invariants[key] !== true)) throw new Error("backup preflight evidence is incomplete");
  ...
  if (!Number.isSafeInteger(eventSequence) || eventSequence < 0 || warehouse?.available !== true || warehouse.healthy !== true || warehouse.sequence !== eventSequence || warehouse.eventSequence !== eventSequence || warehouse.lagEvents !== 0 || warehouse.reconciliation?.schemaVersion !== 2 || warehouse.reconciliation.verified !== true || warehouse.reconciliation.sequence !== eventSequence) throw new Error("restored warehouse is not exactly reconciled");
