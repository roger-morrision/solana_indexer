...
export function assessExporterStatus(status, staleAfterMs, now = Date.now(), maxLagSlots = 512) {
  ...
  const reason = failures > 0 ? "exporter_failure" : status.commitment !== "finalized" ? "not_finalized" : cursor == null ? "invalid_cursor" : lagSlots == null ? "invalid_lag" : tip != null && cursor > tip ? "cursor_ahead_of_tip" : lagSlots > maxLagSlots ? "exporter_lagging" : ageMs == null ? "invalid_observed_at" : ageMs < 0 ? "observed_at_in_future" : ageMs > staleAfterMs ? "exporter_stale" : null;
