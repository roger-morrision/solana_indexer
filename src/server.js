  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (preparePoolSwap) {
        ...
        const meteora = row.programId === METEORA_DLMM_PROGRAM, orca = row.programId === ORCA_WHIRLPOOL_PROGRAM, clmm = row.programId === RAYDIUM_CLMM_PROGRAM, cpmm = row.programId === RAYDIUM_CPMM_PROGRAM, ammV4 = row.programId === RAYDIUM_AMM_V4_PROGRAM, pumpSwap = row.programId === PUMP_SWAP_PROGRAM;
        if (!meteora && !orca && !clmm && !cpmm && !ammV4 && !pumpSwap) return json(response, 503, { schemaVersion: 1, prepared: false, automationSafe: false, reason: "unsupported_construction_protocol" });
        ...
        const snapshot = { schemaVersion: 1, type: meteora ? "meteora_dlmm_pool_snapshot" : orca ? "orca_whirlpool_pool_snapshot" : clmm ? "raydium_clmm_pool_snapshot" : cpmm ? "raydium_cpmm_pool_snapshot" : ammV4 ? "raydium_amm_v4_pool_snapshot" : "pump_swap_pool_snapshot", commitment: row.commitment, stateSlot: row.stateSlot, openOrdersSlot: row.openOrdersSlot, marketSlot: row.marketSlot, configSlot: row.configSlot, balanceSlot: row.balanceSlot, observedAt: row.observedAt, pools: [row] };
        try {
      ...
      const internalPoolQuote = url.pathname.match(/^\/internal\/pools\/([^/]+)\/quote$/); if (internalPoolQuote) {
        ...
        const cpmm = row.programId === RAYDIUM_CPMM_PROGRAM, ammV4 = row.programId === RAYDIUM_AMM_V4_PROGRAM, clmm = row.programId === RAYDIUM_CLMM_PROGRAM, pumpSwap = row.programId === PUMP_SWAP_PROGRAM, orca = row.programId === ORCA_WHIRLPOOL_PROGRAM, meteora = row.programId === METEORA_DLMM_PROGRAM, phoenix = row.programId === PHOENIX_PROGRAM; if (!cpmm && !ammV4 && !clmm && !pumpSwap && !orca && !meteora && !phoenix) return json(response, 503, { schemaVersion: 1, available: false, automationSafe: false, reason: "unsupported_quote_protocol" }); const concentrated = clmm || orca; if ((concentrated && (!/^-?\d+$/.test(limitTickText ?? "") || !Number.isSafeInteger(Number(limitTickText)))) || (!concentrated && limitTickText != null)) return json(response, 400, { error: "invalid_quote_parameters" });
        const type = cpmm ? "raydium_cpmm_pool_snapshot" : ammV4 ? "raydium_amm_v4_pool_snapshot" : clmm ? "raydium_clmm_pool_snapshot" : pumpSwap ? "pump_swap_pool_snapshot" : orca ? "orca_whirlpool_pool_snapshot" : meteora ? "meteora_dlmm_pool_snapshot" : "phoenix_market_snapshot", snapshot = { schemaVersion: 1, type, commitment: row.commitment, stateSlot: row.stateSlot, openOrdersSlot: row.openOrdersSlot, marketSlot: row.marketSlot, configSlot: row.configSlot, balanceSlot: row.balanceSlot, observedAt: row.observedAt, pools: [row], markets: [row] };
        try { const quote = cpmm ? quoteCpmmSnapshotExactInput({ snapshot, poolAddress, inputMint, amountIn: amountRaw, staleAfterMs: config.staleAfterMs }) : ammV4 ? quoteAmmV4SnapshotExactInput({ snapshot, poolAddress, inputMint, amountIn: amountRaw, staleAfterMs: config.staleAfterMs }) : clmm ? quoteRaydiumSnapshotExactInput({ snapshot, poolAddress, inputMint, amountIn: amountRaw, limitTick: Number(limitTickText), maxAgeMs: config.staleAfterMs }) : pumpSwap ? quotePumpSwapSnapshotExactInput({ snapshot, poolAddress, inputMint, amountIn: amountRaw, staleAfterMs: config.staleAfterMs }) : orca ? quoteOrcaSnapshotExactInput({ snapshot, poolAddress, inputMint, amountIn: amountRaw, limitTick: Number(limitTickText), maxAgeMs: config.staleAfterMs }) : meteora ? quoteMeteoraDlmmSnapshotExactInput({ snapshot, poolAddress, inputMint, amountIn: amountRaw, staleAfterMs: config.staleAfterMs }) : quotePhoenixSnapshotExactInput({ snapshot, poolAddress, inputMint, amountIn: amountRaw, staleAfterMs: config.staleAfterMs }); const blockers = phoenix ? ["unsigned_instruction_construction_unavailable", "local_simulation_required", "external_signer_approval_required", "transaction_submission_out_of_scope"] : ["local_simulation_required", "external_signer_approval_required", "transaction_submission_out_of_scope"]; return json(response, 200, { schemaVersion: 1, available: true, automationSafe: false, executionBoundary: phoenix ? "analysis_only_quote" : "offline_unsigned_construction_and_read_only_confirmation", constructionAvailable: !phoenix, simulationRequired: true, submissionPerformed: false, blockers, quote }); } catch (error) { reportDiagnostic(config, metrics, "pool_quote_failed", error); return json(response, 503, { schemaVersion: 1, available: false, automationSafe: false, reason: "quote_unavailable" }); }
      }
