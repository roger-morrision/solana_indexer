...
async function readJsonFile(filename, maximumBytes = 1_048_576) {
  if (!filename) return null;
  try {
    const before = await fs.lstat(filename); if (!before.isFile() || before.isSymbolicLink() || !Number.isSafeInteger(before.size) || before.size < 1 || before.size > maximumBytes) return { evidenceReadError: "invalid_file" };
    const content = await fs.readFile(filename); if (content.length !== before.size) return { evidenceReadError: "changed_during_read" }; const after = await fs.lstat(filename); if (!after.isFile() || after.isSymbolicLink() || after.size !== before.size || after.dev !== before.dev || after.ino !== before.ino || after.mtimeMs !== before.mtimeMs) return { evidenceReadError: "changed_during_read" };
    return JSON.parse(content.toString("utf8"));
  } catch (error) { if (error.code === "ENOENT") return null; return { evidenceReadError: error instanceof SyntaxError ? "invalid_json" : "read_failed" }; }
}
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (protectedRoute && requestLimit) {
        ...
        let quota; try { quota = await admitQuota(quotaIdentity, requestLimit, requestWeight); } catch { return json(response, 503, { error: "distributed_quota_unavailable" }); }
