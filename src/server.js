...
export function createServer(config, store) {
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (decisionConsumer) { const quality = store.derivedLedgerQuality(); if (!quality.canonical) return json(response, 503, { schemaVersion: 1, available: false, reason: quality.reason }); }
      if (decisionConsumer) { const quality = store.recoveryQuality(); if (!quality.canonical || quality.capacityExceeded) return json(response, 503, { schemaVersion: 1, available: false, reason: quality.reason }); }
      if (decisionConsumer) { const quality = store.aggregateQuality(); if (!quality.canonical) return json(response, 503, { schemaVersion: 1, available: false, reason: quality.reason }); }
      if (decisionConsumer) { const quality = store.programEventQuality(); if (!quality.canonical) return json(response, 503, { schemaVersion: 1, available: false, reason: quality.reason }); }
      if (decisionConsumer) { const quality = store.snapshotQuality(); if (!quality.canonical) return json(response, 503, { schemaVersion: 1, available: false, reason: quality.reason }); }
      if (decisionConsumer) { const quality = store.metadataQuality(); if (!quality.canonical) return json(response, 503, { schemaVersion: 1, available: false, reason: quality.reason }); }
