  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      const internalPoolQuote = url.pathname.match(/^\/internal\/pools\/([^/]+)\/quote$/); if (internalPoolQuote) {
        ...
        const cpmm = row.programId === RAYDIUM_CPMM_PROGRAM, clmm = row.programId === RAYDIUM_CLMM_PROGRAM, pumpSwap = row.programId === PUMP_SWAP_PROGRAM, orca = row.programId === ORCA_WHIRLPOOL_PROGRAM; if (!cpmm && !clmm && !pumpSwap && !orca) return json(response, 503, { schemaVersion: 1, available: false, automationSafe: false, reason: "unsupported_quote_protocol" }); const concentrated = clmm || orca; if ((concentrated && (!/^-?\d+$/.test(limitTickText ?? "") || !Number.isSafeInteger(Number(limitTickText)))) || (!concentrated && limitTickText != null)) return json(response, 400, { error: "invalid_quote_parameters" });
        const type = cpmm ? "raydium_cpmm_pool_snapshot" : clmm ? "raydium_clmm_pool_snapshot" : pumpSwap ? "pump_swap_pool_snapshot" : "orca_whirlpool_pool_snapshot", snapshot = { schemaVersion: 1, type, commitment: row.commitment, stateSlot: row.stateSlot, configSlot: row.configSlot, balanceSlot: row.balanceSlot, observedAt: row.observedAt, pools: [row] };
        try { const quote = cpmm ? quoteCpmmSnapshotExactInput({ snapshot, poolAddress, inputMint, amountIn: amountRaw, staleAfterMs: config.staleAfterMs }) : clmm ? quoteRaydiumSnapshotExactInput({ snapshot, poolAddress, inputMint, amountIn: amountRaw, limitTick: Number(limitTickText), maxAgeMs: config.staleAfterMs }) : pumpSwap ? quotePumpSwapSnapshotExactInput({ snapshot, poolAddress, inputMint, amountIn: amountRaw, staleAfterMs: config.staleAfterMs }) : quoteOrcaSnapshotExactInput({ snapshot, poolAddress, inputMint, amountIn: amountRaw, limitTick: Number(limitTickText), maxAgeMs: config.staleAfterMs }); const orcaBlockers = orca ? ["orca_transaction_construction_unavailable", "local_simulation_required", "external_signer_approval_required", "transaction_submission_out_of_scope"] : ["local_simulation_required", "external_signer_approval_required", "transaction_submission_out_of_scope"]; return json(response, 200, { schemaVersion: 1, available: true, automationSafe: false, executionBoundary: orca ? "analysis_only_quote" : "offline_unsigned_construction_and_read_only_confirmation", blockers: orcaBlockers, quote }); } catch (error) { return json(response, 503, { schemaVersion: 1, available: false, automationSafe: false, reason: error.message }); }
      }
