...
function reportDiagnostic(config, event, error) { if (typeof config.onDiagnostic !== "function") return; try { config.onDiagnostic({ event, error: redactDiagnostic(error, "internal server failure") }); } catch {} }
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (preparePoolSwap) {
        ...
        } catch (error) { reportDiagnostic(config, "pool_swap_preparation_failed", error); return json(response, 503, { schemaVersion: 1, prepared: false, automationSafe: false, reason: "execution_preparation_unavailable" }); }
      }
      if (prepareCurveSwap) {
        ...
        try {
          ...
          return json(response, 200, { schemaVersion: 1, prepared: true, automationSafe: false, signed: false, submitted: false, requiredNextSteps: EXECUTION_HANDOFF_POLICY.requiredSteps, executionHandoff: bindExecutionHandoff(preparation), quote, preparation });
        } catch (error) { reportDiagnostic(config, "curve_swap_preparation_failed", error); return json(response, 503, { schemaVersion: 1, prepared: false, automationSafe: false, reason: "execution_preparation_unavailable" }); }
      }
