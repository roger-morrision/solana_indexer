...
function dispatchRpc(payload, config, store) {
  ...
  if (payload.method === "getIndexedTokenMetadata") {
    ...
    return rpcResult(payload.id, { mint, metadata, metadataPresent: evidence.metadataPresent, authoritativeAbsence: evidence.authoritativeAbsence, offchainMetadata: publicOffchainMetadata(offchainMetadata), provenance: { ...evidence.provenance, searchComplete: evidence.searchComplete }, coverage: evidence.coverage, complete: evidence.complete, safeForAutomation: evidence.safeForAutomation });
  }
...
export function createServer(config, store) {
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      const evidence = url.pathname.match(/^\/internal\/evidence\/([^/]+)$/); if (evidence) return json(response, 200, publicEvidence(store.evidence(decodeURIComponent(evidence[1]), config.staleAfterMs)));
      const internalToken = url.pathname.match(/^\/internal\/tokens\/([^/]+)(?:\/(market|security|holders|trades|ohlcv|liquidity|executable-depth))?$/); if (internalToken) {
        ...
        if (view === "holders") return json(response, 200, publicHolders(token.observedHolders)); if (view === "trades") return json(response, 200, { schemaVersion: 1, data: swaps.slice(0, limit(url)) }); if (view === "liquidity") return json(response, 200, { schemaVersion: 1, pools: poolAddresses.map((address) => poolCatalogRow(address, store.pool(address).summary)) }); if (view === "ohlcv") return json(response, 200, { schemaVersion: 1, pools: poolAddresses.map((address) => store.candles(address, candleInterval(url), limit(url))) }); if (view === "market") return json(response, 200, store.evidence(mintAddress, config.staleAfterMs).market); if (view === "security") return json(response, 200, store.tokenSecurity(mintAddress, config.staleAfterMs)); if (view === "executable-depth") { const side = url.searchParams.get("side") ?? "sell"; if (!new Set(["buy", "sell"]).has(side)) return json(response, 400, { error: "invalid_quote_side" }); const quote = side === "buy" ? store.buyRouteQuote(mintAddress, url.searchParams.get("amountRaw"), config.staleAfterMs) : store.sellRouteQuote(mintAddress, url.searchParams.get("amountRaw"), config.staleAfterMs); return json(response, quote.available ? 200 : quote.reason === "invalid_amount" ? 400 : 503, quote); } return json(response, 200, publicMintDetail(token));
      }
      const internalWallet = url.pathname.match(/^\/internal\/wallets\/([^/]+)(?:\/(performance|profile|funding|funding-cluster))?$/); if (internalWallet) { const address = decodeURIComponent(internalWallet[1]); return json(response, 200, internalWallet[2] === "performance" ? store.walletPerformance(address) : internalWallet[2] === "profile" ? store.walletProfile(address) : internalWallet[2] === "funding" ? store.walletFunding(address, limit(url)) : internalWallet[2] === "funding-cluster" ? store.walletFundingCluster(address, limit(url)) : publicAccountDetail(store.account(address, limit(url)))); }
      ...
      if (url.pathname === "/api/v1/bot/readiness") { const now = Date.now(), readiness = store.botReadiness(config.staleAfterMs, now, url.searchParams.get("pool")), [exporter, checkpoint, failure] = await Promise.all([readJsonFile(config.exporterStatusFile), readJsonFile(config.warehouseCheckpointFile), readJsonFile(config.warehouseStatusFile)]), ingestion = assessExporterStatus(exporter, config.staleAfterMs, now, config.maxExporterLagSlots), warehouse = applyWarehouseFailureStatus(assessWarehouseCheckpoint(checkpoint, store.state.eventSequence, store.state.events[0]?.sequence ?? store.state.eventSequence + 1, config.warehouseStaleAfterMs, config.maxWarehouseLagEvents, now), failure), gated = gateBotReadiness(readiness, ingestion, warehouse); return json(response, gated.ready ? 200 : 503, gated); }
      ...
      const account = url.pathname.match(/^\/api\/account\/([^/]+)$/); if (account) return json(response, 200, publicAccountDetail(store.account(decodeURIComponent(account[1]), limit(url))));
      const mint = url.pathname.match(/^\/api\/mint\/([^/]+)$/); if (mint) return json(response, 200, publicMintDetail(store.mint(decodeURIComponent(mint[1]), limit(url), config.staleAfterMs)));
      const holders = url.pathname.match(/^\/api\/v1\/holders\/([^/]+)$/); if (holders) return json(response, 200, publicHolders(store.holders(decodeURIComponent(holders[1]), limit(url), config.staleAfterMs)));
      const tokenAccount = url.pathname.match(/^\/api\/v1\/token-account\/([^/]+)$/); if (tokenAccount) { const address = decodeURIComponent(tokenAccount[1]), row = store.tokenAccount(address); return json(response, row ? 200 : 404, row ? indexedTokenAccountRow(store, address, row) : { error: "not_found" }); }
      const pool = url.pathname.match(/^\/api\/v1\/pool\/([^/]+)$/); if (pool) { const row = store.pool(decodeURIComponent(pool[1])); return json(response, row.summary ? 200 : 404, row.summary ? publicPoolDetail(row) : { error: "not_found" }); }
