...
function publicMintDetail(token) { const row = token.summary; return { ...token, summary: row == null ? null : { transferCount: row.transferCount ?? 0, swapCount: row.swapCount ?? 0, lastSlot: row.lastSlot ?? null, lastBlockTime: row.lastBlockTime ?? null, mintInfo: row.mintInfo ?? null, metadata: publicTokenMetadata(row.metadata), offchainMetadata: publicOffchainMetadata(row.offchainMetadata), authoritySourceSlot: row.authoritySourceSlot ?? null } }; }
