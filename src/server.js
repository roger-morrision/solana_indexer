  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (preparePoolSwap) {
        ...
        const meteora = row.programId === METEORA_DLMM_PROGRAM, orca = row.programId === ORCA_WHIRLPOOL_PROGRAM, clmm = row.programId === RAYDIUM_CLMM_PROGRAM, cpmm = row.programId === RAYDIUM_CPMM_PROGRAM, ammV4 = row.programId === RAYDIUM_AMM_V4_PROGRAM, pumpSwap = row.programId === PUMP_SWAP_PROGRAM, phoenix = row.programId === PHOENIX_PROGRAM;
        if (!meteora && !orca && !clmm && !cpmm && !ammV4 && !pumpSwap && !phoenix) return json(response, 503, { schemaVersion: 1, prepared: false, automationSafe: false, reason: "unsupported_construction_protocol" });
        ...
        const snapshot = { schemaVersion: 1, type: meteora ? "meteora_dlmm_pool_snapshot" : orca ? "orca_whirlpool_pool_snapshot" : clmm ? "raydium_clmm_pool_snapshot" : cpmm ? "raydium_cpmm_pool_snapshot" : ammV4 ? "raydium_amm_v4_pool_snapshot" : pumpSwap ? "pump_swap_pool_snapshot" : "phoenix_market_snapshot", commitment: row.commitment, stateSlot: row.stateSlot, openOrdersSlot: row.openOrdersSlot, marketSlot: row.marketSlot, configSlot: row.configSlot, balanceSlot: row.balanceSlot, observedAt: row.observedAt, pools: [row], markets: [row] };
        try {
