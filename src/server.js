  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      const internalToken = url.pathname.match(/^\/internal\/tokens\/([^/]+)(?:\/(market|security|holders|trades|ohlcv|liquidity|executable-depth))?$/); if (internalToken) {
        ...
        if (view === "holders") return json(response, 200, publicHolders(token.observedHolders)); if (view === "trades") return json(response, 200, { schemaVersion: 1, data: swaps.slice(0, limit(url)) }); if (view === "liquidity") return json(response, 200, { schemaVersion: 1, pools: poolAddresses.map((address) => ({ address, ...store.pool(address).summary })) }); if (view === "ohlcv") return json(response, 200, { schemaVersion: 1, pools: poolAddresses.map((address) => store.candles(address, candleInterval(url), limit(url))) }); if (view === "market") return json(response, 200, store.evidence(mintAddress, config.staleAfterMs).market); if (view === "security") return json(response, 200, store.tokenSecurity(mintAddress, config.staleAfterMs)); if (view === "executable-depth") { const side = url.searchParams.get("side") ?? "sell"; if (!new Set(["buy", "sell"]).has(side)) return json(response, 400, { error: "invalid_quote_side" }); const quote = side === "buy" ? store.buyRouteQuote(mintAddress, url.searchParams.get("amountRaw"), config.staleAfterMs) : store.sellRouteQuote(mintAddress, url.searchParams.get("amountRaw"), config.staleAfterMs); return json(response, quote.available ? 200 : quote.reason === "invalid_amount" ? 400 : 503, quote); } return json(response, 200, publicMintDetail(token));
      }
