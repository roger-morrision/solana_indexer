  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (preparePoolSwap) {
        ...
        const meteora = row.programId === METEORA_DLMM_PROGRAM, orca = row.programId === ORCA_WHIRLPOOL_PROGRAM;
        if (!meteora && !orca) return json(response, 503, { schemaVersion: 1, prepared: false, automationSafe: false, reason: "unsupported_construction_protocol" });
        ...
        if (orca && !Number.isSafeInteger(preparePayload.limitTick)) return json(response, 400, { error: "invalid_prepare_parameters" });
        const snapshot = { schemaVersion: 1, type: meteora ? "meteora_dlmm_pool_snapshot" : "orca_whirlpool_pool_snapshot", commitment: row.commitment, stateSlot: row.stateSlot, balanceSlot: row.balanceSlot, observedAt: row.observedAt, pools: [row] };
        try {
          const quote = meteora ? quoteMeteoraDlmmSnapshotExactInput({ snapshot, poolAddress, inputMint: preparePayload.inputMint, amountIn: preparePayload.amountRaw, staleAfterMs: config.staleAfterMs }) : quoteOrcaSnapshotExactInput({ snapshot, poolAddress, inputMint: preparePayload.inputMint, amountIn: preparePayload.amountRaw, limitTick: preparePayload.limitTick, maxAgeMs: config.staleAfterMs }), preparation = meteora ? prepareMeteoraDlmmSwapSimulation({ quote, pool: row, user: preparePayload.user, inputTokenAccount: preparePayload.inputTokenAccount, outputTokenAccount: preparePayload.outputTokenAccount, minimumOutputRaw: preparePayload.minimumOutputRaw, recentBlockhash: preparePayload.recentBlockhash, inputPreAmountRaw: preparePayload.inputPreAmountRaw, outputPreAmountRaw: preparePayload.outputPreAmountRaw, bitmapExtension: preparePayload.bitmapExtension ?? null, hostFeeAccount: preparePayload.hostFeeAccount ?? null, transferHookAccountData: preparePayload.transferHookAccountData ?? null }) : prepareOrcaWhirlpoolSwapSimulation({ quote, pool: row, tokenAuthority: preparePayload.user, tokenOwnerAccountA: preparePayload.tokenOwnerAccountA, tokenOwnerAccountB: preparePayload.tokenOwnerAccountB, minimumOutputRaw: preparePayload.minimumOutputRaw, recentBlockhash: preparePayload.recentBlockhash, inputPreAmountRaw: preparePayload.inputPreAmountRaw, outputPreAmountRaw: preparePayload.outputPreAmountRaw });
