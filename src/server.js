  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      const internalPoolQuote = url.pathname.match(/^\/internal\/pools\/([^/]+)\/quote$/); if (internalPoolQuote) {
        ...
        try { const common = { snapshot, poolAddress, inputMint, amountIn: amountRaw, staleAfterMs: config.staleAfterMs }, quote = cpmm ? quoteCpmmSnapshotExactInput(common) : ammV4 ? quoteAmmV4SnapshotExactInput(common) : clmm ? quoteRaydiumSnapshotExactInput({ ...common, limitTick: Number(limitTickText), maxAgeMs: config.staleAfterMs }) : pumpSwap ? quotePumpSwapSnapshotExactInput(common) : orca ? quoteOrcaSnapshotExactInput({ ...common, limitTick: Number(limitTickText), maxAgeMs: config.staleAfterMs }) : meteora ? quoteMeteoraDlmmSnapshotExactInput(common) : phoenix ? quotePhoenixSnapshotExactInput(common) : quoteOpenBookSnapshotExactInput(common); const constructionAvailable = !openBook, blockers = constructionAvailable ? ["local_simulation_required", "external_signer_approval_required", "transaction_submission_out_of_scope"] : ["unsigned_instruction_construction_unavailable", "local_simulation_required", "external_signer_approval_required", "landed_transaction_confirmation_required"]; return json(response, 200, { schemaVersion: 1, available: true, automationSafe: false, executionBoundary: constructionAvailable ? "offline_unsigned_construction_and_read_only_confirmation" : "analysis_only_quote", constructionAvailable, simulationRequired: true, submissionPerformed: false, blockers, quote }); } catch (error) { reportDiagnostic(config, metrics, "pool_quote_failed", error); return json(response, 503, { schemaVersion: 1, available: false, automationSafe: false, reason: "quote_unavailable" }); }
      }
