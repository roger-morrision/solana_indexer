...
const SCRIPT = "local count=redis.call('INCRBY',KEYS[1],ARGV[1]); if count==tonumber(ARGV[1]) then redis.call('PEXPIRE',KEYS[1],ARGV[2]) end; local ttl=redis.call('PTTL',KEYS[1]); return {count,ttl}";
...
export function compileRedisQuotaRequest(identity, window, windowMs, password = null, weight = 1) {
  if (typeof identity !== "string" || !identity || !Number.isSafeInteger(window) || window < 0 || !Number.isInteger(windowMs) || windowMs < 1_000 || !Number.isInteger(weight) || weight < 1 || weight > 100 || (password != null && (typeof password !== "string" || !password))) throw new Error("invalid Redis quota request");
  const identityHash = crypto.createHash("sha256").update(identity).digest("hex"), key = `terminal_dex:quota:${window}:${identityHash}`, commands = []; if (password) commands.push(resp(["AUTH", password])); commands.push(resp(["EVAL", SCRIPT, 1, key, weight, windowMs])); return { payload: Buffer.concat(commands), expectedReplies: commands.length, identityHash };
}
...
export function createRedisQuotaAdmitter({ host = "127.0.0.1", port = 6379, password = null, timeoutMs = 250 } = {}, connect = net.createConnection) {
  ...
  return (identity, limit, now = Date.now(), weight = 1) => new Promise((resolve, reject) => {
    if (!Number.isInteger(limit) || limit < 1 || limit > 100_000 || !Number.isFinite(now) || now < 0 || !Number.isInteger(weight) || weight < 1 || weight > 100) return reject(new Error("invalid Redis quota admission"));
    const windowMs = 60_000, window = Math.floor(now / windowMs), request = compileRedisQuotaRequest(identity, window, windowMs, password, weight); let buffer = Buffer.alloc(0), replies = 0, settled = false;
