...
import { decodeBase58Address } from "./solana-pda.js";

const MAINNET_GENESIS_HASH = "5eykt4UsFv8P8NJdTREpY1vzqKqZKvdpKuc147dw2N9d";
const CATEGORIES = new Set(["burn", "exchange", "locker", "pool", "protocol", "vault"]);

function canonicalAddress(value) { try { decodeBase58Address(value); return true; } catch { return false; } }
...
export function compileHolderExclusions(value) {
  ...
  const completeMints = new Set(); for (const mint of value.completeMints) { if (!canonicalAddress(mint) || completeMints.has(mint)) throw new Error("invalid complete holder exclusion mint"); completeMints.add(mint); }
  ...
  for (const entry of value.entries) {
    ...
    if (!canonicalAddress(entry?.mint) || hasOwner === hasTokenAccount || hasOwner && !canonicalAddress(entry.owner) || hasTokenAccount && !canonicalAddress(entry.tokenAccount) || !CATEGORIES.has(entry.category) || typeof entry.evidenceSource !== "string" || !entry.evidenceSource) throw new Error("invalid holder exclusion entry");
