...
- Run: `2026-08-29T09:36:00+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `45a5412b2de950bfa0a8c25bafa7720e50ed6e01`
- Compared with QA baseline: `49ae3018fb7459688a9902ed4969cb479a1abc34` (2 DEV commits, 4 changed files)
- Compared with `origin/main`: 50 ahead, 0 behind before this evidence report
- Latest DEV commits: `6ca0f92` (per-array output capacity) and `45a5412` (per-array input and fee-adjusted output capacity)
- Overall result: 2 PASS, 0 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. Both advertised capacity outcomes close their exact boundaries: output redistribution and consumed-input redistribution now reject for a real two-array quote, including collect-fee-on-output capacity. The parent HIGH finding remains 21/22 strict outcomes because redistributing row trading fees from `1/1` to `2/0` preserves every published aggregate and capacity rule and is still admitted although the deterministic producer cannot emit it from the same finalized bins. Independent reconciliation remains 68 PASS, 1 FAIL, and 1 BLOCKED across 70 domains; live qualification is blocked by absent fresh canonical evidence.

## Reviewed DEV delta (2/20)

### Meteora per-array capacity closure (2 PASS)

| Item | Route | Status | Independent evidence |
|---|---|---|---|
| `UPSTREAM-METEORA-DLMM-BIN-ARRAY-CAPACITY-PARITY-006` | pool `prepare-swap` / output capacity | `PASS` | The real two-array quote emits output/capacity rows `100/100` and `98/100`. Both `101/97` and `99/99` redistributions reject; a terminal `101/100` overflow rejects. |
| `UPSTREAM-METEORA-DLMM-BIN-ARRAY-INPUT-CAPACITY-PARITY-007` | pool `prepare-swap` / input and fee-adjusted output capacity | `PASS` | The same quote emits input/capacity rows `101/101` and `99/101`; changing them to `102/98` rejects. A collect-fee-on-output control emits exact `100/100` input and `99/99` output rows for both arrays. |

- Available DEV delta: exactly two distinct committed outcomes exist after `49ae301`, one in `6ca0f92` and one in `45a5412`. QC detected the active DEV writer lock and source movement after starting against `6ca0f92`, discarded that mixed-state probe, waited for lock release, refreshed the complete delta to clean stable `45a5412`, and reran every tier. No additional distinct DEV outcome exists.
- Verification result: 2 PASS, 0 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct outcomes, and splitting fields, capacity dimensions, fee modes, mutation probes, or assertions would be padding.
- Validation: real two-array and collect-fee-on-output controls PASS; output and input redistribution negatives reject; a fresh fee-only redistribution is admitted and retains the deduplicated parent HIGH finding. Strict protocol-specific closure remains 21/22 PASS and 1/22 FAIL. Focused Meteora suite 11/11 PASS; 78 response schemas, eleven quote references, eleven instruction references, 28 dialect keywords, 27 relationship kinds, and digest `7e56bb0f3b949cd0f75a7ca38d59d93828b0309fa30e1f12c1079f07cc77cbb2` are structurally coherent; full suite 439/439 PASS; syntax 86/86 PASS; replay invariants PASS at 8,738.15 blocks/s with 9,396,504-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
- Severity: `FAIL` / `HIGH`
- Owner: `DEV`
- Reproduction: generate a real X-to-Y quote crossing bins 70 and 69 in arrays 1 and 0. The producer emits per-array trading fees `1/1`. Change only those rows to `2/0`, preserving the global fee total, consumed input, gross output, input/output capacity, path endpoints, array indexes, protocol fees, maximum rate, direction, and transfer-fee evidence, then apply every published `meteora_dlmm_quote_v1` relationship.
- Evidence: `6ca0f92` closes the prior output redistribution with finalized per-array output capacity; `45a5412` closes consumed-input redistribution and uses fee-adjusted output capacity for collect-fee-on-output quotes. Both advertised outcomes pass. The real two-array body validates, the prior output and input mutations reject, but fee-only `1/1 -> 2/0` still satisfies traversal aggregates and both capacity dimensions although the deterministic producer returns `1/1` from the same finalized bins.
...
- Expected versus actual behavior: the advertised schema must admit every real producer body and reject per-array economics the finalized producer cannot emit. Direction, transfer-fee, range, global aggregate, path, and input/output capacity contracts now match; row-local trading/protocol fee parity does not.
- Acceptance criteria: retain the four real directional controls, exact transfer-fee boundaries, decoder caps, global sums, endpoint/order/array-set constraints, both capacity dimensions, collect-fee-on-output behavior, and immutable execution gates; bind every traversal row's trading and protocol fees to sufficient finalized economics or a producer-derived content commitment; retain the real two-array positive and isolated fee-redistribution negatives.
- Validation results: real capacity controls PASS; prior output/input redistribution negatives reject; fee-only redistribution FAIL because it is admitted. Focused Meteora suite 11/11 PASS; full 439/439 PASS; syntax 86/86 PASS; replay PASS at 8,738.15 blocks/s with 9,396,504-byte heap growth.
- Compatibility/performance impact: the additive quote fields and expanded relationship change digest/ETag and generated validators. A bounded fee commitment or finalized row-local evidence must remain within the existing 13,312-row cap; no replay or full-suite regression is observed.
...
- Validation results: prior closed boundaries remain PASS; strict protocol-specific closure remains 21/22 PASS and 1/22 FAIL (ten quote plus all eleven instruction variants PASS; Meteora row-local quote parity fails). Focused Meteora suite 11/11 PASS; full suite 439/439 PASS; syntax 86/86 PASS; replay PASS at 8,738.15 blocks/s with 9,396,504-byte heap growth.
...
- NEXT_DEV_ACTION: bind each Meteora `binTraversal` row's `tradingFeeRaw` and `protocolFeeRaw` to sufficient finalized fee economics or a producer-derived content commitment so fee-only cross-array redistribution rejects, retaining the real two-array and collect-fee-on-output positives plus every completed direction, transfer-fee, aggregate, range, capacity, and execution-safety control.
