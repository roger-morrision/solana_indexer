...
- Run: `2026-08-28T23:36:50+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `dc9cbb184dbc0b6917fd2c5775ba565f21e8c662`
- Compared with QA baseline: `3c5680899b4d9bc33ff0e4d39e17450d3d3f00d2` (2 DEV commits, 3 changed files)
- Compared with `origin/main`: 22 ahead, 0 behind before this evidence report
- Latest DEV commits: `5d92788` (Meteora/Phoenix parity repair) and `dc9cbb1` (OpenBook V2 instruction-evidence schema)
- Overall result: 1 PASS, 2 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. Meteora now matches every reported constructor relationship. Phoenix rejects all previously reported side/lot/output contradictions but still admits expiry beyond the 150-slot ceiling and u64-overflow evidence. The new OpenBook schema rejects actual preparation evidence because it omits `minimumOutputRaw`, while admitting constructor-impossible slot ordering and i64 overflow. The parent HIGH finding remains open. Live qualification is independently blocked by absent fresh canonical evidence.

## Reviewed DEV delta (3/20)

### Instruction-evidence schema constructor parity (1 PASS, 2 FAIL)

| Item | Route | Status | Independent evidence |
|---|---|---|---|
| `UPSTREAM-METEORA-DLMM-INSTRUCTION-EVIDENCE-SCHEMA-001` | pool `prepare-swap` / Meteora catalog row | `PASS` | The real legacy SPL constructor output with optional finalized route data now validates; the same control with either nonzero legacy transfer fee rejects. The four prior bitmap/hook/version/uniqueness contradictions remain rejected, and real Token-2022 construction coverage stays green. |
| `UPSTREAM-PHOENIX-IOC-INSTRUCTION-EVIDENCE-SCHEMA-001` | pool `prepare-swap` / Phoenix catalog row | `FAIL` | Real bid/ask controls validate and all four prior side-positive/output-bound mutations reject. Fresh independent probes show the schema still accepts `lastValidSlot = quoteCurrentSlot + 151` and `priceInTicks = 2^64`; the real constructor rejects both. |
| `UPSTREAM-OPENBOOK-V2-INSTRUCTION-EVIDENCE-SCHEMA-001` | pool `prepare-swap` / OpenBook V2 catalog row | `FAIL` | The closed schema accepts instruction-only evidence but rejects the actual preparation evidence because `minimumOutputRaw` is an unknown omitted schema field. It also admits `quoteCurrentSlot < balanceSlot` and `priceLots = 2^63`, both rejected by the real constructor. |

- Available DEV delta: `5d92788` delivers two distinct protocol-schema repair outcomes and `dc9cbb1` adds one distinct BA/PO-selected OpenBook enhancement after `3c56808`. The complete delta was exhausted. Source advanced during report preparation; QC withheld the stale `5d92788` report, refreshed to clean stable `dc9cbb1`, and reran every tier. No additional DEV commit or enhancement exists.
- Verification result: 1 PASS, 2 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 17; the stable delta contains exactly three distinct protocol-schema outcomes, and splitting relationship descriptors, contradiction probes, schema properties, or assertions would be padding.
- Validation: independent real-constructor harnesses accept Meteora legacy route, Phoenix bid/ask, and OpenBook instruction-only controls; reject all five prior Meteora/Phoenix contradictions; reproduce two Phoenix constructor-bound failures; reject the actual OpenBook preparation body; and reproduce two OpenBook constructor contradictions. Strict protocol-specific closure remains 9/22 PASS and 13/22 FAIL (four quote and five instruction-evidence variants PASS; Phoenix and OpenBook remain incomplete; seven quote and four other instruction variants remain null). Focused committed protocol suite 17/17 PASS; 68 response schemas, 18 declared relationship kinds, and digest `956ece55da6eb8e0f05c9353df8a15b82b3a287943e8a0e192cfcbf97d42469e` are structurally coherent; full suite 428/428 PASS; syntax 86/86 PASS; replay invariants PASS at 5,132.73 blocks/s with 11,646,232-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| HTTP response schema registry structure | `PASS` | The registry publishes 68 schemas with stable identities and digest `956ece55da6eb8e0f05c9353df8a15b82b3a287943e8a0e192cfcbf97d42469e`. Independent recomputation matches. Recursive enumeration confirms every live schema-node keyword and all eighteen relationship kinds are declared, including conditional decimal patterns and decimal less-than-or-equal, while unknown future keywords remain fail closed. |
...
| Swap-preparation success schema | `FAIL` | Top-level, handoff/binding, preparation/transaction, simulation-policy, universal amount-effect, and catalog identity boundaries are closed. Five instruction-evidence variants plus CPMM, CLMM, Orca, and AMM v4 quotes have constructor parity. Phoenix and OpenBook are referenced but remain constructor-incomplete; seven quote variants and four other instruction-evidence variants remain schema-null. |
...
- Severity: `FAIL` / `HIGH`
- Owner: `DEV`
- Reproduction: resolve the Phoenix catalog schema and validate real bid/ask constructor controls. Confirm the repaired zero/positive/output contradictions reject, then independently set only (1) `lastValidSlot` to `quoteCurrentSlot + 151`, or (2) `priceInTicks` to `18446744073709551616` (`2^64`). Compare generated validation with direct builder calls using the same expiry or price.
- Evidence: `5d92788` adds conditional positive bid/ask lot patterns and `minimumOutputRaw <= quotedOutputRaw`; real controls pass and all four prior side/lot/output mutations reject. Yet generated validation admits both fresh mutations. `buildPhoenixImmediateOrCancelInstruction` rejects expiry above `quote.currentSlot + 150`, and its shared `integer` parser rejects price/lot values above u64.
...
- Expected versus actual behavior: an exact Phoenix instruction-evidence schema must accept every real bid/ask constructor output and reject any expiry or encoded integer the constructor cannot emit. Actual discovery still accepts an over-ceiling expiry and a u64-overflow encoded price.
- Acceptance criteria: bind `lastValidSlot <= quoteCurrentSlot + 150`; cap all constructor-u64 instruction evidence fields at `18446744073709551615` with canonical decimal encoding; retain exact keys, current slot order, side-specific zero/positive rules, output ordering, and real bid/ask controls; add isolated ceiling, overflow, and maximum-boundary tests.
- Validation results: real bid and ask constructor controls 2/2 PASS; all four prior side/lot/output contradictions reject PASS; over-ceiling expiry admitted FAIL; u64-overflow price admitted FAIL; direct constructor calls reject both; focused Meteora/Phoenix suite 9/9 PASS; full suite 427/427 PASS; syntax 86/86 PASS; replay PASS at 5,949.65 blocks/s with 9,618,064-byte heap growth.
...
- Severity: `FAIL` / `HIGH`
- Owner: `DEV`
- Reproduction: construct a real OpenBook V2 IOC preparation with `prepareOpenBookPlaceTakeOrderSimulation` and validate `preparation.instructionEvidence` against the catalog schema. Then validate the instruction-only control and independently mutate only (1) `quoteCurrentSlot` below `balanceSlot`, or (2) `priceLots` to `9223372036854775808` (`2^63`); compare both with direct builder calls.
- Evidence: `dc9cbb1` closes fifteen instruction-builder fields and binds `consumedInRaw <= amountInRaw`. The instruction-only control validates. The actual preparation contains a sixteenth constructor field, `minimumOutputRaw`, added before the body is returned; `additionalProperties:false` therefore rejects a real response. The schema also accepts both isolated mutations while `buildOpenBookPlaceTakeOrderInstruction` rejects current slot before balance and its signed-i64 price overflow.
...
- Expected versus actual behavior: the catalog schema must accept the exact instruction evidence returned by the preparation route and reject every slot/integer tuple the constructor rejects. Actual discovery rejects a real preparation body and admits two constructor-impossible states.
- Acceptance criteria: add and bind positive canonical `minimumOutputRaw <= quotedOutputRaw`; require `quoteCurrentSlot >= balanceSlot`; cap `priceLots`, `maxBaseLots`, and `maxQuoteLotsIncludingFees` at signed-i64 maximum and amount/output fields at u64 maximum; preserve exact closure, side/order/match-limit vocabulary, consumed-input conservation, and the real bid/ask preparation controls; add isolated equality, maximum-boundary, inversion, and overflow tests.
- Validation results: real instruction-only control PASS; real preparation body rejected FAIL; current-slot inversion admitted FAIL; signed-i64 overflow price admitted FAIL; direct builder rejects both mutations; focused Meteora/Phoenix/OpenBook suite 17/17 PASS; full suite 428/428 PASS; syntax 86/86 PASS; replay PASS at 5,132.73 blocks/s with 11,646,232-byte heap growth.
...
- Evidence: prior increments close handoff, transaction, policy, universal amounts, all three input/output effect bindings, catalog discovery, and type/protocol membership. Five instruction-evidence schemas plus CPMM, CLMM, Orca, and AMM v4 quotes match their constructors. Phoenix and OpenBook are referenced but retain independently reproduced constructor-parity failures; seven quote and four other instruction-evidence variants remain schema-null.
...
- Actual behavior: external authority, cataloged preparation identity, preparation, transaction, shared simulation policy, universal raw amounts, all three signer-to-effect relationships, five instruction-evidence variants, and four quote variants are verifiable. Phoenix and OpenBook are structurally referenced but semantically incomplete; seven quotes and four other instruction-evidence variants remain schema-null. Generated validators still cannot establish complete protocol evidence for thirteen variants.
...
- Validation results: prior closed boundaries remain PASS; strict protocol-specific schema coverage remains 9/22 PASS and 13/22 FAIL (four quote plus five instruction variants PASS; Phoenix and OpenBook instruction evidence incomplete; seven quotes and four other instruction variants null). Independent constructor validation closes Meteora, reproduces two Phoenix parity failures, rejects the real OpenBook preparation body, and reproduces two OpenBook constructor contradictions. Focused suite 17/17 PASS; full suite 428/428 PASS; syntax 86/86 PASS; replay PASS at 5,132.73 blocks/s with 11,646,232-byte heap growth.
...
- NEXT_DEV_ACTION: repair Phoenix expiry/u64 bounds and OpenBook real-body, slot-order, signed-i64/u64 bounds with exact boundary and overflow negatives before selecting another schema.
