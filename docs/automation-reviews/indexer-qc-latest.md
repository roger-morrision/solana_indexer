...
- Run: `2026-08-29T17:35:37+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `7b090c3fbb9d91f14c895d677225df1f8125c943`
- Compared with QA baseline: `1c5afd417351b20aeb68891adcab2c7baf3d4241` (2 DEV commits, 3 changed files)
- Compared with `origin/main`: 75 ahead, 0 behind before this evidence report
- Latest DEV commits: `7e5449f` (critical query-value profile closure) and `7b090c3` (string query-value profile closure)
- Overall result: 2 PASS, 0 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. The query-contract bootstrap schema now closes the exact-input `amountRaw`, scoped-pagination `cursor`, collection-filter, mint, and signed-tick profiles. The existing schema-only Meteora authenticity finding remains 21/22 strict outcomes because `binArrayPayloadHash` still lacks independent finalized anchoring. Independent reconciliation remains 68 PASS, 1 FAIL, and 1 BLOCKED across 70 domains; live qualification is blocked by absent fresh canonical evidence.
...
|---|---|---|---|
| `UPSTREAM-QUERY-CONTRACTS-CRITICAL-VALUE-PROFILES-SCHEMA-022` | `/api/v1/query-contracts` / `valueConstraints.amountRaw,cursor` | `PASS` | Both real closed profiles validate with exact kinds, patterns, bounds, maximum lengths, and cursor scope binding. Independent probes reject every missing, substituted, or wrong-type field, unknown field, and null/array/scalar/number/boolean profile shape: 39/39 negatives reject; real route admission parity also passes. |
| `UPSTREAM-QUERY-CONTRACTS-STRING-VALUE-PROFILES-SCHEMA-023` | `/api/v1/query-contracts` / `valueConstraints.collectionFilter,inputMint,limitTick` | `PASS` | The three closed profiles publish exact string lengths/control policy, nonempty mint policy, and canonical signed safe-integer pattern. A unified independent harness accepts all five current profiles and rejects 84/84 missing, substituted, wrong-type, unknown-field, and wrong-shape mutations. |

- Available DEV delta: exactly two distinct committed outcomes exist after `1c5afd4`, in `7e5449f` and `7b090c3`. QC detected the DEV writer lock and source movement during validation, discarded mixed-state results, waited for lock release, refreshed the complete stable two-commit delta, and reran every tier. No additional distinct DEV outcome exists.
- Verification result: 2 PASS, 0 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct outcomes, and splitting related profiles, fields, values, types, bounds, runtime cases, or mutation assertions would be padding.
- Validation: five real profile objects and all 84 independent negative mutations PASS. Strict protocol-specific schema closure remains 21/22 PASS and 1/22 FAIL; the recomputed coupled Meteora mutation remains schema-valid because its payload hash is unanchored. Focused contract 4/4 and Meteora suites 19/19 PASS (12 contract/quote and 7 execution); 79 response schemas, eleven quote references, eleven instruction references, 28 dialect keywords, 29 relationship kinds, registry digest `16c2606ec4548570f51c76e8fa1a941a2f9ea34e2529cf269dadc8a9eb84b667`, and contract digest `20d1dd5ab20871da783f6b271ef5c874db060a89f94112718ba6f1f5850ff12a` are structurally coherent; full suite 441/441 PASS; syntax 86/86 PASS; replay invariants PASS at 6,290.39 blocks/s with 9,512,928-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| HTTP response schema registry structure | `PASS` | The registry publishes 79 schemas with stable identities and canonical registry digest `16c2606ec4548570f51c76e8fa1a941a2f9ea34e2529cf269dadc8a9eb84b667`; the full contract digest is `20d1dd5ab20871da783f6b271ef5c874db060a89f94112718ba6f1f5850ff12a`. Independent recomputation matches. Recursive enumeration confirms all 28 live schema-node keywords and all 29 used relationship kinds are declared with no unused entries. The discovery bootstrap's catalog now closes critical exact-input, scoped-pagination, collection-filter, mint, and signed-tick profiles in addition to its prior fail-closed boundaries. |
