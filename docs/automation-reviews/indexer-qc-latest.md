...
- Run: `2026-08-29T19:36:09+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `67a7f7281f231cf6e31e3776f2bddbcd055ca22d`
- Compared with QA baseline: `7c6ffb261e3fa6a99d73a26ad1a28758af8a9432` (2 DEV commits, 3 changed files)
- Compared with `origin/main`: 81 ahead, 0 behind before this evidence report
- Latest DEV commits: `516d2f2` (preparation variant catalog schema closure) and `67a7f72` (response schema catalog closure)
- Overall result: 2 PASS, 0 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. The exact eleven-entry preparation catalog and exact 79-entry response-schema catalog now fail closed for omissions, duplicates, substitutions, unknown members, wrong value shapes, and outer-shape mutations. The existing schema-only Meteora authenticity finding remains 21/22 strict outcomes because `binArrayPayloadHash` still lacks independent finalized anchoring. Independent reconciliation remains 68 PASS, 1 FAIL, and 1 BLOCKED across 70 domains; live qualification is blocked by absent fresh canonical evidence.
...
|---|---|---|---|
| `UPSTREAM-QUERY-CONTRACTS-PREPARATION-CATALOG-SCHEMA-026` | `/api/v1/query-contracts` / `preparationVariants` | `PASS` | The schema requires the exact ordered eleven-entry catalog and the exact five fields per entry: `type`, `protocol`, `routeFamily`, `quoteSchema`, and `instructionEvidenceSchema`. The canonical catalog passes; 159/159 omission, duplicate, nonzero rotation, field-remap, missing-field, unknown-field, length, and outer-shape mutations reject. All eleven quote and instruction references resolve, with eleven unique quote schemas and ten unique instruction schemas as intended by the shared Pump instruction schema. |
| `UPSTREAM-QUERY-CONTRACTS-RESPONSE-SCHEMA-CATALOG-027` | `/api/v1/query-contracts` / `responseBodySchemas` | `PASS` | The schema requires exactly all 79 published response-schema names, rejects unknown names, and requires every value to be an object. The canonical registry passes; 480/480 per-name omissions, five wrong value shapes per name, unknown-name, and outer-shape mutations reject. All 113 HTTP response-body references resolve. |

- Available DEV delta: exactly two distinct committed outcomes exist after `7c6ffb2`, in `516d2f2` and `67a7f72`. QC detected the DEV writer lock and source movement during validation, discarded every mixed-state result, waited for lock release, refreshed the complete stable two-commit delta, and reran every tier. No additional distinct DEV outcome exists.
- Verification result: 2 PASS, 0 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct outcomes, and splitting catalog entries, schema names, fields, references, mutation classes, or assertions would be padding.
- Validation: preparation catalog 1/1 positive and 159/159 negative plus response-schema catalog 1/1 positive and 480/480 negative PASS. Strict protocol-specific schema closure remains 21/22 PASS and 1/22 FAIL; the recomputed coupled Meteora mutation remains schema-valid because its payload hash is unanchored. Focused contract 3/3 and Meteora suites 19/19 PASS (12 contract/quote and 7 execution); 79 response schemas, 113 HTTP outcome-schema references, eleven quote references, eleven instruction references, 28 dialect keywords, 29 relationship kinds, registry digest `61ee4de1bc527526efcdbf66d51f81d32850ba74991d72763ce77a6a4adfed0e`, and contract digest `a7d3a7aef3694265fe97ba5bfc487739f90d69415545db0789054a63dac9b8b2` are structurally coherent; full suite 441/441 PASS; syntax 86/86 PASS; replay invariants PASS at 7,744.33 blocks/s with 9,633,712-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
