...
- Run: `2026-08-29T10:35:31+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `bf2f410a477708d0ba7273b2c8d714fe670107d5`
- Compared with QA baseline: `41cb754c607341e3ea73873ff5f80ca10c3960a0` (2 DEV commits, 4 changed files)
- Compared with `origin/main`: 54 ahead, 0 behind before this evidence report
- Latest DEV commits: `22ec185` (per-array trading/protocol fee capacity) and `bf2f410` (per-array maximum fee-rate capacity)
- Overall result: 0 PASS, 2 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. Fixed-capacity fee and rate mutations now reject, but both advertised constructor-parity closures are incomplete because a row value and its producer-carried capacity can be changed together while preserving every published rule. The parent HIGH finding therefore remains 21/22 strict outcomes. Independent reconciliation remains 68 PASS, 1 FAIL, and 1 BLOCKED across 70 domains; live qualification is blocked by absent fresh canonical evidence.
...
|---|---|---|---|
| `UPSTREAM-METEORA-DLMM-BIN-ARRAY-FEE-CAPACITY-PARITY-008` | pool `prepare-swap` / trading and protocol fee capacity | `FAIL` | The real two-array quote emits trading fee/capacity `1/1` and `1/1`; changing only fees to `2/0` rejects, but changing the first capacity with its fee to `2/2` makes the same constructor-impossible redistribution pass every published rule. |
| `UPSTREAM-METEORA-DLMM-BIN-ARRAY-RATE-PARITY-009` | pool `prepare-swap` / maximum fee-rate capacity | `FAIL` | The finalized rows emit rate/capacity `25000/25000`; changing only a row rate rejects, but changing the row rate and its capacity together to `24999/24999` satisfies exact equality and the unchanged global maximum although the deterministic producer cannot emit that pair. |

- Available DEV delta: exactly two distinct committed outcomes exist after `41cb754`, in `22ec185` and `bf2f410`. Source advanced from `22ec185` to `bf2f410` during finalization; QC discarded the stale commit plan, refreshed the complete stable delta after DEV lock release, and reran every tier. No additional distinct DEV outcome exists.
- Verification result: 0 PASS, 2 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct outcomes, and splitting fee fields, rate fields, capacity fields, mutation probes, fee modes, or assertions would be padding.
- Validation: the real two-array and collect-fee-on-output controls PASS; fixed-capacity fee/rate mutations reject; coupled fee/capacity and rate/capacity mutations are admitted and retain the deduplicated parent HIGH finding. Strict protocol-specific closure remains 21/22 PASS and 1/22 FAIL. Focused Meteora suite 11/11 PASS; 78 response schemas, eleven quote references, eleven instruction references, 28 dialect keywords, 28 relationship kinds, and digest `c31e138cbe95850f453e6b8bf4391d2ea72d8ac8e22c4332f9e8417ae02b9bee` are structurally coherent; full suite 439/439 PASS; syntax 86/86 PASS; replay invariants PASS at 6,877.07 blocks/s with 9,824,688-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
- Severity: `FAIL` / `HIGH`
- Owner: `DEV`
- Reproduction: generate a real X-to-Y quote crossing bins 70 and 69 in arrays 1 and 0. The producer emits per-array trading fee/capacity pairs `1/1` and `1/1`. Change the nonterminal pair to `2/2` and the terminal fee to `0`, preserving the global fee total, consumed input, gross output, all other capacities, path endpoints, array indexes, protocol fees, maximum rate, direction, and transfer-fee evidence, then apply every published `meteora_dlmm_quote_v1` relationship.
- Evidence: `22ec185` publishes trading/protocol fee capacities and `bf2f410` publishes maximum fee-rate capacities. Both reject mutations when capacities are held fixed. Stronger coupled mutations remain structurally valid: traversal aggregates still equal top-level values, each row equals its altered capacity, and neither the capacity relationship nor a content commitment binds the capacity atoms to finalized bins, although the deterministic producer emits fee pairs `1/1` and rate pairs `25000/25000`.
...
- Expected versus actual behavior: the advertised schema must admit every real producer body and reject per-array economics the finalized producer cannot emit. Fixed producer-carried capacities are insufficient when the schema also admits constructor-impossible capacity values; row-local fee parity remains open.
- Acceptance criteria: retain all completed direction, transfer-fee, range, global aggregate, path, amount-capacity, collect-fee-on-output, and execution-gate controls; bind every traversal row's capacity tuple to sufficient finalized bin economics or a producer-derived content commitment; retain the real two-array positive plus fixed-capacity and coupled fee/capacity negatives.
- Validation results: real and fixed-capacity controls PASS; coupled fee/capacity and rate/capacity mutations FAIL because they are admitted. Focused Meteora suite 11/11 PASS; full 439/439 PASS; syntax 86/86 PASS; replay PASS at 6,877.07 blocks/s with 9,824,688-byte heap growth.
...
- NEXT_DEV_ACTION: bind each Meteora `binTraversal` capacity tuple to sufficient finalized bin economics or a producer-derived content commitment so coupled fee-and-capacity redistribution rejects, retaining the real two-array and collect-fee-on-output positives plus every completed direction, transfer-fee, aggregate, range, capacity, and execution-safety control.
