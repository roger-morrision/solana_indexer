...
- Run: `2026-08-29T20:35:40+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `17a476cf8a5d4ce2860f4105cbc403ab91fb3058`
- Compared with QA baseline: `50b1acdf93dbbad662b5e06fd909ea36833a4ba3` (2 DEV commits, 3 changed files)
- Compared with `origin/main`: 83 ahead, 0 behind before this evidence report
- Latest DEV commits: `107d5ff` (HTTP route-envelope schema) and `17a476c` (HTTP outcome-envelope schema)
- Overall result: 0 PASS, 2 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. Both committed tests pass, but they enforce semantic checks in local helper code that are absent from the published schema. The route schema admits duplicate paths with distinct envelopes plus method/path and parameter-partition contradictions; the outcome schema admits outcome/status/retryability, body-presence, and schema-reference contradictions. The existing schema-only Meteora authenticity finding remains 21/22 strict outcomes because `binArrayPayloadHash` still lacks independent finalized anchoring. Independent reconciliation is now 68 PASS, 3 FAIL, and 1 BLOCKED across 72 domains; live qualification is blocked by absent fresh canonical evidence.
...
|---|---|---|---|
| `UPSTREAM-QUERY-CONTRACTS-HTTP-ROUTE-ENVELOPE-SCHEMA-028` | `/api/v1/query-contracts` / `http[]` | `FAIL` | The real 54-route catalog validates and structural negatives reject, but `uniqueItems` only prevents duplicate whole objects. A distinct route object can reuse another route's `path` and remains schema-valid because no `unique_by(path)` relationship or exact catalog is published. Independent mutations also prove that `/rpc` can advertise GET, templated routes can omit path bindings, and required/optional parameter partitions can overlap while satisfying the schema. |
| `UPSTREAM-QUERY-CONTRACTS-HTTP-OUTCOME-SCHEMA-029` | `/api/v1/query-contracts` / `http[].responseOutcomes[]` | `FAIL` | The real outcomes validate and the four/five-field shapes are closed, but their fields are only independently typed/enumerated. Independent mutations admit `success` with status 503, successful status 200 as retryable, an `empty` body kind while a body remains required, and an unknown `private_schema_v1` reference. No relationships bind outcome/status/retryability or representation semantics, and schema references are not constrained to the exact response catalog. |

- Available DEV delta: exactly two distinct committed outcomes exist after `50b1acd`, in `107d5ff` and `17a476c`. QC detected the DEV writer lock and source movement during validation, discarded every mixed-state result, waited for lock release, refreshed the complete stable two-commit delta, and reran every tier. No additional distinct DEV outcome exists.
- Verification result: 0 PASS, 2 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct outcomes, and splitting route fields, outcome fields, individual mutation cases, schema keywords, or assertions would be padding.
- Validation: the real route and outcome controls PASS, but eight independent semantic contradictions are admitted 8/8. The committed focused test passes 1/1 because its helper manually enforces path uniqueness and only checks outcome field vocabularies; those helper-only rules are not in the published contract. Strict protocol-specific schema closure remains 21/22 PASS and 1/22 FAIL; the recomputed coupled Meteora mutation remains schema-valid because its payload hash is unanchored. Meteora suites 19/19 PASS (12 contract/quote and 7 execution); 54 routes, 79 response schemas, 113 HTTP outcome-schema references, 28 dialect keywords, 29 relationship kinds, registry digest `f19a691d97806ecbf130810d6df90f2827dafa80b928d99ac0320505eb9188a0`, and contract digest `53eaf2668bcb842e0cccff93af894bbe9f03bafd3c9c89f85eac1f800037244f` are structurally coherent; full suite 441/441 PASS; syntax 86/86 PASS; replay invariants PASS at 6,455.26 blocks/s with 9,639,064-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| HTTP route-envelope discovery | `FAIL` | All 54 real routes satisfy the ten-field closed row shape, but published `uniqueItems` does not enforce unique paths. Distinct objects with duplicate paths, a GET `/rpc`, missing template bindings, and overlapping required/optional partitions remain schema-valid. |
...
| Bounded performance | `PASS` | Full suite passes 441/441; syntax passes 86/86; replay completes at 6,455.26 blocks/s with 9,639,064-byte heap growth below 536,870,912 bytes. |
| Live operational qualification | `BLOCKED` | All six supported provider variables and active exporter, warehouse checkpoint/status, backup, and recovery files are absent while one retained external exporter artifact remains. Both retained indexes fail with `indexed_block_mainnet_identity_missing_or_invalid`; retained finalized exporter evidence has zero recorded failures but is 406,432 slots behind and 640,422,992 ms old at this trigger. |

The contract minimum is satisfied with 72 distinct evidence domains: 68 PASS, 3 FAIL, and 1 BLOCKED. These domains use separate contracts or failure boundaries and are not cosmetic splits.
...
- Severity: `FAIL` / `HIGH`
- Owner: `DEV`
- Reproduction: load `query_contracts_success_v1.properties.http`, validate the real 54-route catalog, then copy one route's `path` onto a different route without making the whole objects identical. Separately advertise `/rpc` as GET, remove a templated route's path binding, and place the same query names in both required and optional partitions. Apply only the published schema keywords and relationships.
- Evidence: the real catalog validates, but all four contradictions remain schema-valid. `uniqueItems` compares complete objects and cannot enforce unique values of `path`; the array publishes neither `exactItems` nor a `unique_by(path)` relationship. The committed test's `new Set(value.map(({ path }) => path))` check is helper-only and is not discoverable by generated clients. No relationship binds method to path, template placeholders to `pathParameters`, or `parameters` to disjoint required/optional partitions.
...
- Acceptance criteria: encode path uniqueness in the declared dialect, or publish the exact 54-entry route catalog; bind GET/POST to the correct paths, template placeholders to exact path-parameter keys, and query parameters to disjoint complete required/optional partitions. Retain the real catalog and each isolated negative.
- Validation results: real catalog PASS; duplicate-path, method/path, template-binding, and overlapping-partition negatives FAIL because all 4/4 are admitted. Focused committed test 1/1, full 441/441, syntax 86/86, Meteora 19/19, and replay PASS.
...
- NEXT_DEV_ACTION: encode the HTTP route catalog's claimed path uniqueness and protocol-invariant method, template-binding, and disjoint parameter-partition semantics in the published fail-closed schema itself, retaining the real 54-route control and rejecting all four independently reproduced contradictions without relying on test-only helper logic.
