...
|---|---|---|---|
| `3ee3657` / `UPSTREAM-METEORA-VERIFICATION-RECEIPT-122` | Complete pool-evidence receipt binding | `PASS` | Independent controls report baseline verification true, key-order-invariant canonical hashing true, same-address `tokenMint0` plus `baseFactor` mutation rejected, and the legacy under-bound receipt rejected. The new pool digest closes `UPSTREAM-QA-METEORA-VERIFICATION-RECEIPT-001` without weakening quote, slot, account, owner, payload-hash, policy, or receipt-hash checks. |
| `eb4afd5` / `UPSTREAM-METEORA-VERIFIED-PREPARATION-123` | Receipt-bound preparation and simulation | `FAIL` | Initial binding produces a valid content hash and committed tests reject unbound or pool-drifted input before RPC. Independent cross-quote mutation replaces the embedded receipt with one matching different amount economics, recomputes the preparation hash, and supplies that receipt's quote/pool; simulation calls local RPC once because it revalidates only receipt-to-quote/pool, not preparation-to-quote semantics. |

- Available DEV delta: exactly two distinct committed outcomes exist after `af45b55`, in `3ee3657` and `eb4afd5`; no additional distinct committed outcome exists.
...
- Validation: independent receipt controls report `receiptBaseline=true`, `keyOrderInvariant=true`, `sameAddressMutationRejected=true`, and `legacyReceiptRejected=true`. Independent preparation controls report `boundHashValid=true`, `crossQuoteReceiptProviderCalls=1`, and `crossQuoteReceiptRejectedBeforeRpc=false`, reproducing the missing simulation-time semantic rebind. Focused Meteora/transfer-hook tests 23/23 PASS; full suite 496/496 PASS; syntax 87/87 PASS; replay invariants PASS at 5,915.18 blocks/s with 9,631,976-byte heap growth. The real monitoring preflight reports explicit `SKIP/promtool_unavailable`; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Public mainnet RPC health PASS; all six checked provider variables are absent; local index status remains `wrong_network`; exporter health remains unavailable. The retained finalized external-exporter artifact has zero failures, is 406,432 slots behind, and is 838,444,805 ms old from its raw UTC `observedAt` timestamp at this trigger. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts. QA discarded all provisional `3ee3657` review when the DEV writer lock appeared, refreshed the complete two-commit delta, and reran every tier only after final `eb4afd5` was clean, stable, and unlocked.
...
### Finalized Meteora identity and verifier (2 FAIL at prior baseline)

- Prior exact shortfall: 18. `UPSTREAM-METEORA-FINALIZED-BIN-IDENTITY-116` and `UPSTREAM-METEORA-FINALIZED-BIN-VERIFIER-117` exposed the canonical-address and exact-context contradictions independently; both are closed by current IDs 118–119 while their owner/decoder/pool/index/hash/exact-quote controls remain retained.
...
| Swap-preparation success schema | `PASS` | Top-level, handoff/binding, preparation/transaction, simulation-policy, universal amount-effect, exact ordered eleven-entry catalog identity, and all eleven instruction-evidence variants are closed. All eleven quote schemas retain constructor parity; Meteora additionally binds canonical bin-array identity, exact finalized acquisition context, owner/decoder/pool/index/raw hash, and complete independent quote reproduction. |
...
- Severity: `PASS` (closed by `cd88f43` and `7b4c91d`)
- Owner: `DEV`
- Reproduction: inspect `meteora_dlmm_quote_v1.properties.binTraversal.items.properties.binArrayAddress`, the `meteora_finalized_bin_array_account` relationship, `verifyMeteoraDlmmQuoteFinalizedAccounts`, and `acquireAndVerifyMeteoraDlmmQuoteFinalizedAccounts`; then run canonical acquisition plus noncanonical-address and later-context negatives.
- Evidence: `cd88f43` publishes canonical Solana-public-key address bounds, changes the relationship to `exact_advertised_slot`, decodes every identity to 32 bytes, and requires supplied evidence context equality. `7b4c91d` adds bounded finalized `getMultipleAccounts` acquisition at the quoted minimum, requires every batch to return the exact advertised context, converts base64 bytes to evidence, and invokes the complete owner/10,136-byte decoder/pool/index/raw-SHA-256/exact-quote verifier. Canonical positive acquisition passes; noncanonical identities and later contexts reject, while preserved/recomputed economics and corrupt bytes remain rejected.
...
- Expected versus actual behavior: every advertised source identity is a canonical 32-byte Solana public key, and accepted independently fetched bytes are bound to the exact advertised finalized context before quote economics are trusted. Expected and actual now match; later provider contexts fail closed rather than being relabeled as historical evidence.
- Acceptance criteria: require canonical base58 `binArrayAddress`; require each supplied/fetched finalized account's returned context slot to equal the advertised row/top-level acquisition slot; reject the existing noncanonical positives and an otherwise-valid later-slot vector; retain owner, size, decoder, pool/index/hash, complete quote reproduction, real positive, preserved-digest, recomputed-digest, corrupt-byte, direction, transfer-fee, range, aggregate, path, capacity, and execution-gate controls. All criteria are met.
- Validation results: focused Meteora/transfer-hook tests 15/15 and full suite 495/495 PASS. Independent contract/runtime inspection confirms canonical identity, exact-context acquisition and verifier equality, bounded batching, and complete verifier chaining; syntax 87/87 and replay PASS at 7,845.92 blocks/s with 8,980,192-byte heap growth.
...
- Blockers: none; this finding is closed. Live provider usability remains part of the separate operational-qualification blocker and was not inferred from offline mocks.
...
- Severity: `PASS` (closed by the current Meteora finalized-account chain)
- Owner: `DEV`
...
- Evidence: prior increments close handoff, transaction, policy, universal amounts, all three input/output effect bindings, catalog discovery, and type/protocol membership. All eleven instruction-evidence variants and all eleven quote schemas are referenced. The current Meteora quote relationship now binds canonical bin-array identities to exact finalized account acquisition and the complete owner/decoder/pool/index/raw-hash/economics verifier, closing the last protocol-specific semantic gap.
...
- Actual behavior: external authority, cataloged preparation identity, preparation, transaction, shared simulation policy, universal raw amounts, all three signer-to-effect relationships, all eleven instruction-evidence variants, and all eleven quote variants are verifiable, including Meteora's exact finalized account context and independently reproduced economics.
...
- Validation results: all retained boundaries PASS; strict protocol-specific schema closure is now 22/22 PASS across eleven quote and eleven instruction variants. Current focused Meteora/transfer-hook tests pass 15/15, full suite 495/495, syntax 87/87, and replay invariants PASS.
...
- Evidence: `3ee3657` adds a deterministic SHA-256 `poolEvidenceHash` over canonicalized complete pool evidence, requires the strengthened verification policy, and intentionally rejects prior under-bound receipts. Independent controls report baseline and reordered-key verification true while same-address `tokenMint0` plus `baseFactor` mutation and the legacy receipt both reject.
...
- Expected versus actual behavior: the receipt verifier now rejects quote-critical pool drift, including same-address evidence changes, while preserving deterministic verification for semantically identical object key order.
...
- Validation results: all independent receipt controls PASS; focused Meteora/transfer-hook tests 23/23, full suite 496/496, syntax 87/87, and replay invariants PASS.
- Compatibility/performance impact: new receipts include `poolEvidenceHash` and the strengthened policy; prior receipts deliberately require regeneration. Hashing the bounded pool projection is negligible, with no focused, full-suite, syntax, or replay regression observed.
- Blockers: none; closed by `3ee3657`.

## UPSTREAM-QA-METEORA-PREPARATION-RECEIPT-002
