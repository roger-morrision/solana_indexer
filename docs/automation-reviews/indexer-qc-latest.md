...
- Run: `2026-08-30T04:35:17+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `412e190f3279950cd720939f9c2ecb3cb5321c89`
- Compared with QA baseline: `ed9b299d04b1e2e852c404ee98e78fc74e6619f9` (2 DEV commits, 3 changed files)
- Compared with `origin/main`: 108 ahead, 0 behind before this evidence report
- Latest DEV commits: `c58f917` (bound canonical block/transaction RPC result invariants) and `412e190` (closed paginated block/signature history RPC contracts)
- Overall result: 0 PASS, 2 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. The hardened singular schemas reject all nine retained unsafe mutations, and the new pagination descriptors, canonical pages, cursor controls, and thirteen page-schema negatives pass. However, canonical persistence still admits a retained block without count or finalized-provenance fields; both the singular and paginated RPC producers then return a successful body that violates their published block schema. The two reviewed outcomes therefore fail on one shared runtime/schema parity root. Independent reconciliation remains 82 PASS, 2 FAIL, and 1 BLOCKED across 85 deduplicated domains; live qualification remains blocked by absent fresh canonical evidence.

## Reviewed DEV delta (2/20)

### Canonical singular and paginated history RPC result contracts (2 FAIL)

| Item | Route | Status | Independent evidence |
|---|---|---|---|
| `UPSTREAM-RPC-BLOCK-TRANSACTION-CONTRACTS-042` | `/rpc` / `getIndexedBlock`, `getIndexedTransaction` | `FAIL` | The strengthened schemas reject all 5/5 block and 4/4 transaction mutations, and canonical fixtures/null misses/descriptors/catalogs validate. But `canonicalPersistedBlock` admits a retained block that has ancestry and time yet omits transaction/instruction counts and finalized provenance; `getIndexedBlock` returns that row successfully with `provenance:null` and omitted counts, failing `indexed_block_result_v1`. |
| `UPSTREAM-RPC-PAGINATED-HISTORY-CONTRACTS-043` | `/rpc` / `getIndexedBlocks`, `getIndexedSignaturesForAddress` | `FAIL` | Both exact parameter descriptors, the bootstrap catalog, four canonical real pages, two non-null cursor continuations, six runtime parameter negatives, thirteen schema negatives, and snapshot isolation pass. The accepted minimal retained block nevertheless propagates into a successful `getIndexedBlocks` page whose item fails the reused singular block schema. This is the same persistence/producer parity root as 042, not a padded independent domain. |

- Available DEV delta: exactly two distinct committed outcomes exist after `ed9b299`, in `c58f917` and `412e190`; the complete two-commit, three-file delta was exhausted. No additional distinct DEV outcome exists.
- Verification result: 0 PASS, 2 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct outcomes, and splitting schema fields, parameter styles, cursor cases, individual mutations, or shared-root symptoms would be padding.
- Validation: singular generated-style validation accepts 2/2 canonical results, 2/2 null misses, 2/2 parameter descriptors, and 2/2 bootstrap catalogs; rejects 5/5 block plus 4/4 transaction mutations; confirms `decimal_less_than` and detached snapshots, but a real response from an admitted minimal retained block fails its advertised schema. Pagination validation accepts 2/2 descriptors, the exact bootstrap map, 4/4 real canonical pages, 2/2 non-null cursor continuations, 6/6 runtime parameter negatives, and 13/13 schema negatives, with detached snapshots; the same admitted block makes a real page invalid. Focused RPC/query-contract run is 11/11 PASS, and Meteora suites remain 19/19 PASS (12 contract/quote and 7 execution). The registry digest is `216c43549ba7aa98ac304f14780e3079d89af4f4618cad1dcb5bd46afdfafda9`; the complete contract digest is `4234bea5f1115292af1f26bcb0d97dd30710059e76067bb20ca0342fb4f6ba69`. Full suite 447/447 PASS; syntax 86/86 PASS; replay invariants PASS at 8,608.98 blocks/s with 9,393,704-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
## Independent 85-domain reconciliation
...
| HTTP response schema registry structure | `PASS` | The registry publishes exactly 81 required object schemas and has canonical registry digest `216c43549ba7aa98ac304f14780e3079d89af4f4618cad1dcb5bd46afdfafda9`; the full contract digest is `4234bea5f1115292af1f26bcb0d97dd30710059e76067bb20ca0342fb4f6ba69`. Independent recomputation matches. |
...
| RPC block/history result discovery | `FAIL` | Canonical singular results, null misses, pagination descriptors/pages/cursors, dialect declarations, and snapshot isolation validate; all nine singular and thirteen page mutations reject. Runtime parity still fails because canonical persistence admits a retained block without counts or finalized provenance, and both singular and paginated RPC producers emit that schema-invalid row successfully. The two reviewed symptoms share this one deduplicated domain/root. |
...
| Bounded performance | `PASS` | Full suite passes 447/447; syntax passes 86/86; replay completes at 8,608.98 blocks/s with 9,393,704-byte heap growth below 536,870,912 bytes. |
| Live operational qualification | `BLOCKED` | All six supported provider variables and active exporter, warehouse checkpoint/status, backup, and recovery files are absent while one retained external exporter artifact remains. Both retained indexes fail with `indexed_block_mainnet_identity_missing_or_invalid`; retained finalized exporter evidence has zero recorded failures but is 406,432 slots behind and 669,200,464 ms old at this trigger. |

The contract minimum is satisfied with 85 distinct evidence domains: 82 PASS, 2 FAIL, and 1 BLOCKED. These domains use separate contracts or failure boundaries and are not cosmetic splits.
...
- Severity: `FAIL` / `HIGH`
- Owner: `DEV`
- Reproduction: resolve `indexed_block_result_v1` and `indexed_transaction_result_v1`; verify canonical results, null misses, descriptors, catalogs, and the nine retained schema negatives. Then apply a retained block with valid key/slot/blockhash/previousBlockhash/parentSlot/blockTime but no counts or provenance, confirm `IndexStore.indexedBlocks().available === true`, and request `getIndexedBlock([99])` through the real RPC handler.
- Evidence: both canonical producer results, both null misses, both descriptors, and both catalogs validate. The strengthened block/transaction schemas reject all 5/5 and 4/4 retained unsafe mutations and preserve snapshot isolation. However, persisted-block admission requires ancestry but not transaction/instruction counts or finalized provenance. The real singular producer consequently returns a successful non-null block with `provenance:null` and omitted count keys; that response fails `indexed_block_result_v1`.
...
- Expected versus actual behavior: every block accepted as canonical persistence must yield a non-null singular RPC result satisfying the published closed schema, or be rejected/quarantined before serving. Actual admission and projection are weaker than the advertised schema.
- Acceptance criteria: align persisted-block admission and projection with all required block result fields, including counts and finalized provenance; preserve positional/named calls, canonical positives, null misses, catalog closure, and all nine retained negatives; add a regression proving any accepted retained block validates its singular RPC schema.
- Validation results: canonical producer positives 2/2 PASS; null misses 2/2 PASS; descriptors 2/2 PASS; catalogs 2/2 PASS; unsafe schema mutations reject 9/9; admitted minimal retained block reproduces one schema-invalid successful result. Focused RPC/query-contract 11/11 PASS; Meteora 19/19 PASS; full 447/447 PASS; syntax 86/86 PASS; replay PASS at 8,608.98 blocks/s with 9,393,704-byte heap growth; operational health fails closed as designed.
- Compatibility/performance impact: schema discovery/digest changes are already present. Tightening bounded retained-block admission or completing the bounded projection may quarantine legacy partial rows or add required result keys; provider, production, and database state were not mutated.
...
- Severity: `FAIL` / `HIGH`
- Owner: `DEV`
- Reproduction: validate the exact positional/named descriptors for `getIndexedBlocks` and `getIndexedSignaturesForAddress`, the bootstrap result map, first and continued real pages with non-null cursors, six runtime parameter boundaries, and thirteen schema mutations. Then serve the same admitted minimal retained block used for 042 through `getIndexedBlocks` and validate the page against `indexed_blocks_result_v1`.
- Evidence: both descriptors, the bootstrap catalog, four canonical page bodies, two non-null cursor continuations, six runtime negatives, thirteen schema negatives, and detached snapshots pass. The page item reuses the singular block schema, but the producer also reuses the weaker retained-block projection; the successful page therefore contains the same `provenance:null` and omitted counts and fails its published page schema.
...
- Expected versus actual behavior: every successful page item emitted from accepted retained state must satisfy the exact item schema. Canonical fixture pages do, but an admitted partial retained block does not.
- Acceptance criteria: close the shared persisted-block admission/projection gap from 042 for both singular and page producers; preserve exact parameter defaults/bounds, base64url cursor rules, canonical first/continued pages, all six runtime negatives, all thirteen schema negatives, and signature-page coverage semantics.
- Validation results: descriptors 2/2 PASS; bootstrap 1/1 PASS; real canonical pages 4/4 PASS; non-null cursors 2/2 PASS; runtime parameter negatives 6/6 PASS; schema negatives 13/13 PASS; minimal retained-block page FAIL. This is one shared persistence/producer root with 042 and is not counted as an additional independent reconciliation domain.
- Compatibility/performance impact: exact page contracts and bounded 1-500 cardinality are compatible for canonical rows. Fixing the shared gap may quarantine legacy partial rows or complete their projection; validation remains bounded by the published page cap.
- Blockers: none; deterministic offline reproduction.
...
- NEXT_DEV_ACTION: align canonical persisted-block admission and both singular/paginated RPC producers with the published block schema by rejecting or quarantining retained blocks missing counts or finalized provenance, while preserving all nine singular negatives, page/cursor controls, and canonical real results.
