...
- Run: `2026-08-29T11:35:32+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `d9fcd1aca0388feeb1d142f1748d921b87c93e8f`
- Compared with QA baseline: `1ede880154c1377a09a6a2227f44ffc891411d81` (2 DEV commits, 5 changed files)
- Compared with `origin/main`: 57 ahead, 0 behind before this evidence report
- Latest DEV commits: `9c477e8` (per-array economics commitment) and `d9fcd1a` (execution-time economics reproduction)
- Overall result: 1 PASS, 1 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. Preserving the original commitment rejects coupled mutation, and current producer quotes are now reproduced from the finalized server-side pool before unsigned construction. Schema-only consumers can still recompute the unkeyed digest after changing row economics because `binArrayPayloadHash` is not bound to independently verifiable finalized evidence. The parent HIGH finding therefore remains 21/22 strict outcomes while the execution-time boundary passes. Independent reconciliation remains 68 PASS, 1 FAIL, and 1 BLOCKED across 70 domains; live qualification is blocked by absent fresh canonical evidence.
...
|---|---|---|---|
| `UPSTREAM-METEORA-DLMM-BIN-ARRAY-CAPACITY-COMMITMENT-010` | pool `prepare-swap` / finalized economics commitment | `FAIL` | A real row's commitment verifies and changing fee/capacity while preserving the old commitment rejects. Recomputing `capacityCommitment` over the altered tuple passes, because the included payload hash is not externally anchored and the digest proves only self-consistency of mutable response fields. |
| `UPSTREAM-METEORA-DLMM-EXECUTION-RECOMPUTATION-011` | pool `prepare-swap` / unsigned construction | `PASS` | Current producer quotes carry `calculatedAtUnixMs`; execution reruns the exact quote engine over the finalized server-side pool and requires deep equality. A coupled fee/capacity mutation with recomputed digest rejects before instruction construction, while real preparation remains unsigned and policy-bound. |

- Available DEV delta: exactly two distinct committed outcomes exist after `1ede880`, in `9c477e8` and `d9fcd1a`. QC detected the active DEV writer lock and source edits after provisional `9c477e8` validation, withheld the QA commit, waited for release, refreshed to clean stable `d9fcd1a`, and reran every tier. No additional distinct DEV outcome exists.
- Verification result: 1 PASS, 1 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct outcomes, and splitting commitment fields, execution checks, digest inputs, mutation probes, or assertions would be padding.
- Validation: real commitment and preserved-digest mutation controls PASS; schema-only recomputation remains admitted; execution-time deep reproduction rejects the recomputed coupled mutation. Strict protocol-specific schema closure remains 21/22 PASS and 1/22 FAIL. Focused Meteora suite 12/12 PASS; 78 response schemas, eleven quote references, eleven instruction references, 28 dialect keywords, 29 relationship kinds, and digest `0e171b3ac4d7606ebed660ab742fb2adc81edd92b6b90a46f411f567de798977` are structurally coherent; full suite 440/440 PASS; syntax 86/86 PASS; replay invariants PASS at 6,614.54 blocks/s with 9,628,472-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
- Validation results: real and preserved-digest controls PASS; execution-time recomputation PASS; schema-only recomputed coupled mutation FAIL because it is admitted. Focused Meteora suite 12/12 PASS; full 440/440 PASS; syntax 86/86 PASS; replay PASS at 6,614.54 blocks/s with 9,628,472-byte heap growth.
...
- Validation results: prior closed boundaries and Meteora execution reproduction PASS; strict protocol-specific schema closure remains 21/22 PASS and 1/22 FAIL (ten quote plus all eleven instruction variants PASS; Meteora finalized-content discovery parity fails). Focused Meteora suite 12/12 PASS; full suite 440/440 PASS; syntax 86/86 PASS; replay PASS at 6,614.54 blocks/s with 9,628,472-byte heap growth.
