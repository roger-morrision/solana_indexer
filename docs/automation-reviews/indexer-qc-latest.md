...
- Run: `2026-08-29T08:35:59+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `2336e0c8e4131fe1c338d9758d985cf140463b03`
- Compared with QA baseline: `c653beb4add5179a70a6c3e977b623ec08ad3e5e` (4 DEV commits, 4 changed files)
- Compared with `origin/main`: 47 ahead, 0 behind before this evidence report
- Latest DEV commits: `0bd3e97` (direction/fee-mode semantics), `a51376d` (transfer-fee economics), `12d41c1` (bin-traversal aggregates), and `2336e0c` (decoder-range caps)
- Overall result: 3 PASS, 1 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. Direction/fee-mode, exact transfer-fee, and decoder-range outcomes close their advertised boundaries. The traversal outcome binds global sums, endpoints, ordering, rate, and array membership, but a real two-array quote still admits constructor-impossible per-array redistribution while every published rule remains true. The parent HIGH finding remains 21/22 strict outcomes: all eleven instruction schemas and ten quote schemas retain constructor parity, while Meteora remains semantically underbound. Independent reconciliation remains 68 PASS, 1 FAIL, and 1 BLOCKED across 70 domains; live qualification is blocked by absent fresh canonical evidence.

## Reviewed DEV delta (4/20)

### Meteora quote semantic closure (3 PASS, 1 FAIL)

| Item | Route | Status | Independent evidence |
|---|---|---|---|
| `UPSTREAM-METEORA-DLMM-QUOTE-PARITY-002` | pool `prepare-swap` / directional fee mode | `PASS` | Four real X-to-Y/Y-to-X and collect-fee-mode controls validate; all six isolated fee-flag and reverse-traversal contradictions reject. |
| `UPSTREAM-METEORA-DLMM-TRANSFER-FEE-PARITY-003` | pool `prepare-swap` / legacy and Token-2022 transfer fees | `PASS` | Real zero-fee legacy and 100-bps/capped Token-2022 bodies validate. Isolated input fee, output fee, input basis-point, and output maximum-fee contradictions reject under exact inverse/capped ceiling math. |
| `UPSTREAM-METEORA-DLMM-BIN-TRAVERSAL-PARITY-004` | pool `prepare-swap` / bounded bin-array traversal | `FAIL` | A real two-array quote emits consumed/output rows `101/100` and `99/98`. Redistributing them to `102/101` and `98/97` preserves global totals, endpoints, order, array set, fees, and maximum rate and remains schema-valid although the finalized first bin has output capacity 100. |
| `UPSTREAM-METEORA-DLMM-BIN-ARRAY-RANGE-PARITY-005` | pool `prepare-swap` / decoder domain | `PASS` | Top-level and traversal indexes are capped to `-6656..6655`; start/end bins are capped to `-465920..465919`; both bounds map exactly through 70 bins per array and exclude the adjacent impossible indexes. |

- Available DEV delta: exactly four distinct committed outcomes exist after `c653beb`, one in each of `0bd3e97`, `a51376d`, `12d41c1`, and `2336e0c`. QC detected source movement while launching validation, discarded that mixed-state run, refreshed to clean stable `2336e0c`, and reran every tier. No additional distinct DEV outcome exists.
- Verification result: 3 PASS, 1 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 16; the stable delta contains exactly four distinct outcomes, and splitting fields, relationship descriptors, quote directions, mutation probes, or range endpoints would be padding.
- Validation: four real directional controls and all six direction negatives PASS; real legacy/Token-2022 controls and four transfer-fee negatives PASS; decoder limits match all six schema locations; the real two-array traversal validates but one per-array redistribution contradiction is admitted. Strict protocol-specific closure remains 21/22 PASS and 1/22 FAIL. Focused committed suite 6/6 PASS; 78 response schemas, eleven quote references, eleven instruction references, 28 dialect keywords, 27 relationship kinds, and digest `867dca135948e5c249bf1e952446a51b1123cfe6c73399d87c6fec7acc42cf0c` are structurally coherent; full suite 438/438 PASS; syntax 86/86 PASS; replay invariants PASS at 6,565.79 blocks/s with 9,384,120-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| HTTP response schema registry structure | `PASS` | The registry publishes 78 schemas with stable identities and digest `867dca135948e5c249bf1e952446a51b1123cfe6c73399d87c6fec7acc42cf0c`. Independent recomputation matches. Recursive enumeration confirms all 28 live schema-node keywords and all 27 relationship kinds are declared, including Meteora direction, transfer-fee, and bin-traversal kinds, while unknown future keywords remain fail closed. |
...
- Severity: `FAIL` / `HIGH`
- Owner: `DEV`
- Reproduction: generate a real X-to-Y quote crossing bins 70 and 69 in arrays 1 and 0. The producer emits per-array `consumedInRaw/grossOutputRaw` pairs `101/100` and `99/98`. Change only those pairs to `102/101` and `98/97`, preserving all global totals, path endpoints, array indexes, fees, rates, direction, and transfer-fee evidence, then validate against `meteora_dlmm_quote_v1`.
- Evidence: `0bd3e97` closes direction and fee-mode semantics, `a51376d` closes legacy/Token-2022 transfer-fee reconstruction, `12d41c1` publishes bounded per-array traversal aggregates, and `2336e0c` matches decoder index/bin ranges. The real two-array body validates. The redistributed body also validates even though finalized bin 70 has output capacity 100 and the producer cannot emit 101 from that array. Global sum relationships cannot bind each row to finalized bin liquidity/price/economics.
...
- Expected versus actual behavior: the advertised schema must admit every real producer body and reject per-array economics the finalized producer cannot emit. Direction, transfer-fee, range, global aggregate, and path contracts now match; row-local constructor parity does not.
- Acceptance criteria: retain the four real directional controls, exact transfer-fee boundaries, decoder caps, global sums, endpoint/order/array-set constraints, and immutable execution gates; bind every traversal row to sufficient finalized bin evidence or a producer-derived content commitment so cross-array consumed/output/fee redistribution rejects; add a real two-array positive and isolated redistribution negatives.
- Validation results: directional controls 4/4 PASS and negatives 6/6 reject; legacy/Token-2022 controls and four fee contradictions PASS; range limits match six schema locations; real two-array quote PASS; cross-array redistribution negative FAIL because it is admitted. Focused committed suite 6/6 PASS; full 438/438 PASS; syntax 86/86 PASS; replay PASS at 6,565.79 blocks/s with 9,384,120-byte heap growth.
- Compatibility/performance impact: the current additive quote fields and relationship kinds change digest/ETag and generated validators. A bounded row commitment or finalized row-local evidence must remain within the existing 13,312-row cap; no runtime throughput regression is currently observed.
...
- Evidence: prior increments close handoff, transaction, policy, universal amounts, all three input/output effect bindings, catalog discovery, and type/protocol membership. All eleven instruction-evidence variants and all eleven quote schemas are referenced. Ten quote schemas match their constructors. Meteora direction, transfer fees, aggregate traversal, and decoder ranges are now bound, but a constructor-impossible cross-array amount redistribution remains admitted.
...
- Validation results: prior closed boundaries remain PASS; strict protocol-specific closure remains 21/22 PASS and 1/22 FAIL (ten quote plus all eleven instruction variants PASS; Meteora row-local quote parity fails). Focused suite 6/6 PASS; full suite 438/438 PASS; syntax 86/86 PASS; replay PASS at 6,565.79 blocks/s with 9,384,120-byte heap growth.
...
- NEXT_DEV_ACTION: bind each Meteora `binTraversal` row to sufficient finalized bin economics or a producer-derived content commitment so cross-array consumed/output/fee redistribution rejects, retaining the real two-array positive, isolated redistribution negatives, and all completed direction, transfer-fee, aggregate, range, and execution-safety controls.
