...
- Run: `2026-08-28T20:36:17+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `2cf7f81444a225c0aa7eeeb2919fb198069da812`
- Compared with QA baseline: `76d3a4689d8614d4a29c14af09d35783df1d2c81` (2 DEV commits, 3 changed files)
- Compared with `origin/main`: 12 ahead, 0 behind before this evidence report
- Latest DEV commits: `dc1808d` (Orca quote schema) and `2cf7f81` (AMM v4 quote schema)
- Overall result: 0 PASS, 2 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. Both catalog references, exact key closure, identity constants, slot ordering, and advertised relationships are structurally coherent, but neither new quote schema has real-constructor semantic parity. Orca admits out-of-range ticks, unrelated square-root price limits, and a fee rate the constructor rejects. AMM v4 admits output amounts impossible under its constant-product reserves. The parent HIGH finding remains open. Live qualification is independently blocked by absent fresh canonical evidence.
...
|---|---|---|---|
| `UPSTREAM-QA-ORCA-WHIRLPOOL-QUOTE-SCHEMA-001` | pool quote / Orca catalog row | `FAIL` | The valid control, exact key closure, amount sum, tick cardinality, and all three slot inversions behave as advertised. However, generated validation admits 3/3 isolated constructor contradictions: `limitTick:-443637`, an unrelated `sqrtPriceLimitX64:"1"`, and `feeRateMillionths:"1000000"`. The real constructor rejects the tick and fee bound, while execution requires the square-root price derived exactly from the tick. |
| `UPSTREAM-QA-RAYDIUM-AMM-V4-QUOTE-SCHEMA-001` | pool quote / AMM v4 catalog row | `FAIL` | The valid constructor control, exact key closure, reserve mode, fee-less-than-input rule, missing gates, and three slot inversions pass. Yet the schema admits `amountOutRaw == outputReserveRaw`; the real constant-product constructor produced `1813` for the same `1000` input, `3` fee, and `10000/20000` reserves and rejects output at or above the output reserve. |

- Available DEV delta: two BA/PO-selected commits after `76d3a46` add two distinct quote-schema outcomes. The complete delta was exhausted. The AMM v4 DEV writer turn began during the first Orca validation; mixed-state digest/results were discarded, QC waited for lock release and commit, refreshed from `dc1808d` to stable `2cf7f81`, and reran every tier. No additional DEV commit or enhancement exists.
- Verification result: 0 PASS, 2 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct protocol-schema outcomes, and splitting individual contradiction probes, properties, or assertions would be padding.
- Validation: the independent generated-validator harness accepts both real-constructor controls but reproduces four admitted contradictions: three Orca price/tick/fee states and one AMM v4 reserve/output state. Strict protocol-specific closure remains 6/22 PASS and 16/22 FAIL (CPMM and CLMM quotes plus four instruction-evidence variants PASS; Orca and AMM v4 quotes referenced but incomplete; seven quote variants and seven instruction variants null). Focused committed suite 2/2 PASS; 65 response schemas, 10 declared relationship kinds, and digest `17bf3861e8785c67f9a5647e37a221b238cf83623516bd9a89f1ed8ba14f30dc` are structurally coherent; full suite 425/425 PASS; syntax 86/86 PASS; replay invariants PASS at 7,575.25 blocks/s with 9,446,280-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| HTTP response schema registry structure | `PASS` | The registry publishes 65 schemas with stable identities and digest `17bf3861e8785c67f9a5647e37a221b238cf83623516bd9a89f1ed8ba14f30dc`. Independent recomputation matches. Recursive enumeration confirms every live schema-node keyword and all ten relationship kinds are declared, including decimal ordering, conditional types, array cardinality, canonical decimal sums, and fail-closed preparation catalog membership, while unknown future keywords remain fail closed. |
...
| Swap-preparation success schema | `FAIL` | Top-level, handoff/binding, preparation/transaction, simulation-policy, universal amount-effect, and catalog identity boundaries are closed. Four instruction-evidence variants plus CPMM and CLMM quotes have constructor parity. Orca and AMM v4 quote schemas are referenced but semantically incomplete; seven quote variants and seven instruction-evidence variants remain schema-null. |
...
- Evidence: `dc1808d` publishes exact closure, tick-array cardinality, exact-input conservation, static-fee identity, and `stateSlot <= tickArraySlot <= balanceSlot <= mintEvidenceSlot`. The valid control passes and all three direct slot inversions reject. Nevertheless, its schema has no Orca tick bounds, no `sqrtPriceLimitX64 = orcaSqrtPriceX64AtTick(limitTick)` binding, and no fee-rate ceiling. Independent generated validation admits all 3/3 contradictions. The real math constructor rejects the out-of-range tick and denominator fee; the execution guard independently requires the exact tick-derived square-root price.
...
- Expected versus actual behavior: an exact Orca quote schema must reject any price-limit or fee tuple the real constructor/execution path rejects. Actual discovery accepts three constructor-impossible tuples.
...
- Validation results: real control 1/1 PASS; exact key/constant/redaction controls PASS; amount and tick relationships 2/2 PASS; slot inversions reject 3/3 PASS; tick/price/fee contradictions admitted 3/3 FAIL; focused committed suite 2/2 PASS; full suite 425/425 PASS; syntax 86/86 PASS; replay PASS at 7,575.25 blocks/s with 9,446,280-byte heap growth.
...
- Evidence: `2cf7f81` publishes exact closure, authoritative reserve mode, exact missing gates, `swapFeeRaw < amountInRaw`, and `stateSlot <= openOrdersSlot <= marketSlot <= balanceSlot`. The real constructor produces `amountOutRaw:"1813"` for the control. The generated schema admits `amountOutRaw:"20000"` because it neither requires output below the output reserve nor binds output to `(amountIn - fee) * outputReserve / (inputReserve + amountIn - fee)`. The constructor cannot emit the admitted tuple.
...
- Expected versus actual behavior: an exact reserve-backed quote schema must bind the quoted output to the constructor's integer constant-product result. Actual discovery accepts arbitrary positive output, including the complete output reserve.
...
- Validation results: real constructor control 1/1 PASS; exact key/constant/reserve/redaction controls PASS; fee-bound and missing-gate controls PASS; slot inversions reject 3/3 PASS; isolated reserve/output contradiction admitted 1/1 FAIL; focused committed suite 2/2 PASS; full suite 425/425 PASS; syntax 86/86 PASS; replay PASS at 7,575.25 blocks/s with 9,446,280-byte heap growth.
...
- Evidence: prior increments close handoff, transaction, policy, universal amounts, all three input/output effect bindings, catalog discovery, and type/protocol membership. Four instruction-evidence schemas plus CPMM and CLMM quotes match their constructors. Orca and AMM v4 quote schemas are referenced but admit independently reproduced constructor contradictions; seven quote and seven instruction-evidence variants remain schema-null.
...
- Actual behavior: external authority, cataloged preparation identity, preparation, transaction, shared simulation policy, universal raw amounts, all three signer-to-effect relationships, four instruction-evidence variants, and CPMM/CLMM quotes are verifiable. Orca and AMM v4 are structurally referenced but semantically incomplete; seven quotes and seven instruction-evidence variants remain schema-null. Generated validators still cannot establish complete quote/output provenance or protocol shape for sixteen variants.
...
- Validation results: prior closed boundaries remain PASS; strict protocol-specific schema coverage remains 6/22 PASS and 16/22 FAIL (CPMM and CLMM quotes plus four instruction-evidence variants PASS; Orca and AMM v4 quotes incomplete; seven quotes and seven instruction variants null). The new generated-validator harness admits four constructor contradictions. Focused suite 2/2 PASS; full suite 425/425 PASS; syntax 86/86 PASS; replay PASS at 7,575.25 blocks/s with 9,446,280-byte heap growth.
...
- NEXT_DEV_ACTION: repair Orca tick/price/fee semantic bounds and AMM v4 exact constant-product output conservation with isolated constructor-parity negatives before selecting another quote schema.
