...
- Run: `2026-08-29T12:36:33+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `97befe56f58b14ae591e18b077cd3659966714a0`
- Compared with QA baseline: `508aaf48dd21070eccb994715e0f5267d7a5df1f` (2 DEV commits, 5 changed files)
- Compared with `origin/main`: 60 ahead, 0 behind before this evidence report
- Latest DEV commits: `a5583fe` (quote-reproduction instruction evidence) and `97befe5` (real-producer verification regression)
- Overall result: 2 PASS, 0 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. Meteora instruction evidence now exposes whether exact finalized-bin economics were reproduced or the compatibility-only path remained unattested, and a real quote-engine result proves the verified value is assigned only after reproduction and deep equality succeed. The existing schema-only authenticity finding remains 21/22 strict outcomes because `binArrayPayloadHash` still lacks independent finalized anchoring. Independent reconciliation remains 68 PASS, 1 FAIL, and 1 BLOCKED across 70 domains; live qualification is blocked by absent fresh canonical evidence.
...
|---|---|---|---|
| `UPSTREAM-METEORA-DLMM-REPRODUCTION-EVIDENCE-012` | pool `prepare-swap` / instruction evidence | `PASS` | `quoteEconomicsVerification` is required and closed to `reproduced_from_finalized_bin_arrays` or `legacy_unattested`; current calculated quotes reach the verified value only after exact reproduction, legacy construction is explicitly unattested, unknown/extra evidence remains rejected, and unsigned preparation controls pass. |
| `UPSTREAM-METEORA-DLMM-VERIFICATION-PROOF-013` | Meteora real quote engine / instruction construction | `PASS` | A real finalized snapshot quote now reaches unsigned construction and asserts `reproduced_from_finalized_bin_arrays`; the implementation captures one validated calculation timestamp and assigns the verified label only after exact reproduction and deep equality, while a coupled recomputed mutation still rejects. |

- Available DEV delta: exactly two distinct committed outcomes exist after `508aaf4`, in `a5583fe` and `97befe5`. QC detected source movement under the active DEV writer lock, discarded mixed-state checks, waited for release, refreshed the complete stable delta, and reran every tier. No additional distinct DEV outcome exists.
- Verification result: 2 PASS, 0 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct outcomes, and splitting enum values, evidence fields, compatibility branches, fixture setup, or individual assertions would be padding.
- Validation: the new required closed enum, verified current-producer branch, explicit legacy-unattested branch, instruction schema, real quote-engine regression, coupled-mutation rejection, and unsigned preparation controls PASS. Strict protocol-specific schema closure remains 21/22 PASS and 1/22 FAIL. Focused Meteora suites 19/19 PASS (12 contract/quote and 7 execution); 78 response schemas, eleven quote references, eleven instruction references, 28 dialect keywords, 28 relationship kinds, and digest `001af425323d8235d717921057d082153c646ee083a7592e9bf64fea0a4cd934` are structurally coherent; full suite 440/440 PASS; syntax 86/86 PASS; replay invariants PASS at 6,712.00 blocks/s with 9,619,168-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| HTTP response schema registry structure | `PASS` | The registry publishes 78 schemas with stable identities and canonical registry digest `616a25a55ae5b80ed0c935e8066432f276de460c700e90490d087535930326e6`; the full contract digest is `001af425323d8235d717921057d082153c646ee083a7592e9bf64fea0a4cd934`. Independent recomputation matches. Recursive enumeration confirms all 28 live schema-node keywords and all 28 relationship kinds are declared, including Meteora direction, transfer-fee, bin-traversal, and capacity-commitment kinds, while unknown future keywords remain fail closed. |
...
- Validation results: real and preserved-digest controls PASS; execution-time reproduction, required evidence labeling, and real-producer verification proof PASS; schema-only recomputed coupled mutation FAIL because it is admitted. Focused Meteora suites 19/19 PASS; full 440/440 PASS; syntax 86/86 PASS; replay PASS at 6,712.00 blocks/s with 9,619,168-byte heap growth.
...
- Validation results: prior closed boundaries, Meteora execution reproduction, instruction-evidence labeling, and the real-producer verification proof PASS; strict protocol-specific schema closure remains 21/22 PASS and 1/22 FAIL (ten quote plus all eleven instruction variants PASS; Meteora finalized-content discovery parity fails). Focused Meteora suites 19/19 PASS; full suite 440/440 PASS; syntax 86/86 PASS; replay PASS at 6,712.00 blocks/s with 9,619,168-byte heap growth.
