...
- Run: `2026-08-31T01:36:07+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `ca515717c678e120aefc83021ea65cb3145102d6`
- Compared with QA baseline: `ba1c56d9e68fb0520a59b56924539b588c3d63a6` (2 DEV commits, 3 changed files)
- Compared with `origin/main`: 171 ahead, 0 behind before this evidence report
- Latest DEV commits: `63f07e7` (semantic alert hold-timing evaluation) and `ca51571` (transient semantic-alert recovery evaluation)
- Overall result: 2 PASS, 0 FAIL, 0 BLOCKED, and 0 SKIP across the complete two-outcome DEV delta. `UPSTREAM-PROMTOOL-ALERT-HOLD-SEMANTICS-074` proves all three semantic alerts remain pending at zero minutes for sustained contradictions and absence, then fire after one minute; `UPSTREAM-PROMTOOL-TRANSIENT-RECOVERY-075` proves contradictions or missing required series restored after 30 seconds never reach firing at one minute. Reconciliation is 114 PASS, 1 FAIL, and 1 BLOCKED across 116 deduplicated domains; live qualification remains blocked despite a healthy public mainnet RPC probe because the retained local index, exporter, warehouse, backup, and recovery evidence is not current canonical operational evidence.
...
|---|---|---|---|
| `63f07e7` / `UPSTREAM-PROMTOOL-ALERT-HOLD-SEMANTICS-074` | Semantic alert pending-to-firing transition | `PASS` | Finite contradictions and complete series absence each evaluate the retry, holder-exclusion, and progress alerts at zero minutes with explicit empty results, then preserve the exact critical outcomes after one minute. Independent checks bind six pending evaluations, ten total non-firing expectations, exact per-alert transitions, retained one-minute rules, severity, fixed cardinality, and credential-free input. |
| `ca51571` / `UPSTREAM-PROMTOOL-TRANSIENT-RECOVERY-075` | Transient semantic-alert recovery | `PASS` | Two 30-second profiles begin with either all three finite contradictions or missing retry, exclusion, and source-tip series, restore canonical values after 30 seconds, and explicitly require all three alerts to remain absent at one minute. Independent controls bind both transitions, six recovery expectations, exact alert identities, retained holds, and absence of credentials or endpoint material. |

- Available DEV delta: exactly two distinct committed outcomes exist after `ba1c56d`, in `63f07e7` and `ca51571`; QC detected the root DEV writer lock and source movement during validation, withheld the prepared report, discarded mixed-state results, refreshed after lock release, and reran every tier. No additional distinct DEV outcome exists.
- Verification result: 2 PASS, 0 FAIL, 0 BLOCKED, 0 SKIP across the two distinct DEV outcomes.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct outcomes, and splitting alert identities, pending assertions, recovery transitions, one-minute outcomes, severity, or fixture groups would be padding.
- Validation: independent alert-timing/recovery harness 53/53 PASS; focused monitoring 3/3 PASS; focused RPC/query-contract 19/19 PASS; focused health/feed/monitoring 37/37 PASS; focused Meteora/transfer-hook 13/13 PASS. The response-schema registry digest remains `cb5e53a7edef553930dce900aef315b3a3aeb71373a4584fdf3f3a8dcb8dcfe2`; the complete contract digest remains `949e3a693b71085fa42667e7d7db51bd1c2beaca90ccb2177af95e95199824c1`. Full suite 469/469 PASS; syntax 87/87 PASS; replay invariants PASS at 5,266.88 blocks/s with 9,496,408-byte heap growth. The real monitoring preflight reports explicit `SKIP/promtool_unavailable`; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Public mainnet RPC health PASS; local index status remains `wrong_network`, and exporter health remains unavailable. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| Bounded performance | `PASS` | Full suite passes 469/469; syntax passes 87/87; replay completes at 5,266.88 blocks/s with 9,496,408-byte heap growth below 536,870,912 bytes. |
| Live operational qualification | `BLOCKED` | All six supported provider variables and active exporter, warehouse checkpoint/status, backup, and recovery files are absent while one retained external exporter artifact remains. The retained local index fails with `indexed_block_mainnet_identity_missing_or_invalid`; retained finalized exporter evidence has zero recorded failures but is 406,432 slots behind and 770,050,431 ms old at this trigger. |

The review reconciles 116 distinct evidence domains: 114 PASS, 1 FAIL, and 1 BLOCKED. These domains use separate contracts or failure boundaries and are not cosmetic splits.
