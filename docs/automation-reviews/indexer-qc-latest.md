...
- Run: `2026-08-29T13:35:33+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `c48b9161982a19ca4e650333701c5df027f7fbda`
- Compared with QA baseline: `f1387c6566a54dcd9b9298c72fa21935bdb37024` (2 DEV commits, 3 changed files)
- Compared with `origin/main`: 63 ahead, 0 behind before this evidence report
- Latest DEV commits: `c1540d1` (Meteora capacity-dialect parity) and `c48b916` (query-contract bootstrap schema)
- Overall result: 2 PASS, 0 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. The fail-closed response-schema dialect now declares all 29 used relationship kinds, and `/api/v1/query-contracts` publishes a closed bootstrap schema for its own exact top-level envelope. The existing schema-only authenticity finding remains 21/22 strict outcomes because `binArrayPayloadHash` still lacks independent finalized anchoring. Independent reconciliation remains 68 PASS, 1 FAIL, and 1 BLOCKED across 70 domains; live qualification is blocked by absent fresh canonical evidence.
...
|---|---|---|---|
| `UPSTREAM-CONTRACT-DIALECT-METEORA-CAPACITY-014` | `/api/v1/query-contracts` / response-schema dialect | `PASS` | `meteora_bin_array_output_capacity` is both used by the live Meteora quote schema and now declared by the fail-closed dialect. Independent recursive enumeration finds 29 used and 29 declared relationship kinds, zero undeclared, and zero unused; the exhaustive committed regression and exact ordered dialect control pass. |
| `UPSTREAM-QUERY-CONTRACTS-BOOTSTRAP-SCHEMA-015` | `/api/v1/query-contracts` / HTTP 200 body | `PASS` | The real discovery document validates against closed `query_contracts_success_v1` with all eleven exact top-level sections. Removing any required section, adding an unknown field, emptying the HTTP or preparation catalog, or replacing the lowercase SHA-256 identity rejects; runtime response bytes remain unchanged. |

- Available DEV delta: exactly two distinct committed outcomes exist after `f1387c6`, in `c1540d1` and `c48b916`. QC detected source movement under the active DEV writer lock, discarded mixed-state checks, waited for release, refreshed the complete stable delta, and reran every tier. No additional distinct DEV outcome exists.
- Verification result: 2 PASS, 0 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct outcomes, and splitting one declaration, traversal mechanics, top-level sections, schema occurrences, or individual assertions would be padding.
- Validation: the exact ordered dialect control, exhaustive used-kind traversal, explicit target presence, real bootstrap document, eleven missing-section negatives, unknown-field negative, two empty-catalog negatives, and malformed-hash negative PASS. Strict protocol-specific schema closure remains 21/22 PASS and 1/22 FAIL. Focused contract 3/3 and Meteora suites 19/19 PASS (12 contract/quote and 7 execution); 79 response schemas, eleven quote references, eleven instruction references, 28 dialect keywords, 29 relationship kinds, registry digest `e70fbbaf3f2b8407ca1ee6eeb6ac4c168dc42da355af7af6064d7cc95ee9c032`, and contract digest `e8063f569cafcc1f83c89b6ae2e5c458f72c4ef7d581d163653c53053611cc76` are structurally coherent; full suite 441/441 PASS; syntax 86/86 PASS; replay invariants PASS at 6,173.91 blocks/s with 9,642,608-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| HTTP response schema registry structure | `PASS` | The registry publishes 79 schemas with stable identities and canonical registry digest `e70fbbaf3f2b8407ca1ee6eeb6ac4c168dc42da355af7af6064d7cc95ee9c032`; the full contract digest is `e8063f569cafcc1f83c89b6ae2e5c458f72c4ef7d581d163653c53053611cc76`. Independent recomputation matches. Recursive enumeration confirms all 28 live schema-node keywords and all 29 used relationship kinds are declared with no unused entries. The discovery endpoint now references its own closed bootstrap schema; unknown future keywords, relationship kinds, or top-level bootstrap fields remain fail closed. |
...
- Validation results: real and preserved-digest controls PASS; execution-time reproduction, required evidence labeling, real-producer verification proof, complete dialect declaration, and bootstrap schema PASS; schema-only recomputed coupled mutation FAIL because it is admitted. Focused contract 3/3 and Meteora suites 19/19 PASS; full 441/441 PASS; syntax 86/86 PASS; replay PASS at 6,173.91 blocks/s with 9,642,608-byte heap growth.
...
- Validation results: prior closed boundaries, Meteora execution reproduction, instruction-evidence labeling, real-producer verification proof, complete dialect declaration, and bootstrap schema PASS; strict protocol-specific schema closure remains 21/22 PASS and 1/22 FAIL (ten quote plus all eleven instruction variants PASS; Meteora finalized-content discovery parity fails). Focused contract 3/3 and Meteora suites 19/19 PASS; full suite 441/441 PASS; syntax 86/86 PASS; replay PASS at 6,173.91 blocks/s with 9,642,608-byte heap growth.
