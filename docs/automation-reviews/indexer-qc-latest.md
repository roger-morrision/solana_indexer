...
- Run: `2026-08-29T18:36:38+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `8e785e39e5509ee62b1e37c1f2ccf202cd6ad0ca`
- Compared with QA baseline: `8934c2df778db86a625cd84d2ed4587a8ffd3e8f` (2 DEV commits, 3 changed files)
- Compared with `origin/main`: 78 ahead, 0 behind before this evidence report
- Latest DEV commits: `8ccb1c7` (remaining query-value profile closure) and `8e785e3` (WebSocket discovery schema closure)
- Overall result: 2 PASS, 0 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. All ten query-value profiles and the complete WebSocket discovery object are now semantically closed. The existing schema-only Meteora authenticity finding remains 21/22 strict outcomes because `binArrayPayloadHash` still lacks independent finalized anchoring. Independent reconciliation remains 68 PASS, 1 FAIL, and 1 BLOCKED across 70 domains; live qualification is blocked by absent fresh canonical evidence.
...
|---|---|---|---|
| `UPSTREAM-QUERY-CONTRACTS-ENUMERIC-VALUE-PROFILES-SCHEMA-024` | `/api/v1/query-contracts` / `valueConstraints.interval,limit,side,status,window` | `PASS` | The five exact profiles publish ordered enum values, integer syntax/bounds, defaults, and leading-zero policy. Independent schema probes accept 5/5 canonical profiles and reject 82/82 missing, substituted, wrong-type, reordered/duplicate/short/long enum, unknown-field, and wrong-shape mutations; runtime parity accepts 22/22 valid values and rejects 29/29 invalid values. |
| `UPSTREAM-QUERY-CONTRACTS-WEBSOCKET-SCHEMA-025` | `/api/v1/query-contracts` / `webSocket` | `PASS` | The discovery schema binds `/ws`, seven ordered parameters, four ordered topics and topic/filter maps, closed filter constraints, and ordered acknowledgement values. Independent schema probes accept the canonical object and reject 20/20 missing, unknown, reordered, expanded-filter, widened, unsafe-control, and wrong-shape mutations; parser parity accepts 12/12 canonical subscriptions and rejects 16/16 invalid subscriptions. |

- Available DEV delta: exactly two distinct committed outcomes exist after `8934c2d`, in `8ccb1c7` and `8e785e3`. QC detected the DEV writer lock and source movement during validation, discarded every mixed-state result, waited for lock release, refreshed the complete stable two-commit delta, and reran every tier. No additional distinct DEV outcome exists.
- Verification result: 2 PASS, 0 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct outcomes, and splitting profiles, fields, enum values, topic mappings, runtime cases, or mutation assertions would be padding.
- Validation: profile schema 5/5 positive and 82/82 negative, profile runtime 22/22 positive and 29/29 negative, WebSocket schema 1/1 positive and 20/20 negative, and WebSocket runtime 12/12 positive and 16/16 negative PASS. Strict protocol-specific schema closure remains 21/22 PASS and 1/22 FAIL; the recomputed coupled Meteora mutation remains schema-valid because its payload hash is unanchored. Focused contract 4/4 and Meteora suites 19/19 PASS (12 contract/quote and 7 execution); 79 response schemas, eleven quote references, eleven instruction references, 28 dialect keywords, 29 relationship kinds, registry digest `72a8bcfcf152670196aea04b368f48cc76b6ee41a1430bbcb899e71d33e7d460`, and contract digest `5c7bf2ffa228ce3cb90337846dbc35ec4b5208a0322047626b53240b0905414f` are structurally coherent; full suite 441/441 PASS; syntax 86/86 PASS; replay invariants PASS at 5,859.76 blocks/s with 9,653,992-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| HTTP response schema registry structure | `PASS` | The registry publishes 79 schemas with stable identities and canonical registry digest `72a8bcfcf152670196aea04b368f48cc76b6ee41a1430bbcb899e71d33e7d460`; the full contract digest is `5c7bf2ffa228ce3cb90337846dbc35ec4b5208a0322047626b53240b0905414f`. Independent recomputation matches. Recursive enumeration confirms all 28 live schema-node keywords and all 29 used relationship kinds are declared with no unused entries. All ten query-value profiles and the complete WebSocket bootstrap object are now closed in addition to the prior fail-closed boundaries. |
...
| Machine-readable query discovery | `PASS` | The WebSocket object now has an exact closed schema for its path, ordered parameter/topic catalogs, four topic/filter mappings, filter constraints, and acknowledgement values. Independent schema probes reject 20/20 structural mutations and parser parity accepts 12/12 canonical subscriptions while rejecting 16/16 invalid subscriptions. |
...
- Validation results: real and preserved-digest controls PASS; execution-time reproduction, required evidence labeling, real-producer verification proof, complete dialect declaration, bootstrap schema, nested dialect closure, admission-order closure, admission-outcome closure, canonicalization closure, path-profile closure, all ten value-profile closures, and WebSocket bootstrap closure PASS; schema-only recomputed coupled mutation FAIL because it is admitted. Focused contract 4/4 and Meteora suites 19/19 PASS; full 441/441 PASS; syntax 86/86 PASS; replay PASS at 5,859.76 blocks/s with 9,653,992-byte heap growth.
...
- Validation results: prior closed boundaries, Meteora execution reproduction, instruction-evidence labeling, real-producer verification proof, complete dialect declaration, bootstrap schema, nested dialect closure, admission-order closure, admission-outcome closure, canonicalization closure, path-profile closure, all ten value-profile closures, and WebSocket bootstrap closure PASS; strict protocol-specific schema closure remains 21/22 PASS and 1/22 FAIL (ten quote plus all eleven instruction variants PASS; Meteora finalized-content discovery parity fails). Focused contract 4/4 and Meteora suites 19/19 PASS; full suite 441/441 PASS; syntax 86/86 PASS; replay PASS at 5,859.76 blocks/s with 9,653,992-byte heap growth.
