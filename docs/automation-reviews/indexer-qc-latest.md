...
- Run: `2026-08-29T14:37:04+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `4d7454f0d497d97937d50eb6b5f1be9d7de9eff7`
- Compared with QA baseline: `a30cad9b4a9d0f4fb773f0060f828e5da91657f5` (2 DEV commits, 3 changed files)
- Compared with `origin/main`: 66 ahead, 0 behind before this evidence report
- Latest DEV commits: `18a1bc5` (query-contract dialect-envelope closure) and `4d7454f` (query-contract HTTP-admission closure)
- Overall result: 2 PASS, 0 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. The query-contract bootstrap schema now closes its nested dialect envelope and the exact ordered 14-stage HTTP-admission sequence. The existing schema-only Meteora authenticity finding remains 21/22 strict outcomes because `binArrayPayloadHash` still lacks independent finalized anchoring. Independent reconciliation remains 68 PASS, 1 FAIL, and 1 BLOCKED across 70 domains; live qualification is blocked by absent fresh canonical evidence.
...
|---|---|---|---|
| `UPSTREAM-QUERY-CONTRACTS-DIALECT-SCHEMA-016` | `/api/v1/query-contracts` / `bodySchemaDialect` | `PASS` | The real nested dialect validates with exact name, version 1, `fail_closed`, 28 unique nonempty keywords, and 29 unique nonempty relationship kinds. Wrong name/version/policy, every missing field, an unknown field, empty/duplicate/non-string keyword lists, and empty/duplicate/non-string relationship lists all reject. |
| `UPSTREAM-QUERY-CONTRACTS-ADMISSION-SCHEMA-017` | `/api/v1/query-contracts` / `httpAdmission` | `PASS` | The real nested admission object validates with exactly `order` and `outcomes`; the order matches all 14 unique stages exactly. Missing order/outcomes, an unknown field, adjacent/reverse reorder, short/long order, duplicate/unknown stage, and array/scalar outcomes all reject. |

- Available DEV delta: exactly two distinct committed outcomes exist after `a30cad9`, in `18a1bc5` and `4d7454f`. QC detected source movement under the active DEV writer lock, discarded mixed-state checks, waited for release, refreshed the complete stable delta, and reran every tier. No additional distinct DEV outcome exists.
- Verification result: 2 PASS, 0 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct outcomes, and splitting dialect fields, vocabulary arrays, admission stages, nested fields, or individual mutation assertions would be padding.
- Validation: the real nested dialect and admission objects plus exact constants, types, required fields, closure, array cardinality, item types, uniqueness, allowed values, and admission order controls PASS. Strict protocol-specific schema closure remains 21/22 PASS and 1/22 FAIL. Focused contract 3/3 and Meteora suites 19/19 PASS (12 contract/quote and 7 execution); 79 response schemas, eleven quote references, eleven instruction references, 28 dialect keywords, 29 relationship kinds, registry digest `6e11e8e2c045f6fe71b7f156d52c61c56657480faa3bcab8c67e730a698d5c35`, and contract digest `064a4872279e15e82bd701053b60d564adbe43f7ff64079bc71d500d883616f0` are structurally coherent; full suite 441/441 PASS; syntax 86/86 PASS; replay invariants PASS at 6,619.32 blocks/s with 9,636,112-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| HTTP response schema registry structure | `PASS` | The registry publishes 79 schemas with stable identities and canonical registry digest `6e11e8e2c045f6fe71b7f156d52c61c56657480faa3bcab8c67e730a698d5c35`; the full contract digest is `064a4872279e15e82bd701053b60d564adbe43f7ff64079bc71d500d883616f0`. Independent recomputation matches. Recursive enumeration confirms all 28 live schema-node keywords and all 29 used relationship kinds are declared with no unused entries. The discovery endpoint references its own closed bootstrap schema, whose dialect and admission-order projections are closed; unknown vocabulary and altered admission stages remain fail closed. |
...
- Validation results: real and preserved-digest controls PASS; execution-time reproduction, required evidence labeling, real-producer verification proof, complete dialect declaration, bootstrap schema, nested dialect closure, and admission-order closure PASS; schema-only recomputed coupled mutation FAIL because it is admitted. Focused contract 3/3 and Meteora suites 19/19 PASS; full 441/441 PASS; syntax 86/86 PASS; replay PASS at 6,619.32 blocks/s with 9,636,112-byte heap growth.
...
- Validation results: prior closed boundaries, Meteora execution reproduction, instruction-evidence labeling, real-producer verification proof, complete dialect declaration, bootstrap schema, nested dialect closure, and admission-order closure PASS; strict protocol-specific schema closure remains 21/22 PASS and 1/22 FAIL (ten quote plus all eleven instruction variants PASS; Meteora finalized-content discovery parity fails). Focused contract 3/3 and Meteora suites 19/19 PASS; full suite 441/441 PASS; syntax 86/86 PASS; replay PASS at 6,619.32 blocks/s with 9,636,112-byte heap growth.
