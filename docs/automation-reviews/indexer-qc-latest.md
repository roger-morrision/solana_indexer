...
- Run: `2026-08-28T22:35:49+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `45da8f6a5c0d222d5d3f04127cd57e8d496eedb1`
- Compared with QA baseline: `a2f167c50f49bb3658b99fefa99b35b6d7708ca5` (2 DEV commits, 3 changed files)
- Compared with `origin/main`: 19 ahead, 0 behind before this evidence report
- Latest DEV commits: `ecd46e0` (Meteora relationship semantics) and `45da8f6` (Phoenix IOC instruction-evidence schema)
- Overall result: 0 PASS, 2 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. The Meteora repair rejects the four prior contradictions but is still not constructor-equivalent in two independently reproduced directions. The new Phoenix schema is closed and ordered but admits three constructor-impossible lot/output states. The parent HIGH finding remains open. Live qualification is independently blocked by absent fresh canonical evidence.

## Reviewed DEV delta (2/20)

### Instruction-evidence schema constructor parity (0 PASS, 2 FAIL)

| Item | Route | Status | Independent evidence |
|---|---|---|---|
| `UPSTREAM-METEORA-DLMM-INSTRUCTION-EVIDENCE-SCHEMA-001` | pool `prepare-swap` / Meteora catalog row | `FAIL` | `ecd46e0` rejects all four prior bitmap/hook/version/uniqueness contradictions. Independent real-constructor parity still fails: the schema rejects a valid legacy SPL constructor output when optional finalized route data is supplied, while it admits nonzero legacy transfer fees that the constructor rejects. |
| `UPSTREAM-PHOENIX-IOC-INSTRUCTION-EVIDENCE-SCHEMA-001` | pool `prepare-swap` / Phoenix catalog row | `FAIL` | Real bid and ask constructor controls validate, keys close, slots order, and advertised side-zero fields bind. Independent mutations show generated validation admits a bid with zero required quote/base-minimum lots, an ask with zero minimum quote lots, and `minimumOutputRaw > quotedOutputRaw`; all three are constructor-impossible. |

- Available DEV delta: `ecd46e0` repairs the prior Meteora finding and `45da8f6` adds one distinct BA/PO-selected Phoenix enhancement after `a2f167c`. The complete delta was exhausted. A second DEV writer turn began during validation; QC discarded the stale `ecd46e0` tier results, waited for lock release and clean commit, refreshed to stable `45da8f6`, and reran every tier. No additional DEV commit or enhancement exists.
- Verification result: 0 PASS, 2 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct protocol-schema outcomes, and splitting individual relationship descriptors, contradiction probes, properties, or assertions would be padding.
- Validation: independent real-constructor harnesses accept Meteora legacy plus Phoenix bid/ask controls, confirm all four prior Meteora contradictions now reject, then reproduce two remaining Meteora parity failures and three Phoenix parity failures. Strict protocol-specific closure remains 8/22 PASS and 14/22 FAIL (four quote and four instruction-evidence variants PASS; Meteora and Phoenix instruction schemas are referenced but incomplete; seven quote and five other instruction variants remain null). Focused committed protocol suite 9/9 PASS; 67 response schemas, 16 declared relationship kinds, and digest `358ce7033f9415d9f1e404130798f96f0f609437817ac678c8547752849658e4` are structurally coherent; full suite 427/427 PASS; syntax 86/86 PASS; replay invariants PASS at 5,058.48 blocks/s with 9,754,352-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| HTTP response schema registry structure | `PASS` | The registry publishes 67 schemas with stable identities and digest `358ce7033f9415d9f1e404130798f96f0f609437817ac678c8547752849658e4`. Independent recomputation matches. Recursive enumeration confirms every live schema-node keyword and all sixteen relationship kinds are declared, including the Meteora relationship descriptors and Phoenix conditional values, while unknown future keywords remain fail closed. |
...
| Swap-preparation success schema | `FAIL` | Top-level, handoff/binding, preparation/transaction, simulation-policy, universal amount-effect, and catalog identity boundaries are closed. Four instruction-evidence variants plus CPMM, CLMM, Orca, and AMM v4 quotes have constructor parity. Meteora and Phoenix instruction schemas are referenced but still lack constructor cross-field parity; seven quote variants and five other instruction-evidence variants remain schema-null. |
...
- Severity: `FAIL` / `HIGH`
- Owner: `DEV`
- Reproduction: run the independent Meteora parity harness against `ecd46e0` or later. First construct a valid legacy SPL swap while supplying optional finalized transfer-hook route data; the real builder returns evidence with one hook row at slot 104, but the published legacy-empty relationship rejects it. Then mutate the constructor control to nonzero `inputTransferFeeRaw` and `outputTransferFeeRaw`; the published relationships accept it although the real builder rejects nonzero fees when neither mint uses Token-2022.
- Evidence: `ecd46e0` adds nullable pairing, hook-row slot bounds, legacy emptiness/nullability, and unique slice-type relationships; all four prior contradictions now reject. Independent construction nevertheless proves both remaining directions: one real constructor output is rejected and one constructor-impossible fee tuple is admitted. This is semantic drift between `buildMeteoraDlmmSwapInstruction` and generated validators, not a closure or scalar-bound defect.
...
- Expected versus actual behavior: an exact instruction-evidence schema must accept every real constructor output and reject every tuple the constructor cannot emit. Actual discovery rejects optional finalized route evidence that the legacy builder preserves and accepts impossible nonzero legacy fees.
- Acceptance criteria: decide and bind whether optional finalized route data is part of legacy evidence; if accepted by the constructor, admit its exact bounded summary, otherwise reject it before evidence construction. Bind legacy SPL `inputTransferFeeRaw` and `outputTransferFeeRaw` to zero while preserving real Token-2022 fee/hook cases. Retain the four repaired relationships and add one isolated real-constructor control/negative for each remaining direction.
- Validation results: all four prior contradictions reject PASS; legacy constructor control without route data PASS; real constructor with optional finalized route data rejected FAIL; impossible nonzero legacy fees admitted FAIL; focused Meteora/Phoenix suite 9/9 PASS; full suite 427/427 PASS; syntax 86/86 PASS; replay PASS at 5,058.48 blocks/s with 9,754,352-byte heap growth.
...
- Severity: `FAIL` / `HIGH`
- Owner: `DEV`
- Reproduction: resolve the Phoenix catalog row's `instructionEvidenceSchema`; construct real bid and ask IOC evidence through `buildPhoenixImmediateOrCancelInstruction`; then independently mutate only (1) bid `numQuoteLots` and `minBaseLotsToFill` to `"0"`, (2) ask `minQuoteLotsToFill` to `"0"`, or (3) bid `minimumOutputRaw` above `quotedOutputRaw`.
- Evidence: `45da8f6` closes all fourteen fields, binds `stateSlot <= balanceSlot <= quoteCurrentSlot < lastValidSlot`, restricts bid/ask vocabulary, requires positive amount/output/price/base-lot strings, and binds the three advertised side-specific zero fields. Both real constructor controls pass. All three isolated constructor-impossible mutations are admitted because the opposite side-required lot values and minimum-versus-quoted output relation are not represented.
...
- Expected versus actual behavior: an exact Phoenix instruction-evidence schema must accept every real bid/ask constructor output and reject any side/lot/output tuple the constructor cannot emit. Actual discovery admits zero required bid lots, zero required ask minimum quote lots, and a minimum output above the quoted output.
- Acceptance criteria: require positive `numQuoteLots` and `minBaseLotsToFill` for bid; require positive `minQuoteLotsToFill` for ask; require `minimumOutputRaw <= quotedOutputRaw`; retain ask `numQuoteLots=0`, ask `minBaseLotsToFill=0`, bid `minQuoteLotsToFill=0`, exact key closure, slot ordering, and real bid/ask controls; add isolated negatives for all three contradictions.
- Validation results: real bid and ask constructor controls 2/2 PASS; exact keys, side vocabulary, slot/expiry ordering, and advertised zero conditions PASS; constructor-impossible mutations admitted 3/3 FAIL; focused Meteora/Phoenix suite 9/9 PASS; full suite 427/427 PASS; syntax 86/86 PASS; replay PASS at 5,058.48 blocks/s with 9,754,352-byte heap growth.
- Compatibility/performance impact: discovery-only semantic hardening changes digest/ETag and generated validators while preserving runtime bodies, persistence, providers, RPC/WebSocket, database, and configuration. The relationships are constant-time scalar comparisons.
- Blockers: none; the defect is deterministic and offline reproducible.
...
- Evidence: prior increments close handoff, transaction, policy, universal amounts, all three input/output effect bindings, catalog discovery, and type/protocol membership. Four instruction-evidence schemas plus CPMM, CLMM, Orca, and AMM v4 quotes match their constructors. Meteora and Phoenix instruction schemas are referenced but retain independently reproduced constructor-parity failures; seven quote and five other instruction-evidence variants remain schema-null.
...
- Actual behavior: external authority, cataloged preparation identity, preparation, transaction, shared simulation policy, universal raw amounts, all three signer-to-effect relationships, four instruction-evidence variants, and four quote variants are verifiable. Meteora and Phoenix are structurally referenced but semantically incomplete; seven quotes and five other instruction-evidence variants remain schema-null. Generated validators still cannot establish complete protocol evidence for fourteen variants.
...
- Validation results: prior closed boundaries remain PASS; strict protocol-specific schema coverage remains 8/22 PASS and 14/22 FAIL (four quote plus four instruction variants PASS; Meteora and Phoenix instruction evidence incomplete; seven quotes and five other instruction variants null). Independent constructor harnesses reproduce two Meteora and three Phoenix parity failures. Focused suite 9/9 PASS; full suite 427/427 PASS; syntax 86/86 PASS; replay PASS at 5,058.48 blocks/s with 9,754,352-byte heap growth.
...
- NEXT_DEV_ACTION: repair Meteora legacy route-data/fee parity and Phoenix side-required-lot/minimum-output parity with isolated real-constructor controls and negatives before selecting another schema.
