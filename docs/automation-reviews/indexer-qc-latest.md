...
- Run: `2026-08-29T15:36:35+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `1b9bdf8f369bea11d8ac5e34c9d1194759e25b2d`
- Compared with QA baseline: `f0ea5d5d23ce8ced224f89d7bcee04a942bd5902` (2 DEV commits, 3 changed files)
- Compared with `origin/main`: 69 ahead, 0 behind before this evidence report
- Latest DEV commits: `e5ab253` (query-contract admission-outcome closure) and `1b9bdf8` (query-contract canonicalization closure)
- Overall result: 2 PASS, 0 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. The query-contract bootstrap schema now closes all 13 pre-route admission outcomes and its exact request-canonicalization policy. The existing schema-only Meteora authenticity finding remains 21/22 strict outcomes because `binArrayPayloadHash` still lacks independent finalized anchoring. Independent reconciliation remains 68 PASS, 1 FAIL, and 1 BLOCKED across 70 domains; live qualification is blocked by absent fresh canonical evidence.
...
|---|---|---|---|
| `UPSTREAM-QUERY-CONTRACTS-ADMISSION-OUTCOMES-SCHEMA-018` | `/api/v1/query-contracts` / `httpAdmission.outcomes` | `PASS` | The real exact 13-gate catalog validates. Exhaustive independent probes reject every omitted gate, unknown gate/field, flipped retryability, omitted required field, wrong status/type, reordered/duplicate/non-integer status list, false `Allow`, invalid `Retry-After`, and null/array/scalar catalog: 104/104 negatives reject. |
| `UPSTREAM-QUERY-CONTRACTS-CANONICALIZATION-SCHEMA-019` | `/api/v1/query-contracts` / `canonicalization` | `PASS` | The real four-field policy validates with exact `url-search-params-sort-v1` and three mandatory true controls. Every omitted field, substituted value, wrong type, unknown extension, and null/array/scalar object rejects: 16/16 negatives reject. |

- Available DEV delta: exactly two distinct committed outcomes exist after `f0ea5d5`, in `e5ab253` and `1b9bdf8`. QC detected source movement after the first validation pass, discarded that stale evidence, refreshed the complete stable delta, and reran every tier. No additional distinct DEV outcome exists.
- Verification result: 2 PASS, 0 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct outcomes, and splitting gates, status codes, retry/header fields, canonicalization booleans, types, or individual mutation assertions would be padding.
- Validation: two real nested objects and all 120 independent negative mutations PASS. Strict protocol-specific schema closure remains 21/22 PASS and 1/22 FAIL; the recomputed coupled Meteora mutation remains schema-valid because its payload hash is unanchored. Focused contract 3/3 and Meteora suites 19/19 PASS (12 contract/quote and 7 execution); 79 response schemas, eleven quote references, eleven instruction references, 28 dialect keywords, 29 relationship kinds, registry digest `8aaba8d1459235a72a53de7eae4f01fc5d0e4d44c7f8fb9abe333d28164a45b3`, and contract digest `e7d502a167537443274511085b081e0374b55b41c1572cb606d060f159a68a13` are structurally coherent; full suite 441/441 PASS; syntax 86/86 PASS; replay invariants PASS at 6,900.66 blocks/s with 9,810,312-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| HTTP response schema registry structure | `PASS` | The registry publishes 79 schemas with stable identities and canonical registry digest `8aaba8d1459235a72a53de7eae4f01fc5d0e4d44c7f8fb9abe333d28164a45b3`; the full contract digest is `e7d502a167537443274511085b081e0374b55b41c1572cb606d060f159a68a13`. Independent recomputation matches. Recursive enumeration confirms all 28 live schema-node keywords and all 29 used relationship kinds are declared with no unused entries. The discovery endpoint references its own closed bootstrap schema; its dialect, admission order, 13 admission outcomes, and request-canonicalization policy are closed and fail safe under mutation. |
