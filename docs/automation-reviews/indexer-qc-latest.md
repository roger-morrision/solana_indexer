...
- Run: `2026-08-28T21:36:48+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `90eb60fa3c426df3f12f07cb22b202aa94a55ab3`
- Compared with QA baseline: `52a3a04cacd7885de67d6a5f80a32abc1ffe84cc` (2 DEV commits, 3 changed files)
- Compared with `origin/main`: 16 ahead, 0 behind before this evidence report
- Latest DEV commits: `2e3aae3` (quote constructor semantics) and `90eb60f` (Meteora instruction-evidence schema)
- Overall result: 2 PASS, 1 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. The Orca and AMM v4 repairs now reject every prior constructor contradiction. The new Meteora schema closes keys and scalar/slot bounds but admits four constructor-impossible cross-field states involving bitmap identity, finalized hook rows, legacy-swap hook slices, and duplicate slice types. The parent HIGH finding remains open. Live qualification is independently blocked by absent fresh canonical evidence.

## Reviewed DEV delta (3/20)

### Constructor and instruction-evidence schema parity (2 PASS, 1 FAIL)

| Item | Route | Status | Independent evidence |
|---|---|---|---|
| `UPSTREAM-QA-ORCA-WHIRLPOOL-QUOTE-SCHEMA-001` | pool quote / Orca catalog row | `PASS` | The real control validates. Independent generated validation rejects the three prior isolated contradictions: `limitTick:-443637`, unrelated `sqrtPriceLimitX64:"1"`, and `feeRateMillionths:"1000000"`. The published bounded tick and exact Q64.64 relationship match the real constructor. |
| `UPSTREAM-QA-RAYDIUM-AMM-V4-QUOTE-SCHEMA-001` | pool quote / AMM v4 catalog row | `PASS` | The real `1813` output control validates for input `1000`, fee `3`, and reserves `10000/20000`. Independent generated validation rejects both `amountOutRaw == outputReserveRaw` and isolated equation mismatch `1812`, matching integer constant-product construction. |
| `UPSTREAM-METEORA-DLMM-INSTRUCTION-EVIDENCE-SCHEMA-001` | pool `prepare-swap` / Meteora catalog row | `FAIL` | Exact keys, nested hook-summary closure, version vocabulary, scalar bounds, and four direct slot inversions behave as advertised. However, generated validation admits 4/4 constructor-impossible states: bitmap address with null bitmap slot, a hook row older than mint evidence, legacy `swap` with transfer-hook slices, and two distinct `transferHookX` slice rows. |

- Available DEV delta: `2e3aae3` repairs two prior findings and `90eb60f` adds one distinct BA/PO-selected Meteora enhancement after `52a3a04`. The complete delta was exhausted. A new DEV writer turn began during this QA run; mixed-state results were discarded, QC waited for lock release and commit, refreshed from `2e3aae3` to clean stable `90eb60f`, and reran every tier. No additional DEV commit or enhancement exists.
- Verification result: 2 PASS, 1 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 17; the stable delta contains exactly three distinct protocol-schema outcomes, and splitting individual relationship descriptors, contradiction probes, properties, or assertions would be padding.
- Validation: the independent fail-closed generated-validator harness passes 13/13 probes: seven Orca/AMM controls and negatives plus the Meteora control, closure negative, and four admitted constructor contradictions. Strict protocol-specific closure advances to 8/22 PASS and 14/22 FAIL (four quote and four instruction-evidence variants PASS; the new Meteora instruction schema is referenced but incomplete; seven quote variants and six other instruction variants remain null). Focused committed/constructor suite 34/34 PASS; 66 response schemas, 12 declared relationship kinds, and digest `0ff1a515795c9323322923dfcc8468f36c3a3f431b17f0e4851819659b0dd294` are structurally coherent; full suite 426/426 PASS; syntax 86/86 PASS; replay invariants PASS at 6,694.68 blocks/s with 9,427,152-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| HTTP response schema registry structure | `PASS` | The registry publishes 66 schemas with stable identities and digest `0ff1a515795c9323322923dfcc8468f36c3a3f431b17f0e4851819659b0dd294`. Independent recomputation matches. Recursive enumeration confirms every live schema-node keyword and all twelve relationship kinds are declared, including the new exact constant-product and Orca tick-price kinds, while unknown future keywords remain fail closed. |
...
| Swap-preparation success schema | `FAIL` | Top-level, handoff/binding, preparation/transaction, simulation-policy, universal amount-effect, and catalog identity boundaries are closed. Four instruction-evidence variants plus CPMM, CLMM, Orca, and AMM v4 quotes have constructor parity. The new Meteora instruction-evidence schema is referenced but lacks constructor cross-field parity; seven quote variants and six other instruction-evidence variants remain schema-null. |
...
- Severity: `FAIL` / `HIGH`
- Owner: `DEV`
- Reproduction: resolve the Meteora catalog row's `instructionEvidenceSchema`; validate the closed `swap2` control; then independently (1) set `bitmapExtensionSlot:null` while retaining `optionalBitmapExtension`, (2) set a transfer-hook summary row slot below `mintEvidenceSlot`, (3) change only `instructionVersion` to legacy `swap` while retaining hook data and slices, or (4) provide distinct `transferHookX` slice rows with different lengths.
- Evidence: `90eb60f` publishes exact top-level/nested keys, scalar bounds, version and slice vocabularies, unique bounded arrays, and component-slot ordering. The control and closure negative behave as advertised, and all four direct slot inversions reject. Yet independent generated validation admits all 4/4 cross-field contradictions. The real constructor emits bitmap address and slot together, requires every finalized hook row at or after mint evidence, selects `swap` only without Token-2022 hooks, and creates at most one X plus one Y slice.
...
- Expected versus actual behavior: an exact instruction-evidence schema must reject any evidence tuple that `buildMeteoraDlmmSwapInstruction` cannot emit. Actual discovery accepts four constructor-impossible combinations.
- Acceptance criteria: bind `bitmapExtensionSlot` and `optionalBitmapExtension` nullability; require nonempty hook summaries to have a nonnull maximum source slot and every row slot at or after `mintEvidenceSlot`; bind legacy `swap` to empty hook summaries/slices and null hook-data slot; allow at most one `transferHookX` and one `transferHookY` slice; preserve real legacy, fee-only Token-2022, hook, account-data, and bitmap controls; add isolated negatives for every relationship.
- Validation results: generated control 1/1 PASS; exact top-level/nested closure and version vocabulary PASS; direct slot inversions reject 4/4 PASS; constructor contradictions admitted 4/4 FAIL; committed Meteora schema/constructor focus PASS within 34/34; full suite 426/426 PASS; syntax 86/86 PASS; replay PASS at 6,694.68 blocks/s with 9,427,152-byte heap growth.
...
- Evidence: prior increments close handoff, transaction, policy, universal amounts, all three input/output effect bindings, catalog discovery, and type/protocol membership. Four instruction-evidence schemas plus CPMM, CLMM, Orca, and AMM v4 quotes match their constructors. The new Meteora instruction schema is referenced but admits four independently reproduced constructor contradictions; seven quote and six other instruction-evidence variants remain schema-null.
...
- Actual behavior: external authority, cataloged preparation identity, preparation, transaction, shared simulation policy, universal raw amounts, all three signer-to-effect relationships, four instruction-evidence variants, and four quote variants are verifiable. Meteora is structurally referenced but semantically incomplete; seven quotes and six other instruction-evidence variants remain schema-null. Generated validators still cannot establish complete protocol evidence for fourteen variants.
...
- Validation results: prior closed boundaries remain PASS; strict protocol-specific schema coverage advances to 8/22 PASS and 14/22 FAIL (four quote plus four instruction variants PASS; Meteora instruction evidence incomplete; seven quotes and six other instruction variants null). The independent generated-validator harness rejects every prior Orca/AMM contradiction but admits four Meteora constructor contradictions. Focused suite 34/34 PASS; full suite 426/426 PASS; syntax 86/86 PASS; replay PASS at 6,694.68 blocks/s with 9,427,152-byte heap growth.
...
- NEXT_DEV_ACTION: bind Meteora bitmap, finalized hook-row, instruction-version, and unique slice-type cross-field semantics with isolated real-constructor negatives before selecting another schema.
