...
- Run: `2026-08-29T06:35:57+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `3e0dcdb920cefc0dac607638d127b2ae236c4ed1`
- Compared with QA baseline: `fe13fce8cd12a08ba8b253088e8f5b01b4ccbcc8` (2 DEV commits, 4 changed files)
- Compared with `origin/main`: 43 ahead, 0 behind before this evidence report
- Latest DEV commits: `7d8fb92` (OpenBook zero-output rejection) and `3e0dcdb` (Meteora DLMM quote-schema publication)
- Overall result: 1 PASS, 1 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. `7d8fb92` closes the OpenBook producer/schema boundary across the exact zero-output fee interval while retaining positive adjacent quotes and prior contradiction rejection. `3e0dcdb` publishes the eleventh quote schema and accepts real legacy, partial, and Token-2022 Meteora bodies, but it admits seven isolated producer-impossible fee, mode, rate, and bin-path mutations. The parent HIGH finding advances to 21/22 strict outcomes: all eleven instruction schemas and ten quote schemas have constructor parity, while Meteora remains semantically underbound. Independent reconciliation remains 68 PASS, 1 FAIL, and 1 BLOCKED across 70 domains; live qualification is blocked by absent fresh canonical evidence.
...
|---|---|---|---|
| `UPSTREAM-OPENBOOK-V2-QUOTE-SCHEMA-001` | pool `prepare-swap` / OpenBook place-take-order quote | `PASS` | A sweep from `989990` through `1000000` millionths accepts all 11 positive-output cases through `990000` and rejects all 10,000 zero-output cases from `990001` onward. Ordinary bid/ask and the adjacent output-1 control remain schema-valid, and all six isolated lot/output/fee contradictions still reject. |
| `UPSTREAM-METEORA-DLMM-QUOTE-SCHEMA-001` | pool `prepare-swap` / Meteora DLMM quote | `FAIL` | Real legacy, partial, and Token-2022 producer bodies validate and discovery references all eleven quote schemas. Seven isolated constructor contradictions remain admitted: nonzero legacy input fee, altered trading fee, altered maximum rate, flipped `feeOnInput`, flipped `feeOnTokenX`, impossible end bin, and unrelated bin-array path. |

- Available DEV delta: exactly two distinct outcomes exist after `fe13fce`: OpenBook zero-output closure in `7d8fb92` and Meteora quote publication in `3e0dcdb`. QC detected the active DEV writer lock and source movement during validation, discarded mixed-state results, waited for lock release, refreshed the complete stable two-commit delta, and reran every tier. No additional DEV outcome exists.
- Verification result: 1 PASS, 1 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the stable delta contains exactly two distinct outcomes, and splitting fee points, fields, relationship descriptors, mutation probes, or quote directions would be padding.
- Validation: OpenBook boundary sweep 10,011/10,011 PASS, ordinary bid/ask plus adjacent output-1 controls 3/3 schema-valid, and isolated contradictions 6/6 reject. Meteora real legacy/partial/Token-2022 controls 3/3 validate, but seven producer-impossible mutations are admitted 7/7. Strict protocol-specific closure is 21/22 PASS and 1/22 FAIL (all eleven quote and eleven instruction schemas are referenced; only Meteora quote semantic parity fails). Focused committed suite 5/5 PASS; 78 response schemas, eleven quote references, eleven instruction references, 28 dialect keywords, 24 relationship kinds, and digest `f60cd1431c06d41a1e17450bd50891674981bee2c8e39258b3060d402dc29cdf` are structurally coherent; full suite 435/435 PASS; syntax 86/86 PASS; replay invariants PASS at 4,931.98 blocks/s with 9,543,776-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| HTTP response schema registry structure | `PASS` | The registry publishes 78 schemas with stable identities and digest `f60cd1431c06d41a1e17450bd50891674981bee2c8e39258b3060d402dc29cdf`. Independent recomputation matches. Recursive enumeration confirms all 28 live schema-node keywords and all 24 relationship kinds are declared, including bounded Pump constant-product, ceiling-fee, OpenBook lot-economics, conditional-pattern, and array-decimal-sum kinds, while unknown future keywords remain fail closed. |
...
| Swap-preparation success schema | `FAIL` | Top-level, handoff/binding, preparation/transaction, simulation-policy, universal amount-effect, catalog identity, and all eleven instruction-evidence variants are closed. All eleven quote schemas are now referenced; ten retain constructor parity. OpenBook now rejects every zero-output boundary, while Meteora admits seven isolated fee/mode/rate/bin contradictions. |
...
- Severity: `FAIL` / `HIGH`
- Owner: `DEV`
- Reproduction: generate real legacy, partial-liquidity, and Token-2022 transfer-fee quotes with `quoteMeteoraDlmmSnapshotExactInput`, validate them against `meteora_dlmm_quote_v1`, then alter one constructor-derived fee, mode, rate, end-bin, or bin-array field at a time without changing the rest of the body.
- Evidence: `3e0dcdb` publishes the final quote reference and a closed 30-field schema. Real legacy, partial, and Token-2022 bodies validate. Independent isolated mutations to legacy `inputTransferFeeRaw`, `tradingFeeRaw`, `maximumFeeRateNumerator`, `feeOnInput`, `feeOnTokenX`, `endBinId`, and `binArrayIndexes` also validate 7/7 even though the producer derives each from finalized mint evidence, collect-fee mode, bin traversal, and exact fee math.
...
- Expected versus actual behavior: the advertised schema must admit every real producer body and reject producer-impossible fee, mode, rate, and path combinations. Actual discovery admits the real bodies but does not bind seven constructor-derived directions.
- Acceptance criteria: retain exact closure, status/input/output conservation, u64/rate bounds, slot ordering, immutable execution gates, and all three real controls; bind legacy versus Token-2022 transfer-fee semantics, trading/protocol fee math, `feeOnInput`/`feeOnTokenX` to direction and collect-fee mode, and end-bin/bin-array path to the traversed evidence; reject each isolated contradiction.
- Validation results: real legacy/partial/Token-2022 controls 3/3 PASS; committed negatives 7/7 reject; fresh constructor-parity negatives 0/7 reject. Focused committed suite 5/5, full 435/435, syntax 86/86, and replay PASS, so the committed suite covers shape but not producer parity.
- Compatibility/performance impact: additive discovery changes digest/ETag and generated validators without changing runtime quote bytes. The required bounded relationships operate on a maximum 13,312-entry path already capped by the schema.
...
- Evidence: prior increments close handoff, transaction, policy, universal amounts, all three input/output effect bindings, catalog discovery, and type/protocol membership. All eleven instruction-evidence variants and all eleven quote schemas are now referenced. Ten quote schemas match their constructors; OpenBook's zero-output interval is closed, while Meteora admits seven producer-impossible fee/mode/rate/bin mutations.
...
- Actual behavior: external authority, cataloged preparation identity, preparation, transaction, shared simulation policy, universal raw amounts, all three signer-to-effect relationships, all eleven instruction-evidence variants, and ten quote variants are verifiable. Meteora is no longer null but remains semantically underbound, leaving one variant outcome unsafe.
...
- Validation results: prior closed boundaries remain PASS; strict protocol-specific closure advances to 21/22 PASS and 1/22 FAIL (ten quote plus all eleven instruction variants PASS; Meteora quote semantic parity fails). Focused suite 5/5 PASS; full suite 435/435 PASS; syntax 86/86 PASS; replay PASS at 4,931.98 blocks/s with 9,543,776-byte heap growth.
...
- NEXT_DEV_ACTION: bind Meteora's published quote schema to exact legacy/Token-2022 transfer-fee semantics, trading/protocol fee math, direction-dependent fee mode, and traversed end-bin/bin-array evidence, with the three real controls and seven isolated contradiction regressions retained.
