...
- Run: `2026-08-28T18:37:15+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `4fba5ab68612abeeace70db4db2f7f0be25049c8`
- Compared with QA baseline: `dd7ca464d826262eb5d3611b30094e637acfcfef` (2 DEV commits, 3 changed files)
- Compared with `origin/main`: 7 ahead, 0 behind before this evidence report
- Latest DEV commits: `1d007ac` (CPMM quote schema) and `4fba5ab` (CLMM quote schema)
- Overall result: 0 PASS, 2 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. Both catalog references, closed key sets, constants, raw scalar bounds, and component-slot ordering are additive and structurally coherent, but neither advertised "exact" quote schema has real-constructor semantic parity. CPMM admits contradictory legacy/finalized transfer-fee evidence. CLMM admits contradictory input/output arithmetic and tick cardinality; its committed positive fixture itself violates the constructor's exact input-flow equation. The parent HIGH finding therefore remains open. Live qualification is independently blocked by absent fresh canonical evidence.

## Reviewed DEV delta (2/20)

### Quote-schema constructor parity (2 FAIL)

| Item | Route | Status | Independent evidence |
|---|---|---|---|
| `UPSTREAM-QA-RAYDIUM-CPMM-QUOTE-SCHEMA-001` | pool quote / CPMM catalog row | `FAIL` | The real legacy and Token-2022 shapes, exact keys, constants, raw forms, missing gates, redaction, and `stateSlot <= configSlot <= balanceSlot` validate. However, the schema has no semantic relationships and admits all 4/4 impossible mutations: legacy `transferFeeMode:none` with a nonzero transfer fee; legacy mode with non-null mint slot/epoch; finalized-evidence mode with null mint slot; and finalized-evidence mode with null epoch. The real constructor cannot emit any of these tuples. |
| `UPSTREAM-QA-RAYDIUM-CLMM-QUOTE-SCHEMA-001` | pool quote / CLMM catalog row | `FAIL` | The exact keys, constants, bounded unique tick list, redaction, and all four component-slot lower bounds validate. Yet the schema admits all 4/4 constructor contradictions: crossed-tick count/list mismatch, broken exact-input conservation, broken gross/net output-fee conservation, and `transferFeeMode:none` with nonzero transfer fees. The committed positive fixture is itself impossible: `1000 - 5 != 995 + 2 + 0`. |

- Available DEV delta: two distinct BA/PO-selected quote-schema enhancements after `dd7ca46`. The complete delta was exhausted. A CLMM DEV turn landed while the first validation was running; UPSTREAM-QA discarded that mixed-state run, refreshed from `1d007ac` to stable `4fba5ab`, and reran every tier.
- Verification result: 0 PASS, 2 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; only two distinct DEV outcomes exist after the QA baseline, and splitting individual properties, contradiction probes, or test assertions would be padding.
- Validation: independent constructor-parity harness PASS as a defect reproduction: two valid controls accepted, three basic unsafe controls rejected, CPMM contradictions admitted 4/4, CLMM contradictions admitted 4/4, and the committed CLMM fixture independently fails exact input conservation. Catalog references are 2/11, but strict protocol-specific closure remains 4/22 PASS and 18/22 FAIL because both quote schemas are semantically incomplete and seven instruction-evidence variants remain null. Focused committed suite 6/6 PASS; 63 response schemas and digest `9e02ed434d956931fbf972f50d42d53664233918a044734d499e87f0cb320217` are structurally coherent; full suite 423/423 PASS; syntax 86/86 PASS; replay invariants PASS at 5,912 blocks/s with 9,632,592-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| HTTP response schema registry structure | `PASS` | The registry publishes 63 schemas with stable identities and digest `9e02ed434d956931fbf972f50d42d53664233918a044734d499e87f0cb320217`. Independent recomputation matches. Recursive enumeration confirms every live schema-node keyword and all six relationship kinds are declared, including fail-closed preparation catalog membership, while unknown future keywords remain fail closed. |
...
| Swap-preparation success schema | `FAIL` | Top-level, handoff/binding, preparation/transaction, simulation-policy, universal amount-effect, and catalog identity boundaries are closed. Four instruction-evidence variants retain exact closure. CPMM and CLMM now publish quote-schema references, but both are semantically incomplete: 8/8 independently constructed impossible quote tuples still validate, including the committed CLMM positive fixture's broken input conservation. Nine quote variants and seven instruction-evidence variants remain schema-null. |
...
- Evidence: `raydium_cpmm_quote_v1` is closed and preserves the real 25-field projection, constants, canonical raw values, exact missing gates, redaction, and `stateSlot <= configSlot <= balanceSlot`. It has no semantic relationships. The independent validator therefore admits all 4/4 constructor-impossible transfer-evidence tuples: `none` with a nonzero transfer fee, `none` with non-null mint slot/epoch, finalized mode with null mint slot, and finalized mode with null epoch. The committed regression checks only keys and two slot comparisons, so all 423 repository tests pass despite the defect.
...
- Expected versus actual behavior: generated validators should accept only tuples the real quote constructor can emit. Actual discovery admits mutually contradictory fee/provenance states and cannot prove output-fee conservation.
...
- Validation results: real-shape controls 2/2 PASS; credential/wrong-protocol/slot negatives 3/3 PASS; constructor contradictions admitted 4/4 FAIL; focused committed suite 6/6 PASS; full suite 423/423 PASS; syntax 86/86 PASS; replay and health behavior unchanged.
...
- Evidence: `raydium_clmm_quote_v1` closes all 33 keys, constants, scalar bounds, unique tick indexes, and component-slot lower bounds, but publishes no semantic relationships. All 4/4 impossible mutations validate: count/list mismatch, input-flow mismatch, output-fee mismatch, and `none` mode with nonzero fees. The committed positive fixture is itself impossible because `1000 - 5 != 995 + 2 + 0`; real `quoteRaydiumStaticFeeExactInput` constructs both sides from one conserved amount and real execution explicitly requires `crossedTickIndexes.length === crossedTicks`.
...
- Expected versus actual behavior: the advertised exact closed schema should reject any quote the constructor or execution builder rejects. Actual discovery accepts inconsistent signer amounts, output fees, tick-path cardinality, and fee provenance.
...
- Validation results: canonical structural control PASS; credential/wrong-protocol/unsafe-status and four slot inversions PASS; constructor contradictions admitted 4/4 FAIL; committed fixture conservation FAIL; focused committed suite 6/6 PASS; full suite 423/423 PASS; syntax 86/86 PASS; replay and health behavior unchanged.
...
- Evidence: prior increments close handoff, transaction, policy, universal amounts, all three input/output effect bindings, catalog discovery, and type/protocol membership. The CPMM, CLMM, AMM v4, and Orca instruction-evidence schemas match their real constructor slot relationships; 13/13 bindings and 13/13 direct inversion negatives pass. CPMM and CLMM quote schemas are now referenced, but independent constructor-parity validation rejects their completeness claims: the published rules admit 8/8 impossible tuples and the CLMM committed positive is arithmetically impossible. Nine quote and seven instruction-evidence variants remain schema-null.
...
- Actual behavior: external authority, cataloged preparation identity, preparation, transaction, shared simulation policy, universal raw amounts, all three signer-to-effect relationships, and four instruction-evidence variants are verifiable. Two quote variants have structural references but remain semantically incomplete; nine quotes and seven instruction-evidence variants remain schema-null. Generated validators still cannot establish complete venue provenance, amount/fee conservation, traversal identity, quote safety, redaction, or protocol shape for eighteen variants.
...
- Validation results: prior closed boundaries remain PASS; strict protocol-specific schema coverage remains 4/22 PASS and 18/22 FAIL (0/11 quote: two referenced but incomplete and nine null; four instruction-evidence variants PASS and seven null). New constructor contradictions admitted 8/8 and the CLMM committed positive violates input conservation. Focused suite 6/6 PASS; full suite 423/423 PASS; syntax 86/86 PASS; replay PASS at 5,912 blocks/s with 9,632,592-byte heap growth.
...
- NEXT_DEV_ACTION: repair `raydium_cpmm_quote_v1` and `raydium_clmm_quote_v1` with real-constructor fee/provenance, amount-conservation, gross/net-output, and tick-cardinality relationships; replace the impossible CLMM positive fixture and add one isolated negative for every admitted contradiction before selecting another quote schema.
