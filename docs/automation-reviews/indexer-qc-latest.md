...
- Run: `2026-09-01T00:36:29.201+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `898f1b75c74a7567ee8c9b972c74291dda2b574d`
- Compared with QA baseline: `36c33876cc024d1cfd017393eb0c8379beba823c` (2 DEV commits, 5 DEV-changed files)
- Compared with `origin/main`: 239 ahead, 0 behind before this evidence report
- Latest DEV commit: `898f1b7` (finalized Meteora quote verifier)
- Overall result: 0 PASS, 2 FAIL, 0 BLOCKED, and 0 SKIP across the complete two-outcome DEV delta. `UPSTREAM-METEORA-FINALIZED-BIN-IDENTITY-116` adds address/slot/hash discovery but admits non-Solana account names and describes a minimum context as the acquisition slot. `UPSTREAM-METEORA-FINALIZED-BIN-VERIFIER-117` decodes supplied finalized bytes and exactly reproduces the quote, but likewise accepts arbitrary nonempty account names and evidence fetched after the advertised source slot. Both outcomes reconcile to the existing `UPSTREAM-METEORA-DLMM-QUOTE-SCHEMA-001` domain, so reconciliation remains 153 PASS, 1 FAIL, and 1 BLOCKED across 155 deduplicated independent domains rather than padding two overlapping findings; live qualification also remains blocked.

## Reviewed DEV delta (2/20)

### Meteora finalized bin-array discovery and verification (2 FAIL)

| Item | Route | Status | Independent evidence |
|---|---|---|---|
| `7563c40` / `UPSTREAM-METEORA-FINALIZED-BIN-IDENTITY-116` | Independently refetchable finalized bin-array identity | `FAIL` | Address, slot, payload hash, owner, decoder, pool/index, commitment, and acquisition metadata are additive and coherent, but `binArrayAddress` is only a nonempty string. The committed positive uses `"array"`, which cannot identify a Solana account. `getAccountInfo_min_context_slot` also proves only a lower context bound, not that returned bytes were acquired at the advertised `binArraySlot`. |
| `898f1b7` / `UPSTREAM-METEORA-FINALIZED-BIN-VERIFIER-117` | Independent finalized-account quote reproduction | `FAIL` | Owner, 10,136-byte payload, decoded pool/index, raw SHA-256, duplicate/missing accounts, and exact quote reproduction are enforced. Independent inspection still finds `canonicalAddressBound=false`, `exactSlotRequired=false`, and `laterSlotExplicitlyAdmitted=true`; the verifier's positive uses `"bin-array-0"` and its guard accepts `source.slot > row.binArraySlot`, so identity and historical context remain self-asserted. |

- Available DEV delta: exactly two distinct committed outcomes exist after `36c3387`, in `7563c40` and `898f1b7`; no additional distinct committed outcome exists.
- Verification result: 0 PASS, 2 FAIL, 0 BLOCKED, 0 SKIP across the two distinct DEV outcomes; both are one deduplicated pre-existing HIGH provenance domain.
...
- Validation: focused Meteora tests 13/13 PASS and the complete committed verifier controls reject preserved/recomputed economics and corrupt bytes, but the independent schema/verifier contradiction harness reports `canonicalAddressBound=false`, `exactSlotRequired=false`, and `laterSlotExplicitlyAdmitted=true`. Full suite 484/484 PASS; syntax 87/87 PASS; replay invariants PASS at 5,026.18 blocks/s with 9,997,256-byte heap growth. The real monitoring preflight reports explicit `SKIP/promtool_unavailable`; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Public mainnet RPC health PASS; all six checked provider variables are absent; local index status remains `wrong_network`; exporter health remains unavailable. The retained finalized external-exporter artifact has zero failures, is 406,432 slots behind, and is 827,671,903 ms old from its raw UTC `observedAt` timestamp at this trigger. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts. QA discarded the provisional `7563c40` validation when the DEV writer lock appeared and reran every tier only after final `898f1b7` was stable and unlocked.

## Prior reviewed DEV delta (2/20; retained)

### Preparation identity and transfer-hook evidence admission (2 PASS)

- Prior exact shortfall: 18. `UPSTREAM-PREPARATION-BASE58-IDENTITY-114` and `UPSTREAM-PREPARATION-TRANSFER-HOOK-MAP-115` remain PASS; the prior independent identity, nested-map, full-suite, syntax, replay, and fail-closed operational evidence remains retained.
...
### Semantic alert hold timing and transient recovery (2 PASS)

- Prior exact shortfall: 18; prior validation was independent alert-timing/recovery harness 53/53, focused monitoring 3/3, RPC/query-contract 19/19, health/feed/monitoring 37/37, Meteora/transfer-hook 13/13, full 469/469, syntax 87/87, and replay PASS. Stable IDs 074 and 075 remain closed.

## Prior reviewed DEV delta (2/20; retained)

### Governance-series isolation and canonical alert silence (2 PASS)

- Prior exact shortfall: 18; prior validation was independent governance/canonical harness 46/46, focused monitoring 3/3, RPC/query-contract 19/19, health/feed/monitoring 37/37, Meteora/transfer-hook 13/13, full 469/469, syntax 87/87, and replay PASS. Stable IDs 072 and 073 remain closed.

## Prior reviewed DEV delta (2/20; retained)

### Native alert evaluation and progress-operand isolation (2 PASS)

- Prior exact shortfall: 18; prior validation was independent native-rule-evaluation harness 35/35, focused monitoring 3/3, RPC/query-contract 45/45, health/feed/monitoring 37/37, Meteora/transfer-hook 11/11, full 469/469, syntax 87/87, and replay PASS. Stable IDs 070 and 071 remain closed.
...
- Prior exact shortfall: 18; prior validation was independent alert semantics 35/35, focused alert/metrics 3/3, RPC/query-contract 58/58, health/feed 35/35, Meteora 12/12, full 467/467, syntax 86/86, and replay PASS. Stable IDs 066 and 067 remain closed.
...
### Monitoring governance and progress semantics (2 PASS)

- Prior exact shortfall: 18; prior validation was focused RPC/query-contract 26/26, health/feed 33/33, and Meteora 12/12, full 466/466, syntax 86/86, and replay PASS. Stable IDs 062 and 063 remain closed.

## Prior reviewed DEV delta (2/20; retained)

### Monitoring state-semantics closures (2 PASS)

- Prior exact shortfall: 18; prior validation was focused RPC/query-contract 24/24, health/feed 31/31, and Meteora 12/12, full 464/464, syntax 86/86, and replay PASS. Stable IDs 060 and 061 remain closed.
...
### Unavailable monitoring nested-evidence closures (2 PASS)

- Prior exact shortfall: 18; prior validation was focused RPC/query-contract 45/45, stats 5/5, health 28/28, and Meteora 12/12, full 463/463, syntax 86/86, and replay PASS. Stable IDs 056 and 057 remain closed.

## Prior reviewed DEV delta (3/20; retained)

### Monitoring nested-contract repairs (2 PASS, 1 FAIL)

- Prior exact shortfall: 17; prior validation was focused RPC/query-contract 45/45 and Meteora 12/12, full 462/462, syntax 86/86, and replay PASS. Stable IDs 053 and 055 remain closed.

## Prior reviewed DEV delta (2/20; retained)

### Indexer-health nested and state-semantic closures (1 PASS, 1 FAIL)

- Prior exact shortfall: 18; prior validation was focused RPC/query-contract 43/43 and Meteora 12/12, full 460/460, syntax 86/86, and replay PASS. State semantics remain closed; the nested finding was not fully resolved at that baseline and is closed in the current result by `d0ee136`.

## Prior reviewed DEV delta (1/20; retained)

### Nested indexer-health evidence closure (0 PASS, 1 FAIL)

- Prior exact shortfall: 19; prior validation was focused RPC/query-contract 42/42 and Meteora 12/12, full 459/459, syntax 86/86, and replay PASS. `UPSTREAM-RPC-INDEXER-HEALTH-NESTED-CONTRACT-053` is closed in the current result by `d0ee136`.

## Prior reviewed DEV delta (2/20; retained)

### RPC admission and health-envelope closures (2 PASS at prior baseline)

- Prior exact shortfall: 18; prior validation was focused RPC/query-contract 41/41 and Meteora 12/12, full 458/458, syntax 86/86, and replay PASS. `UPSTREAM-RPC-PARAMETERIZED-ADMISSION-051` and `UPSTREAM-RPC-INDEXER-HEALTH-CONTRACT-052` remain closed.
...
### Token-metadata semantic repair and token-holder RPC discovery (1 PASS, 1 FAIL at prior baseline)

- Prior exact shortfall: 18; prior validation was focused RPC/query-contract 36/36 and Meteora 12/12, full 453/453, syntax 86/86, and replay PASS. Current `d74cc5d` partially repairs the retained holder failure, while `UPSTREAM-RPC-TOKEN-METADATA-CONTRACTS-046` remains closed.

## Prior reviewed DEV delta (2/20; retained)

### Token-metadata RPC envelope and nested schema closure (1 PASS, 1 FAIL at prior baseline)

- Prior exact shortfall: 18; prior validation was focused RPC/query-contract 35/35 and Meteora 12/12, full 452/452, syntax 86/86, and replay PASS. Current `83657a7` closes the retained metadata failure.
...
### Retained block admission and token-balance RPC contracts (1 PASS, 2 FAIL at prior baseline)

- Prior exact shortfall: 18; prior validation was focused RPC/query-contract 25/25 and Meteora 19/19, full 449/449, syntax 86/86, and replay PASS. Current `508a3c8` closes both retained failures; `UPSTREAM-RPC-PAGINATED-HISTORY-CONTRACTS-043` remains closed from `4373e5e`.

## Prior reviewed DEV delta (2/20; retained)

### Canonical singular and paginated history RPC result contracts (2 FAIL at prior baseline)

- Prior exact shortfall: 18; prior validation was focused RPC/query-contract 11/11 and Meteora 19/19, full 447/447, syntax 86/86, and replay PASS. Current `4373e5e` closes the shared retained-block page symptom while transaction finality parity remains open.

## Prior reviewed DEV delta (2/20; retained)

### RPC response-envelope closure and method-catalog discovery (1 PASS, 1 FAIL at prior baseline)

- Prior exact shortfall: 18; prior validation was focused RPC/query-contract 8/8 and Meteora 19/19, full 444/444, syntax 86/86, and replay PASS. `UPSTREAM-RPC-METHOD-CATALOG-041` is resolved by current `f4aeb15`.
...
### HTTP route and outcome envelope closure (2 FAIL)

- Prior exact shortfall: 18; prior validation was focused contract 1/1 and Meteora 19/19, full 441/441, syntax 86/86, and replay PASS.

## Prior reviewed DEV delta (2/20; retained)

### Preparation and response-schema catalog closure (2 PASS)

- Prior exact shortfall: 18; prior validation was focused contract 3/3 and Meteora 19/19, full 441/441, syntax 86/86, and replay PASS.

## Prior reviewed DEV delta (2/20; retained)

### Remaining query-value and WebSocket discovery closure (2 PASS)

- Prior exact shortfall: 18; prior validation was focused contract 4/4 and Meteora 19/19, full 441/441, syntax 86/86, and replay PASS.

## Prior reviewed DEV delta (2/20; retained)

### Query-value profile closure (2 PASS)
...
## Independent 104-domain reconciliation
...
| HTTP response schema registry structure | `PASS` | The registry publishes exactly 81 required object schemas and has canonical registry digest `d9a4f0a41589959c906b72c1cac2bc5141d0fd44f9242272b4a39c5fefd07578`; the full contract digest is `b80971a77e1ed9ae0f49c8f738cc0f3f262133bf9120dfbdc241517c467ad2c8`. Independent recomputation matches. |
...
| Holder-exclusion governance monitoring semantics | `PASS` | `5771f0b` publishes one shared four-rule descriptor across all 6/6 REST/RPC stats and health exclusion profiles. Independent unconfigured, fresh, future-observed, and expired producer outputs validate; 13/13 configuration, timing, count, future-state, freshness, and expiry contradictions reject. |
| Index-ingestion progress monitoring semantics | `PASS` | `f193d60` publishes one shared four-rule descriptor across all 5/5 index-derived REST/RPC monitoring profiles while combined-feed exporter ingestion remains distinct. Empty, complete, source-only, and lag-only profiles validate; 7/7 missing-tip, ordering, arithmetic, source, commitment, and progress contradictions reject. |
...
| Executable-depth success schema | `PASS` | The exact two-branch union matches the existing closed Pump sell/buy quote schemas. Real producer bodies select exactly one branch, and independent inspection confirms constant unsafe flags, closed nested fees/evidence, 8/7 inherited economic relationships, disjoint formulas/amount fields, and six descriptor mutations rejected. |
| Swap-preparation success schema | `FAIL` | Top-level, handoff/binding, preparation/transaction, simulation-policy, universal amount-effect, exact ordered eleven-entry catalog identity, and all eleven instruction-evidence variants are closed. All eleven quote schemas are referenced and ten retain constructor parity. Meteora direction, transfer fees, global traversal sums, endpoints, array membership, decoder ranges, and internally consistent per-array capacity commitments are bound, but a schema-only consumer still admits constructor-impossible economics when the producer recomputes a commitment over an unanchored payload hash. |
...
| Public-health unavailable nested schema | `PASS` | `9db59d5` reuses all 13 exact RPC health evidence profiles. Independent recursive validation accepts real empty, malformed-state, and invalid-event HTTP 503 bodies; rejects missing-required and credential-bearing mutations for all four profiles present across those bodies; and confirms all 15 reachable nested schema object nodes are closed. |
...
| Stats unavailable nested schema | `PASS` | `9af9253` reuses the exact five closed success/RPC monitoring profiles. Independent recursive validation accepts the real malformed-state HTTP 503, rejects 5/5 missing-required and 5/5 credential-bearing nested mutations, and confirms all seven reachable object nodes—including retry stages and chain-conflict rows—are closed. |
...
| RPC indexer-health nested result discovery | `PASS` | `9ce7b4b` admits the real empty-index ingestion sentinel and replaces the shared progressive superset with eight distinct required profiles; `d0ee136` then makes `events.oldestSequence` nullable for the producer-valid empty-retained-log state. Independent recursive validation accepts both real runtime profiles and rejects empty and credential-bearing profiles 8/8 each plus all 56/56 foreign-profile shapes. |
| RPC indexer-health state semantics | `PASS` | `7b23ae4` publishes `indexer-health-state-semantics-v1` with 13 bounded rules across all eight states. Independent validation accepts all eight canonical profiles and the real empty-index state while rejecting 13 boolean, reason, age, threshold, and timestamp contradictions. |
| REST index-health unavailable state semantics | `PASS` | `2842e13` gives `index_health_unavailable_v1` the exact RPC `latestBlockTime` schema and 13-rule `indexer-health-state-semantics-v1` relationship while limiting status to the seven unhealthy states. Independent real empty/malformed 503 bodies validate, 7/7 canonical unhealthy profiles pass, and 13/13 isolated contradictions reject. |
...
| RPC token-holder result discovery | `PASS` | The 19-rule algorithm validates real first/continuation/full pages, null, coherent future-clock evidence, and rejects both retained coupled contradictions: terminal top-ten/concentration understatement and stale unconfigured exclusion evidence. |
| RPC owner token-account result discovery | `PASS` | The nine-rule result algorithm remains intact, while request/result equality accepts six canonical named/positional/null/omitted forms and rejects four forged owner or mint-filter echoes, including both prior coupled drifts. |
| RPC indexer-stats result discovery | `PASS` | The RPC result schema exactly reuses the 24-field closed REST stats schema. The live loopback result has exact key parity, and all 24 omissions plus one unknown credential field reject. |
| RPC indexer-stats nested result discovery | `PASS` | `d0ee136` reuses the same closed retry, exclusion, ingestion, structure, and chain schemas across REST and RPC stats. Independent real-result validation proves REST/RPC equality, while all five missing-required and five credential-bearing nested mutations reject. |
...
| HTTP outcome-envelope discovery | `PASS` | `response_outcome_semantics` binds all 119 real outcomes to exact status, retryability, content type, body presence, derived contract identity, and JSON schema membership; `unique_by(outcome)` now closes route-local ambiguity. All 54 routes pass, seven semantic contradictions reject, and duplicate identities reject on all 50 multi-outcome routes. |
...
| HTTP route-outcome identity invariant | `PASS` | Each response-outcome array declares `unique_by(outcome)`. Independent resolution accepts all 54 route-local sets and rejects duplicate identities on all 50 multi-outcome routes, including otherwise valid envelopes with altered statuses. |
...
## UPSTREAM-QUERY-CONTRACTS-HTTP-ROUTE-ENVELOPE-SCHEMA-028

- Severity: `PASS` (completed by `3bfba5c`)
- Owner: `DEV`
...
- Evidence: `4675b29` publishes `unique_by(path)`, `2355381` publishes `set_partition`, `9395882` publishes `path_template_parameters`, and `3bfba5c` publishes `path_method_policy`; independent relationship resolution accepts the real catalog and rejects duplicate-path, all three partition mutations, all four path-binding mutations, wrong methods on all three POST routes, and POST on an ordinary GET route.
...
- Expected versus actual behavior: the advertised route-envelope schema enforces every claimed unique path and protocol-invariant route binding. Expected and actual now match.
- Acceptance criteria: path uniqueness, exact parameter partitioning, exact path-template bindings, and exact GET/POST route-family policy are met; the real catalog and every isolated negative are retained. All criteria are met.
- Validation results: real catalog, duplicate-path, partition, path-template, and method-policy controls PASS. Focused committed tests 3/3, full 441/441, syntax 86/86, Meteora 19/19, and replay PASS.
...
- Severity: `PASS` (delivered by `4675b29`)
- Owner: `DEV`
- Reproduction: resolve `http.relationships` from the schema, validate all 54 real paths, then replace one distinct route's path with another route's path while leaving the rest of its envelope unchanged.
- Evidence: the schema publishes exactly `{kind:"unique_by",property:"path"}`. The real catalog has 54 unique resolved values and passes; the duplicate-path mutation rejects through that declared relationship rather than test-only logic.
- Affected contracts: route identity, request routing, authentication/quota keys, caches, replay mappings, and generated REST/RPC clients.
- Expected versus actual behavior: duplicate route paths fail closed through the published dialect. Expected and actual now match.
- Acceptance criteria: retain exact relationship declaration, real catalog positive, and distinct-object duplicate-path negative. All criteria are met.
- Validation results: relationship declaration, real catalog, and duplicate-path negative 3/3 PASS; focused committed test 1/1, full 441/441, syntax 86/86, Meteora 19/19, and replay PASS.
...
## UPSTREAM-QUERY-CONTRACTS-PATH-METHOD-POLICY-034

- Severity: `PASS` (delivered by `3bfba5c`)
- Owner: `DEV`
- Reproduction: resolve the route item's `path_method_policy` relationship and dialect declaration, validate all real routes, then independently substitute GET for `/rpc` and both `/prepare-swap` routes and substitute POST for an ordinary GET route.
- Evidence: the schema publishes exactly `{kind:"path_method_policy",path:"path",method:"method",exact:{"/rpc":"POST"},suffixes:{"/prepare-swap":"POST"},default:"GET"}` and declares the kind in the fail-closed dialect. Independent resolution accepts 54/54 real routes, identifies exactly three POST identities, and rejects all four contradictions.
- Affected contracts: REST/RPC and preparation routing, method admission, authentication/quota/audit identity, caching, replay mappings, and generated clients.
- Expected versus actual behavior: `/rpc` and both preparation families are POST while every other published route is GET. Expected and actual match.
- Acceptance criteria: declare the relationship kind and exact policy; retain 54 real positives; reject wrong methods for all three POST identities plus an ordinary GET identity. All criteria are met.
- Validation results: dialect, policy descriptor, 54 routes, three POST negatives, and one GET negative PASS; focused contract 3/3, full 441/441, syntax 86/86, Meteora 19/19, and replay PASS.
- Compatibility/performance impact: discovery vocabulary and digest/ETag change; runtime route methods remain unchanged. Validation is bounded by three constant policy branches and the 54-route catalog.
- Blockers: none; this finding is closed.

## UPSTREAM-QUERY-CONTRACTS-PARAMETER-PROFILE-MAP-035

- Severity: `PASS` (delivered by `026fcbe`)
- Owner: `DEV`
- Reproduction: resolve the route item's `map_keys_catalog_values` relationship and dialect declaration, validate all real routes, then independently omit one declared parameter mapping, inject an undeclared mapping, or replace a real mapping with `privateProfile`.
- Evidence: the schema publishes exactly `{kind:"map_keys_catalog_values",map:"parameterConstraints",keys:"parameters",catalog:"valueConstraints"}` and declares the kind in the fail-closed dialect. Independent resolution accepts 54/54 real routes and rejects all three contradictions.
- Affected contracts: request builders, query validation, pagination, filters, quotes, canonical request identity, cache/quota keys, replay mappings, and generated clients.
- Expected versus actual behavior: every declared query parameter has exactly one constraint profile and every profile resolves to the published value catalog. Expected and actual match.
- Acceptance criteria: declare the relationship kind and exact descriptor; retain 54 real positives; reject missing, injected, and unknown-profile mappings. All criteria are met.
- Validation results: dialect, descriptor, 54 routes, and three negatives PASS; focused contract 3/3, full 441/441, syntax 86/86, Meteora 19/19, and replay PASS.
...
## UPSTREAM-EXECUTABLE-DEPTH-SUCCESS-SCHEMA-038

- Severity: `PASS` (delivered by `d3ea607`)
- Owner: `DEV`
- Reproduction: resolve the executable-depth HTTP 200 body schema, compare its branches with the producer-tested Pump sell/buy schemas, exercise real route outputs, then remove a branch, substitute an open object, duplicate a branch, reopen a branch or nested evidence, or remove an inherited economic relationship.
- Evidence: `executable_depth_success_v1` is exactly the two-branch `oneOf` of the closed Pump sell and buy quote schemas. Real sell and buy outputs each select exactly one branch. Independent inspection confirms closed top-level, fee, and evidence objects; constant available/executable/safety/self-hosted flags; exact blockers; disjoint formulas and amount fields; and all 8 sell plus 7 buy relationships. Six descriptor mutations reject.
- Affected contracts: executable-depth HTTP success, token analysis, fee and liquidity presentation, finalized evidence, generated AI/trading validators, cache digest/ETag, and external-execution admission.
- Expected versus actual behavior: every executable-depth success matches exactly one complete producer schema and rejects shallow, ambiguous, unknown, credential-bearing, type-invalid, or economically underbound bodies. Expected and actual match.
- Acceptance criteria: replace the shallow object with the exact sell/buy union; preserve producer positives, nested closure, constants, bounds, and relationships; reject neither/ambiguous branch matches. All criteria are met.
- Validation results: two real producer positives select one branch each; exact union positive PASS; descriptor mutations reject 6/6; committed missing/credential/type-invalid body negatives reject 3/3; focused 5/5, Meteora 19/19, full 442/442, syntax 86/86, and replay PASS.
- Compatibility/performance impact: discovery schema, digest, and ETag change; runtime response bytes are unchanged. Validation is bounded by two already-bounded branches.
- Blockers: none; this finding is closed.

## UPSTREAM-QUERY-CONTRACTS-OUTCOME-IDENTITY-039

- Severity: `PASS` (delivered by `5487498`)
- Owner: `DEV`
- Reproduction: resolve `responseOutcomes.relationships`, validate each route-local outcome set, then append a duplicate outcome identity to every route with more than one outcome while changing the duplicate's status to retain a distinct otherwise-shaped envelope.
- Evidence: every response-outcome array declares exactly `{kind:"unique_by",property:"outcome"}` through the existing fail-closed dialect. Independent resolution accepts all 54 routes and 119 outcomes and rejects duplicate outcome identities on all 50 multi-outcome routes.
- Affected contracts: retry and parsing branch selection, cache/304 handling, RPC clients, response schema dispatch, generated clients, and AI/commercial safety admission.
- Expected versus actual behavior: each route has at most one branch per outcome identity regardless of otherwise distinct status or representation fields. Expected and actual match.
- Acceptance criteria: publish the exact uniqueness relationship, retain all real route outcomes, and reject duplicate success or failure identities independently of envelope validity. All criteria are met.
- Validation results: 54/54 route-local positives and 50/50 duplicate negatives PASS; focused 5/5, Meteora 19/19, full 442/442, syntax 86/86, and replay PASS.
...
- Severity: `PASS` (delivered by `f4aeb15`)
- Owner: `DEV`
...
- Evidence: the emitted snapshot and bootstrap both publish path `/rpc`, `readOnly:true`, exact batch bounds 1-100, the ordered 12 public runtime methods and matching result identities, a complete 12-key result-schema map, seven nullable descriptors, and five object descriptors. Both real catalog fields validate. Independent generated-style validation rejects 10/10 RPC-catalog mutations and 5/5 result-catalog mutations, including all six previously admitted forms, and detached snapshot mutations do not affect subsequent snapshots.
...
- Expected versus actual behavior: the bootstrap independently requires the exact read-only RPC path, 1-100 batch bounds, complete ordered 12-method/result mapping, exact result-schema key membership, and closed descriptors. Expected and actual now match.
...
- Validation results: emitted and bootstrap catalogs 2/2 PASS; independent unsafe mutations reject 15/15; exact method/result membership, nullable/object cardinality, and snapshot isolation PASS; focused 9/9, Meteora 19/19, full 445/445, syntax 86/86, replay PASS at 8,593.19 blocks/s, and health fails closed as designed.
...
## UPSTREAM-RPC-BLOCK-TRANSACTION-CONTRACTS-042

- Severity: `PASS` (delivered by `508a3c8`)
- Owner: `DEV`
- Reproduction: apply a canonical finalized block and transaction, then change both matching provenance commitments to `confirmed`. Confirm block views quarantine, call `IndexStore.indexedTransactions()`, and request `getIndexedTransaction([signature])` through the real RPC handler before applying `indexed_transaction_result_v1`.
- Evidence: `508a3c8` preserves the internally canonical confirmed row but filters it from `indexedTransactions()`. The independent real RPC returns null before finalization; after matching block/transaction promotion it returns a body whose provenance is finalized and satisfies `indexed_transaction_result_v1`.
- Affected contracts: JSON-RPC block/transaction clients, generated validators, ancestry and replay/reorg safety, provenance/finality qualification, time ordering, payer attribution, schema digest/ETag, and cache regeneration.
- Expected versus actual behavior: every successful non-null transaction RPC body satisfies its advertised finalized provenance schema. Confirmed retained evidence remains internally canonical without crossing the finalized-only public result boundary. Expected and actual now match.
- Acceptance criteria: require finalized provenance in the public indexed-transaction view or publish and test an honest confirmed/finalized union; preserve confirmed ingestion/replay semantics, canonical finalized positives, null misses, both parameter styles, all nine retained schema negatives, and block quarantine controls.
- Validation results: confirmed internal canonical control PASS; public collection withholding PASS; singular RPC null-before-finality PASS; promoted finalized producer/schema control PASS. Focused RPC/query-contract 35/35 PASS; Meteora 12/12 PASS; full 451/451 PASS; syntax 86/86 PASS; replay PASS at 4,657.39 blocks/s with 11,522,960-byte heap growth; operational health fails closed as designed.
- Compatibility/performance impact: block quarantine may withhold incomplete legacy rows as intended. Closing transaction parity either withholds confirmed results until finalization or expands discovery honestly; no provider, production, or database state was mutated.
- Blockers: none; this finding is closed.

## UPSTREAM-RPC-PAGINATED-HISTORY-CONTRACTS-043

- Severity: `PASS` (delivered by `4373e5e`)
- Owner: `DEV`
- Reproduction: validate canonical singular/page blocks, then independently remove or corrupt each transaction/instruction/transfer count and mutate time or provenance before evaluating `indexedBlocks()`. Retain the exact descriptors, bootstrap map, real first/continued pages, cursor controls, runtime boundaries, schema negatives, and snapshot isolation.
- Evidence: the canonical singular and page controls remain available. All 18 missing, negative, fractional, unsafe-integer count, invalid-time, null/confirmed/wrong-network/empty-source/invalid-time provenance mutations fail closed. Exact page descriptors, four real pages, two cursor continuations, six runtime negatives, thirteen schema negatives, and detached snapshots remain compatible.
- Affected contracts: generated paginated-history clients, block backfill/history consumers, address-signature scanners, cursor continuation, schema digest/ETag, and singular/page compatibility.
- Expected versus actual behavior: every successful page block satisfies the exact finalized block schema, while incomplete or non-finalized retained blocks quarantine. Expected and actual now match.
- Acceptance criteria: preserve exact parameter defaults/bounds, base64url cursor rules, canonical first/continued pages, all six runtime negatives, all thirteen schema negatives, signature-page coverage semantics, and the 18 retained-block quarantine controls. All criteria are met.
- Validation results: canonical block controls 2/2 PASS; retained-block mutations reject 18/18; prior descriptors/pages/cursors/runtime/schema/snapshot controls remain PASS; focused RPC/query-contract 25/25, full 449/449, syntax 86/86, and replay PASS.
- Compatibility/performance impact: malformed legacy retained blocks now quarantine until canonical replay; valid finalized response bytes and bounded page validation remain unchanged.
- Blockers: none; this finding is closed.

## UPSTREAM-RPC-TOKEN-BALANCE-CONTRACTS-044

- Severity: `PASS` (delivered by `508a3c8`)
- Owner: `DEV`
- Reproduction: create one canonical finalized SPL-token account snapshot, call `getIndexedTokenAccount` and `getIndexedTokenSupply` through the real RPC handler, validate both bodies and null misses, then replace account/supply/withheld raw amounts with `18446744073709551616` and replace token-account/mint/program identities with `"x"` before applying the published result schemas.
- Evidence: both producer bodies, both null misses, both exact positional/named descriptors, all 24 missing-required controls, and nullable owner/withheld controls behave correctly. All four over-u64 mutations now reject through exact `maximumRaw`; all six one-character token-account/mint/owner/program mutations reject through canonical base58 and exact token-program constraints.
- Affected contracts: wallet balances, token detail, holder/supply analysis, Token-2022 withheld-fund accounting, generated RPC validators, exact integer precision, cache identity, and trading-safety admission.
- Expected versus actual behavior: the advertised result schemas admit every real producer body and reject raw amounts or identities that cannot cross the canonical request/producer boundary. Expected and actual now match.
- Acceptance criteria: bind all four balance/supply/withheld raw properties to the producer u64 maximum; bind token-account, mint, owner when non-null, and program identities to canonical Solana/token-program constraints; preserve both parameter styles, real bodies, null misses, closed fields, coverage/completeness constants, and the 24 required-field negatives.
- Validation results: producer positives 2/2 PASS; null misses 2/2 PASS; descriptors 2/2 PASS; missing-required negatives 24/24 reject; over-u64 mutations 4/4 reject; noncanonical identity mutations 6/6 reject; nullable controls 3/3 PASS. Focused RPC/query-contract 35/35, Meteora 12/12, full 451/451, syntax 86/86, and replay PASS.
- Compatibility/performance impact: discovery hardening changes digest/ETag and generated validators while canonical runtime bytes remain compatible. String-length/raw-maximum validation is constant time over already-bounded fields.
- Blockers: none; this finding is closed.

## UPSTREAM-RPC-TOKEN-LARGEST-ACCOUNTS-045

- Severity: `PASS` (delivered by `b72b16e`)
- Owner: `DEV`
- Reproduction: create a canonical finalized SPL-token snapshot with two positive accounts, call `getIndexedTokenLargestAccounts` using named then positional pagination, validate both real pages and a null miss, and apply the published result schema to isolated row/envelope contradictions.
- Evidence: the exact descriptor publishes both styles, required mint, optional limit/cursor, default limit 20, 1-500 bounds, and nullable 1,024-character cursor. Both real pages and the null miss validate; 13/13 missing-required controls, six row contradictions (zero/over-u64 amounts, over-u64 withheld, noncanonical account/owner/program), and five envelope contradictions (mint, finality, coverage, completeness, cursor) reject.
- Affected contracts: holder/whale discovery, wallet/token concentration clients, finalized account-snapshot pagination, cursor regeneration, generated RPC validators, exact raw precision, and schema digest/ETag.
- Expected versus actual behavior: discovery exactly describes the existing bounded finalized producer and excludes rows or envelopes that cannot cross its runtime boundary. Expected and actual match.
- Acceptance criteria: exact positional/named descriptor; bounded cursor and page size; null miss; closed page and row shapes; canonical mint/account/owner/program identities; positive u64 balances; nullable u64 withheld balances; finalized/coverage/completeness constants; first/continued page and cursor isolation. All criteria are met.
- Validation results: descriptor 1/1 PASS; real pages 2/2 PASS; null miss 1/1 PASS; missing-required controls reject 13/13; row contradictions reject 6/6; envelope contradictions reject 5/5. Focused RPC/query-contract 35/35, full 451/451, syntax 86/86, and replay PASS.
...
- Severity: `PASS` (delivered by `83657a7`)
- Owner: `DEV`
...
- Evidence: `83657a7` publishes mutually exclusive null, authoritative-absence, and metadata-present branches, with exact constants and a present-branch nested/top-level mint equality. Real absent, present, enriched, null-miss, and descriptor controls pass; all 28 top-level/provenance omissions, 7/7 retained semantic contradictions, one direct nested-mint mismatch, and 8/8 nested boundary mutations reject.
...
- Expected versus actual behavior: every successful non-null envelope binds presence to metadata nullability, authoritative absence to the inverse complete-search state, off-chain enrichment to present on-chain metadata, and nested metadata mint to the result mint. Expected and actual now match.
- Acceptance criteria: retain all three exclusive branches, null-on-miss, exact positional/named mint input, closed bounded nested projections, finalized complete-search provenance, `safeForAutomation:false`, real absent/present/enriched bodies, and all structural/semantic negatives. All criteria are met.
- Validation results: producer forms 3/3 PASS; null miss and descriptor PASS; missing-required negatives 28/28 reject; retained semantic contradictions 7/7 reject; direct nested mint mismatch rejects; nested boundary negatives 8/8 reject. Focused RPC/query-contract 36/36 PASS; Meteora 12/12 PASS; full 453/453 PASS; syntax 86/86 PASS; replay PASS at 3,724.67 blocks/s with 9,638,064-byte heap growth; operational health fails closed as designed.
- Compatibility/performance impact: discovery digest/ETag and generated validators change; canonical runtime bytes remain compatible. Three bounded branches and one equality comparison are constant-time over already bounded fields.
- Blockers: none; this finding is closed.

## UPSTREAM-RPC-TOKEN-HOLDERS-CONTRACTS-047

- Severity: `PASS` (delivered by `28ec484`)
- Owner: `DEV`
- Reproduction: create a canonical finalized mint snapshot with three token accounts aggregated into two holders, call `getIndexedTokenHolders` through the real RPC handler for first, continuation, and terminal full pages, apply all nineteen `holder-page-semantics-v1` rules, then replay the retained coupled top-ten/concentration and unconfigured-exclusion stale mutations.
- Evidence: `28ec484` adds `terminal_top10_equals_first_ten_rows` and `unconfigured_exclusions_not_stale`. Real pages, null, exact descriptor, coherent future-clock fail-closed control, closed top-level/nested shapes, and the retained mutation corpus remain compatible. Both producer-impossible coupled bodies now reject 2/2.
...
- Expected versus actual behavior: on a terminal full page, `top10ObservedRaw` equals the exact sum of the first ten ranked rows (or every row when fewer than ten), and an unconfigured exclusion registry has `stale:false`. Expected and actual now match.
- Acceptance criteria: reject both coupled mutations while preserving all reported/retained negatives, real first/continuation/full pages, coherent future-clock fail-closed evidence, null result, finalized/exclusion/withheld projections, replacement-invalidated cursor, and `safeForAutomation:false`. All criteria are met.
- Validation results: published rules 19/19 present; real producer controls PASS; retained coupled contradictions 2/2 reject. Focused RPC/query-contract 38/38 PASS; Meteora 19/19 PASS; full 455/455 PASS; syntax 86/86 PASS; replay PASS at 5,237.56 blocks/s with 9,631,992-byte heap growth; operational health fails closed as designed.
...
- Severity: `PASS` (delivered by `dbfa6b4`)
- Owner: `DEV`
- Reproduction: inspect `getIndexedTokenAccountsByOwner.resultBindings`, execute its algorithm over named and positional filtered, omitted-mint, and explicit-null-mint controls, then forge owner or mint-filter echoes independently of the request while preserving internally coherent rows and all nine result-page rules.
- Evidence: `dbfa6b4` publishes `rpc-request-result-equality-v1` with exact parameter name, positional index, result property, and omitted-null policy. Six canonical controls pass, and four forged results reject, including both coupled echo-and-row drifts retained by QC. The nine bounded page/row rules remain unchanged.
...
- Expected versus actual behavior: the result binds its echoed owner and optional mint filter to either request style, then binds rows to those identities, unique ascending token accounts, one decimals/program identity per mint, and null legacy SPL withheld balances. Expected and actual now match.
- Acceptance criteria: publish and enforce request/result equality for owner and mint filter, reject both coupled drifts and all seven retained row contradictions, and preserve named/positional parameters, first/continuation/full/empty results, cursor scope and replacement invalidation, canonical projection gating, u64 precision, `coverage:"latest_canonical_observed_accounts"`, and `complete:false`. All criteria are met.
- Validation results: binding descriptor PASS; canonical request/result forms 6/6 PASS; forged echoes 4/4 reject; result rules 9/9 retained. Focused RPC/query-contract 39/39, Meteora 19/19, full 456/456, syntax 86/86, and replay PASS.
- Compatibility/performance impact: request/result semantic discovery changes digest/ETag and generated validators; canonical runtime response bytes can remain unchanged. Equality checks and row relationships remain bounded by the 500-row cap.
- Blockers: none; the finding is closed.

## UPSTREAM-RPC-ZERO-PARAM-ADMISSION-049

- Severity: `PASS` (delivered by `b349c66`)
- Owner: `DEV`
- Reproduction: call `getIndexerHealth` and `getIndexerStats` over the real loopback RPC handler with parameters absent, null, empty array, empty object, non-empty array/object, string, number, and boolean; compare discovery parameter profiles with admission.
- Evidence: both methods publish `{ styles:["positional","named"], required:[], optional:[] }`. The eight canonical empty controls return results, while all ten non-empty/scalar controls return exact JSON-RPC `-32602` without interpreting the ignored input.
- Affected contracts: health/stats generated clients, cache and monitoring identity, abuse-resistant request admission, query-contract digest/ETag, and commercial RPC correctness.
- Expected versus actual behavior: zero-parameter methods accept only absent, null, empty-array, or empty-object parameter forms and fail closed on all other forms. Expected and actual match.
- Acceptance criteria: exact empty positional/named descriptors; preserve four canonical empty forms for each method; reject non-empty arrays/objects and scalar input; retain result schemas and redacted errors. All criteria are met.
- Validation results: discovery descriptors 2/2 PASS; live empty controls 8/8 PASS; invalid inputs 10/10 reject; focused RPC/query-contract 38/38 PASS; full 455/455 PASS; syntax 86/86 PASS.
- Compatibility/performance impact: clients sending ignored arguments now receive deterministic `-32602`; canonical clients and result bodies are unchanged. Admission is bounded by already parsed request parameters.
- Blockers: none; the finding is closed.

## UPSTREAM-RPC-INDEXER-STATS-CONTRACT-050

- Severity: `PASS` (delivered by `56bf531`)
- Owner: `DEV`
- Reproduction: compare `indexer_stats_result_v1` with `stats_success_v1`, call `getIndexerStats` through the real loopback RPC handler, validate exact top-level key parity, then remove each required key and inject an unknown credential field.
- Evidence: both schemas are deeply equal, require the same 24 keys, allow no optional or additional top-level properties, and the live result has exact key parity with canonical structure/chain evidence. All 24 omissions and the unknown-field mutation reject 25/25.
- Affected contracts: commercial monitoring, SDK generation, operational dashboards, cache safety, REST/RPC schema parity, secret redaction, and developer experience.
- Expected versus actual behavior: the RPC stats result reuses the stable closed REST projection and rejects missing or unknown top-level fields. Expected and actual match.
- Acceptance criteria: exact shared schema equality; closed 24-field result; live loopback key parity; canonical structure/chain evidence; reject every omission and unknown top-level field. All criteria are met.
- Validation results: schema equality and live result PASS; missing/unknown mutations 25/25 reject; focused RPC/query-contract 39/39 PASS; full 456/456 PASS; syntax 86/86 PASS.
- Compatibility/performance impact: discovery digest/ETag and generated validators change; canonical REST/RPC bytes remain unchanged. Top-level validation is bounded to 24 keys.
- Blockers: none; the finding is closed.
...
- Severity: `PASS` (delivered by `d0ee136`)
- Owner: `DEV`
- Reproduction: recursively validate both the real empty-index result and a real invalid-event result produced by applying a canonical finalized block, retaining its nonzero event high-water mark, and emptying only the retained event log. Enumerate the eight progressive profiles and test empty, credential-bearing, and every foreign-profile required-key shape.
- Evidence: `9ce7b4b` admits the bounded `unknown` ingestion sentinel and publishes eight distinct required profiles, but independently reproduced a real invalid-event incompatibility at nullable `events.oldestSequence`. `d0ee136` replaces that field with the bounded nullable count schema. Independent recursive validation now accepts both real empty-index and invalid-event results and rejects 8/8 empty, 8/8 credential-bearing, and 56/56 foreign-profile shapes.
...
- Expected versus actual behavior: each nested evidence section admits real healthy and fail-closed producer bodies, requires its producer-specific identity, and rejects unknown, empty producer-impossible, cross-profile, or credential-bearing content. Expected and actual now match.
...
- Validation results: real empty-index and invalid-event results PASS; distinct profiles 8/8; empty and credential-bearing negatives reject 8/8 each; foreign-profile shapes reject 56/56. Focused RPC/query-contract 45/45, full 462/462, syntax 86/86, replay, and health fail-closed checks pass at stable result `d0ee136`.
- Compatibility/performance impact: discovery digest/ETag and generated validators change; canonical healthy bytes remain compatible and the fail-closed nullable absence evidence now validates. Recursive checks are bounded to the small public-health projection.
- Blockers: none; this finding is closed.

## UPSTREAM-RPC-INDEXER-HEALTH-STATE-SEMANTICS-054

- Severity: `PASS` (delivered by `7b23ae4`)
- Owner: `DEV`
- Reproduction: inspect the `health_state_semantics` relationship on `indexer_health_result_v1`; independently resolve its 13 rules against canonical healthy, stale, empty, invalid-evidence, unknown-time, chain-conflict, wrong-network, and clock-skew profiles plus isolated boolean, reason, age, threshold, and timestamp contradictions.
- Evidence: the relationship is declared in the fail-closed dialect and names all six participating properties. Independent generated-style validation accepts all eight canonical state profiles and the real loopback empty-index state, and rejects all 13 isolated contradictions.
- Affected contracts: RPC health discovery, availability monitoring, AI/trading admission, freshness/finality alerts, generated validators, cache identity, and commercial SLA evaluation.
- Expected versus actual behavior: status is bound to `healthy`, `reason`, `ageMs`, `staleAfterMs`, and `latestBlockTime`; deterministic states use exact public reasons and timed versus untimed states preserve the producer's timestamp/nullability boundary. Expected and actual match.
- Acceptance criteria: publish a declared bounded relationship; accept every producer state; reject boolean identity, reason nullability/exactness, threshold boundary, negative clock, and timestamp-presence contradictions. All criteria are met.
- Validation results: canonical state profiles 8/8 PASS; real empty-index semantic control PASS; isolated contradictions 13/13 reject. Focused RPC/query-contract 43/43, full 460/460, syntax 86/86, replay, and operational fail-closed checks pass.
- Compatibility/performance impact: discovery digest/ETag and generated validators change while runtime health bytes remain unchanged. Validation is constant-time over six scalar properties and one optional timestamp.
- Blockers: none; the state-semantic finding is closed, and the distinct nested producer/schema defect is now closed separately by `d0ee136`.

## UPSTREAM-RPC-INDEXER-STATS-NESTED-CONTRACT-055

- Severity: `PASS` (delivered by `d0ee136`)
- Owner: `DEV`
- Reproduction: obtain the real `/api/stats` and `getIndexerStats` results from one loopback server; compare their complete projections; recursively validate `deadLetterRetry`, `holderExclusions`, `ingestion`, `structure`, and `chain`; then delete one required key or add a credential-bearing key in each nested object.
- Evidence: both routes return byte-equivalent projections and reuse the same five closed nested schemas. The real result validates; all 5/5 missing-required and 5/5 credential-bearing mutations reject.
- Affected contracts: REST/RPC stats parity, ingestion provenance/finality, exclusion governance, dead-letter recovery, chain identity, structural qualification, generated validators, monitoring, cache identity, and secret redaction.
- Expected versus actual behavior: shared nested stats projections must publish one closed contract, accept the real producer result, and reject missing or unknown credential-bearing evidence. Expected and actual match.
- Acceptance criteria: use exact required/optional/property maps with `additionalProperties:false` for all five shared projections; preserve REST/RPC equality; accept the real result; reject missing-required and credential-bearing mutations. All criteria are met.
- Validation results: REST/RPC equality PASS; real recursive result PASS; missing-required negatives reject 5/5; credential-bearing negatives reject 5/5. Focused RPC/query-contract 45/45, full 462/462, syntax 86/86, replay, and operational fail-closed checks pass.
- Compatibility/performance impact: discovery digest/ETag and generated validators change while runtime result bytes remain compatible. Validation is bounded to five small nested projections.
- Blockers: none; this finding is closed.

## UPSTREAM-REST-STATS-UNAVAILABLE-NESTED-CONTRACT-056

- Severity: `PASS` (delivered by `9af9253`)
- Owner: `DEV`
- Reproduction: corrupt persisted event and dead-letter collection shapes, request `/api/stats`, recursively validate the real HTTP 503 against `stats_unavailable_v1`, compare its five monitoring profiles with both `stats_success_v1` and `indexer_stats_result_v1`, then delete one required key or add one credential-bearing key in each nested result object.
- Evidence: the real malformed-state response is HTTP 503 and recursively validates. All five unavailable profiles are deeply identical to both successful REST and RPC stats. Missing-required and credential-bearing mutations reject 5/5 each, and all seven reachable object nodes are closed, including retry-stage and chain-conflict item schemas.
- Affected contracts: incident telemetry, unavailable REST stats discovery, successful REST/RPC compatibility, ingestion provenance/finality, exclusion governance, dead-letter recovery, chain identity, generated validators, monitoring caches, and secret redaction.
- Expected versus actual behavior: the quarantined stats response must reuse the exact closed monitoring evidence profiles without weakening its distinct top-level unavailable envelope. Expected and actual match.
- Acceptance criteria: preserve the real HTTP 503 and top-level unavailable projection; reuse all five exact success/RPC nested schemas; reject missing-required and unknown credential-bearing content recursively; keep retry stages and chain-conflict rows closed. All criteria are met.
- Validation results: real HTTP 503 PASS; success identity 5/5 PASS; RPC identity 5/5 PASS; missing-required negatives reject 5/5; credential-bearing negatives reject 5/5; reachable closed object nodes 7/7 PASS. Focused stats 5/5, focused RPC/query-contract 45/45, full 463/463, syntax 86/86, replay, and operational fail-closed checks pass.
- Compatibility/performance impact: discovery digest/ETag and generated validators change while runtime 503 bytes remain compatible. Recursive validation is bounded to five small monitoring projections.
- Blockers: none; this finding is closed.

## UPSTREAM-REST-INDEX-HEALTH-UNAVAILABLE-NESTED-CONTRACT-057

- Severity: `PASS` (delivered by `9db59d5`)
- Owner: `DEV`
- Reproduction: request `/api/health` from real empty, malformed-collection, and invalid-event stores; recursively validate every HTTP 503 against `index_health_unavailable_v1`; compare all 13 nested schemas with `indexer_health_result_v1`; then delete one required key or add one credential-bearing key for each profile present across the real bodies.
- Evidence: all three real HTTP 503 bodies validate. The unavailable schema deeply equals the RPC schema for 13/13 retry, exclusion, ingestion, structure, chain, event, decoder, ledger, aggregate, snapshot, metadata, and recovery profiles. Missing-required and credential-bearing mutations reject for all 4/4 profiles present across the exercised bodies, and all 15 reachable nested object nodes are closed.
- Affected contracts: public availability monitoring, incident response, freshness/finality and replay admission, AI/trading safety, commercial SLAs, generated validators, cache identity, and secret redaction.
- Expected versus actual behavior: public-health failures must preserve their distinct top-level HTTP envelope while reusing the producer-exact closed evidence profiles already published for RPC health. Expected and actual match.
- Acceptance criteria: preserve real empty and invalid-evidence HTTP 503 bodies; reuse all 13 exact RPC profiles; reject missing-required and credential-bearing content for every present profile; recursively close retry stages and chain-conflict items. All criteria are met.
- Validation results: real HTTP 503 bodies 3/3 PASS; RPC schema identity 13/13 PASS; present-profile missing-required negatives reject 4/4; credential-bearing negatives reject 4/4; reachable closed object nodes 15/15 PASS. Focused health 28/28, RPC/query-contract 45/45, full 463/463, syntax 86/86, replay, and operational fail-closed checks pass.
- Compatibility/performance impact: discovery digest/ETag and generated validators change while runtime health bytes and status/reason semantics remain compatible. Recursive validation is bounded to 13 small evidence profiles.
- Blockers: none; this finding is closed.

## UPSTREAM-REST-FEED-HEALTH-UNAVAILABLE-NESTED-CONTRACT-058

- Severity: `PASS` (delivered by `dadd78b`)
- Owner: `DEV`
- Reproduction: request `/internal/feed/health` from real absent-exporter, malformed-collection, and invalid-event stores; recursively validate every HTTP 503 against `feed_health_unavailable_v1`; compare the 12 index-derived profiles with `index_health_unavailable_v1`; prove exporter ingestion remains distinct; then delete one required key or add one credential-bearing key for each profile present across the real bodies.
- Evidence: all three real HTTP 503 bodies validate. The feed schema deeply equals public index health for 12/12 retry, exclusion, structure, chain, event, decoder, ledger, aggregate, snapshot, metadata, and recovery profiles. Exporter ingestion remains structurally distinct and closed. Missing-required and credential-bearing mutations reject for all 4/4 profiles present across the exercised bodies, and all 15 reachable nested object nodes are closed.
- Affected contracts: combined feed monitoring, WebSocket and replay availability, exporter status, incident response, cache invalidation, commercial feed SLAs, generated validators, and secret redaction.
- Expected versus actual behavior: combined feed failures must reuse closed index-derived evidence while preserving the distinct exporter-status ingestion producer. Expected and actual match.
- Acceptance criteria: preserve real absent-exporter and invalid-index HTTP 503 bodies; reuse all 12 exact index-derived profiles; retain a distinct closed exporter ingestion schema; reject missing-required and credential-bearing content for every present profile. All criteria are met.
- Validation results: real HTTP 503 bodies 3/3 PASS; index schema identity 12/12 PASS; exporter ingestion distinction PASS; present-profile missing-required negatives reject 4/4; credential-bearing negatives reject 4/4; reachable closed object nodes 15/15 PASS. Focused health/feed 28/28, RPC/query-contract 45/45, full 463/463, syntax 86/86, replay, and operational fail-closed checks pass.
- Compatibility/performance impact: discovery digest/ETag and generated validators change while runtime feed-health bytes, exporter assessment, and replay behavior remain compatible. Recursive validation is bounded to 13 small evidence profiles.
- Blockers: none; this finding is closed.

## UPSTREAM-REST-INDEX-HEALTH-STATE-SEMANTICS-059

- Severity: `PASS` (delivered by `2842e13`)
- Owner: `DEV`
- Reproduction: request `/api/health` from real empty and malformed stores; validate both HTTP 503 bodies against `index_health_unavailable_v1`; compare its `latestBlockTime` property and relationship with `indexer_health_result_v1`; enumerate the unavailable status vocabulary; then apply the 13 isolated boolean, reason, age, threshold, timestamp-presence, and exact-reason contradictions.
- Evidence: both real HTTP 503 bodies validate. The REST unavailable and RPC schemas share the exact `latestBlockTime` schema and exact `indexer-health-state-semantics-v1` relationship. The REST vocabulary is the exact seven-state unhealthy subset (`stale`, `empty`, `invalid_evidence`, `unknown_time`, `chain_conflict`, `wrong_network`, and `clock_skew`); all 7/7 canonical profiles pass and all 13/13 isolated contradictions reject.
- Affected contracts: public health availability, incident status/reason interpretation, freshness/finality and clock-skew handling, REST/RPC compatibility, generated validators, monitoring caches, commercial SLAs, and AI/trading fail-closed admission.
- Expected versus actual behavior: unavailable public-health discovery must constrain status, reason, age, threshold, and optional block time with the same bounded semantics as its runtime producer and RPC counterpart. Expected and actual match.
- Acceptance criteria: retain real empty and malformed HTTP 503 bodies; expose exactly the seven unhealthy producer statuses; reuse the RPC timestamp schema and exact 13-rule state relationship; accept all canonical unhealthy profiles and reject each isolated semantic contradiction. All criteria are met.
- Validation results: real HTTP 503 bodies 2/2 PASS; relationship identity PASS; timestamp-schema identity PASS; status vocabulary exact; canonical unhealthy profiles 7/7 PASS; isolated contradictions reject 13/13. Focused health/feed 28/28, RPC/query-contract 45/45, full 463/463, syntax 86/86, replay, and operational fail-closed checks pass.
...
## UPSTREAM-MONITORING-HOLDER-EXCLUSION-SEMANTICS-062

- Severity: `PASS` (delivered by `5771f0b`)
- Owner: `DEV`
- Reproduction: extract `holderExclusions` from successful and unavailable REST stats, unavailable index/feed health, and RPC stats/health schemas; compare relationship descriptors; generate unconfigured, configured-fresh, future-observed, and expired outputs from `IndexStore.holderExclusionQuality`; then mutate configuration, timing, counts, future state, freshness, and expiry independently.
- Evidence: all 6/6 schemas share the exact four-rule `holder-exclusion-registry-semantics-v1` descriptor. Four producer boundary profiles validate: unconfigured null timing with zero counts, configured fresh, configured future-observed, and configured expired. Thirteen independent contradictions reject, including unconfigured timing/count leakage, missing configured timing, age/future mismatches, incorrect freshness, and fresh future/expired evidence.
- Affected contracts: holder and whale accuracy, exclusion governance, concentration and token-risk assessment, REST/RPC monitoring parity, incident dashboards, commercial data integrity, generated validators, and AI/trading admission.
- Expected versus actual behavior: monitoring discovery must distinguish absent governance from configured evidence and bind freshness to observation and expiry timing without accepting future or expired data as current. Expected and actual match.
- Acceptance criteria: reuse one relationship across all six monitoring schemas; bind unconfigured null timing and zero counts; require configured timing; bind future observation to negative age; bind freshness to non-future positive expiry; preserve real boundary profiles and reject isolated contradictions. All criteria are met.
- Validation results: schema identity 6/6 PASS; rules 4/4 present; producer profiles 4/4 PASS; independent contradictions reject 13/13. Focused health/feed 33/33, RPC/query-contract 26/26, full 466/466, syntax 86/86, replay, and operational fail-closed checks pass.
- Compatibility/performance impact: discovery digest/ETag and generated validators change while runtime exclusion bytes, registry loading, holder/security calculations, persistence, providers, and transport remain compatible. Validation is constant-time over seven bounded fields.
- Blockers: none; this finding is closed.

## UPSTREAM-MONITORING-INGESTION-PROGRESS-SEMANTICS-063

- Severity: `PASS` (delivered by `f193d60`)
- Owner: `DEV`
- Reproduction: extract index-derived ingestion projections from successful/unavailable REST stats, unavailable index health, and RPC stats/health; compare their relationship descriptors; prove combined-feed exporter ingestion remains distinct; validate empty, complete, source-tip-only, and lag-only profiles; then mutate parent tip, source-tip ordering, complete-pair arithmetic, and absent-tip diagnostics.
- Evidence: all 5/5 index-derived schemas share the exact four-rule `index-ingestion-progress-semantics-v1` descriptor and `$parent.tip` binding; combined-feed exporter ingestion is structurally distinct. Empty, complete, source-tip-only, and lag-only profiles validate, preserving intentional partial provenance. Seven independent contradictions reject: progress without tip, source tip below index tip, mismatched complete-pair arithmetic, non-unknown absent-tip source or commitment, and present source-tip or lag with absent tip.
- Affected contracts: index lag and provenance monitoring, discovery/trending freshness, replay operations, REST/RPC compatibility, commercial SLA integrity, generated validators, and AI/trading fail-closed admission.
- Expected versus actual behavior: index-derived progress must never detach from the indexed tip; complete source-tip/lag evidence must reconcile arithmetically, while valid partial evidence remains compatible and absent-tip diagnostics are exact. Expected and actual match.
- Acceptance criteria: reuse one descriptor across five index-derived schemas; preserve the distinct exporter projection; require tip for progress; prevent source tip below index tip; reconcile complete pairs; bind absent-tip diagnostic fields; retain complete and partial profiles and reject isolated contradictions. All criteria are met.
- Validation results: schema identity 5/5 PASS; rules 4/4 present; canonical complete/partial profiles 4/4 PASS; exporter schema distinction PASS; independent contradictions reject 7/7. Focused health/feed 33/33, RPC/query-contract 26/26, full 466/466, syntax 86/86, replay, and operational fail-closed checks pass.
- Compatibility/performance impact: discovery digest/ETag and generated validators change while runtime provenance bytes, partial-evidence support, ingestion, persistence, providers, and transport remain compatible. Validation is constant-time over the parent tip and four bounded projection fields.
- Blockers: none; this finding is closed.

## UPSTREAM-REST-HEALTH-SUCCESS-MONITORING-SEMANTIC-PARITY-064

- Severity: `PASS` (delivered by `4c8341f`)
- Owner: `DEV`
- Reproduction: extract `deadLetterRetry`, `holderExclusions`, and `ingestion` from successful public health; extract retry and exclusions from successful combined-feed health; compare them with RPC health; confirm feed exporter ingestion remains distinct; verify closed nested schemas, exact algorithms and rule counts, stricter successful-response constraints, and detached contract snapshots.
- Evidence: independent harness completed 21/21 checks. Public health shares exact RPC retry, exclusion-governance, and index-progress relationships. Combined-feed health shares exact retry and exclusion relationships but retains its three-rule exporter-ingestion contract. Retry/exclusion/ingestion schemas remain closed, their algorithms and 7/4/4 rule sets are exact, public retry remains canonical-only, public ingestion remains finalized-only, and mutating one returned snapshot does not mutate the registry.
- Affected contracts: successful public health, successful combined-feed health, availability monitoring, holder-risk integrity, replay and feed admission, generated validators, cache identity, commercial health SLAs, and AI/trading fail-closed gates.
- Expected versus actual behavior: successful REST health clients must enforce the same monitoring relationships as RPC clients without misapplying index-progress semantics to exporter ingestion. Expected and actual match.
- Acceptance criteria: reuse exact retry and exclusion relationships in both successful REST schemas; add index-progress semantics only to public index ingestion; retain exporter ingestion distinction; retain canonical/finalized success constraints and closed shapes. All criteria are met.
- Validation results: independent parity checks 21/21 PASS; focused health/feed 35/35, RPC/query-contract 58/58, Meteora 12/12, full 467/467, syntax 86/86, replay invariants, public mainnet RPC health, and operational fail-closed checks pass.
- Compatibility/performance impact: discovery digest/ETag and generated validators change while runtime health bytes, status, ingestion, persistence, providers, and transport remain compatible. Validation remains constant-time over bounded monitoring fields; replay sustained 5,229.54 blocks/s.
- Blockers: none; this finding is closed.

## UPSTREAM-METRICS-MONITORING-SEMANTIC-PARITY-065

- Severity: `PASS` (delivered by `58765b0`)
- Owner: `DEV`
- Reproduction: create due, deferred, and legacy dead letters with private filenames and errors; request `/metrics`; verify scheduled retry arithmetic, retry canonicality, exclusion future-state, index source tip, and export lag; then check missing progress, singleton cardinality, and output redaction.
- Evidence: the nontrivial retry profile emits scheduled `2`, due `1`, deferred `1`, legacy `1`, and canonical `0`. Unconfigured exclusions emit observed-in-future `0`; absent index progress emits `NaN` for source tip and export lag. Each new unlabelled gauge appears exactly once, the scheduled retry remains within the existing fixed `state` vocabulary, and no private filename or error content appears.
- Affected contracts: Prometheus monitoring, retry recovery dashboards, exclusion-governance alerts, ingestion-lag monitoring, incident response, commercial observability, and safe automation gates.
- Expected versus actual behavior: operators must be able to evaluate the published retry, exclusion, and ingestion relationships from fixed-cardinality redacted metrics, with absent progress failing closed. Expected and actual match.
- Acceptance criteria: expose scheduled retry count, retry canonicality, future-observed exclusion state, source tip, and export lag; retain fixed cardinality and redaction; emit `NaN` for absent numeric progress; preserve existing metrics. All criteria are met.
- Validation results: independent health/metrics harness 28/28 PASS across both current outcomes; focused health/feed/monitoring 36/36, RPC/query-contract 58/58, Meteora 12/12, full 467/467, syntax 86/86, replay invariants, and operational fail-closed checks pass.
...
## UPSTREAM-ALERTS-OFFLINE-RULE-FIXTURES-068

- Severity: `PASS` (delivered by `69264d3`)
- Owner: `DEV`
- Evidence: the versioned fixture has exactly eight unique ordered profiles and only the three published alert identities. Independent evaluation against the exact rule expressions preserves canonical non-firing, all three finite contradictions, all three required-series absences, and present-`NaN` partial-progress non-firing. Fixture keys are bounded and credential-free.
- Actual behavior: offline tests now preserve the intended bounded alert truth table without requiring live Prometheus, providers, or production state.
- Compatibility/performance impact: test evidence only; alert rules, metrics, REST/RPC/WebSocket contracts, persistence, provider configuration, and production behavior are unchanged.
- Acceptance criteria: exact profile identity and order; exact three-alert vocabulary; canonical and supported partial profiles non-firing; contradictory and missing-series profiles firing; no secret-bearing fixture fields. All criteria are met.
- Validation results: independent combined fixture/preflight harness 64/64 PASS; focused monitoring 3/3, RPC/query-contract 45/45, health/feed/monitoring 37/37, Meteora/transfer-hook 11/11, full 469/469, syntax 87/87, replay invariants, and operational fail-closed checks pass.

## UPSTREAM-PROMTOOL-ALERT-PREFLIGHT-069

- Severity: `PASS` (delivered by `0fb7433`)
- Owner: `DEV`
- Evidence: the canonical package command invokes `promtool check rules` with argv separation, a 30-second timeout, 8,192-byte stdout/stderr bounds, and a bounded nonsymlink regular file. Independent controls prove checked PASS, executable-unavailable SKIP, redacted parser failure, exact path use, and rejection of relative, directory, empty, and oversized inputs.
- Actual behavior: the local canonical command returns versioned `SKIP/promtool_unavailable` with exit 0 because `promtool` is absent. This is explicit unavailable-tool evidence, not a parser PASS or live deployment qualification; parser errors and other tool failures remain fail closed.
- Compatibility/performance impact: additive offline developer/operations validation only; runtime alerts, metrics, API contracts, providers, persistence, and production state are unchanged. Execution is bounded to 30 seconds and 8,192 bytes per output stream.
...
## UPSTREAM-PROMTOOL-GOVERNANCE-SERIES-PRESENCE-072

- Severity: `PASS` (delivered by `28f362e`)
- Owner: `DEV`
- Evidence: two stable native rule-test groups independently omit `terminal_dex_dead_letter_retry_canonical` and `terminal_dex_holder_exclusions_observed_in_future`. Each retains the other governance gauge plus source tip 120, index tip 100, and export lag 20, so the expected critical alert is attributable to exactly one absent series. Each group contains one exact alert expectation and no endpoint, credential, provider, holder selector, or unbounded label.
- Actual behavior: absent retry canonicality fires only `TerminalDexDeadLetterRetryEvidenceInvalid`; absent exclusion future-state evidence fires only `TerminalDexHolderExclusionsFutureDated`. Both are evaluated after the published one-minute hold with the exact fixed summary and severity.
- Compatibility/performance impact: additive native test evidence only; production rules, metrics, API contracts, persistence, providers, migrations, replay, and runtime behavior remain unchanged.
- Acceptance criteria: isolate each governance-series absence; retain all unrelated semantic inputs at canonical values; require only the corresponding critical alert; preserve bounded credential-free fixtures. All criteria are met.
- Validation results: independent governance/canonical harness 46/46 PASS; focused monitoring 3/3, RPC/query-contract 19/19, health/feed/monitoring 37/37, Meteora/transfer-hook 13/13, full 469/469, syntax 87/87, replay invariants, and operational fail-closed checks pass.

## UPSTREAM-PROMTOOL-CANONICAL-NONFIRING-073

- Severity: `PASS` (delivered by `e753ef8`)
- Owner: `DEV`
- Evidence: one complete canonical native profile supplies retry canonicality 1, exclusion future-state 0, source tip 120, index tip 100, and export lag 20. It evaluates the retry, exclusion, and progress semantic alerts separately after one minute and gives each an explicit empty alert expectation. The existing present-`NaN` progress case remains a fourth explicit non-firing expectation.
- Actual behavior: the test corpus now proves both fail-closed absence and false-positive resistance for all three fixed-cardinality semantic alerts without treating missing scrape evidence as canonical silence.
- Compatibility/performance impact: additive native test evidence only; production rules, metrics, REST/RPC/WebSocket contracts, ingestion, persistence, provider configuration, and runtime behavior remain unchanged.
- Acceptance criteria: provide all five canonical inputs; evaluate all three alerts after their hold; require each to remain absent; retain the partial-provenance non-firing control and all contradiction/absence cases. All criteria are met.
- Validation results: independent governance/canonical harness 46/46 PASS; focused monitoring 3/3, RPC/query-contract 19/19, health/feed/monitoring 37/37, Meteora/transfer-hook 13/13, full 469/469, syntax 87/87, replay invariants, and operational fail-closed checks pass.

## UPSTREAM-PROMTOOL-ALERT-HOLD-SEMANTICS-074

- Severity: `PASS` (delivered by `63f07e7`)
- Owner: `DEV`
- Evidence: the finite-contradiction and complete-series-absence groups each evaluate `TerminalDexDeadLetterRetryEvidenceInvalid`, `TerminalDexHolderExclusionsFutureDated`, and `TerminalDexIndexProgressInconsistent` at zero minutes with an explicit empty result, then evaluate the same alert after one minute with its exact critical label and bounded summary. The corpus has six zero-minute evaluations and ten total explicit non-firing expectations, including canonical and present-`NaN` controls.
- Actual behavior: all three semantic alerts preserve their pending interval for both value contradiction and absence failures, while the published one-minute hold, critical severity, fail-closed expressions, and eventual firing behavior remain intact.
- Compatibility/performance impact: additive native timing evidence only; production rules, gauges, REST/RPC/WebSocket contracts, ingestion, persistence, provider configuration, migrations, and runtime behavior remain unchanged.
- Acceptance criteria: cover all three alert identities at zero and one minute in both failure groups; require no zero-minute firing; retain exact one-minute critical outcomes and quiet canonical/partial controls; keep fixtures bounded and credential-free. All criteria are met.
- Validation results: independent alert-timing/recovery harness 53/53 PASS; focused monitoring 3/3, RPC/query-contract 19/19, health/feed/monitoring 37/37, Meteora/transfer-hook 13/13, full 469/469, syntax 87/87, replay invariants, and operational fail-closed checks pass.

## UPSTREAM-PROMTOOL-TRANSIENT-RECOVERY-075

- Severity: `PASS` (delivered by `ca51571`)
- Owner: `DEV`
- Evidence: two stable 30-second native profiles exercise all three semantic alerts. The contradiction profile transitions retry canonicality `0 1 1`, future exclusion state `1 0 0`, and export lag `19 20 20` while holding source and index tips canonical. The missing-series profile transitions retry, exclusion, and source-tip samples from absent to canonical while retaining index tip and export lag. Each profile evaluates all three alerts at one minute with explicit empty results.
- Actual behavior: a semantic contradiction or absent required series that recovers after 30 seconds clears before the one-minute hold completes and never fires. Sustained zero-minute pending, one-minute critical, canonical quiet, and present-`NaN` controls remain intact.
- Compatibility/performance impact: additive native test evidence only; production alert expressions and holds, gauges, REST/RPC/WebSocket contracts, ingestion, persistence, provider configuration, migrations, and runtime behavior remain unchanged.
- Acceptance criteria: provide finite and missing-series recovery profiles; use 30-second samples; restore every failed semantic input; require all three alerts absent at one minute; preserve sustained failure and quiet controls; keep fixtures bounded and credential-free. All criteria are met.
- Validation results: independent alert-timing/recovery harness 53/53 PASS; focused monitoring 3/3, RPC/query-contract 19/19, health/feed/monitoring 37/37, Meteora/transfer-hook 13/13, full 469/469, syntax 87/87, replay invariants, and operational fail-closed checks pass.

## UPSTREAM-PROMTOOL-ALERT-RECOVERY-BOUNDARY-076

- Severity: `PASS` (delivered by `8097848`)
- Owner: `DEV`
- Evidence: one continuous 30-second profile sustains retry canonicality 0, future exclusion state 1, and export lag 19 against source tip 120 and index tip 100 through the one-minute boundary. All three exact semantic alerts fire there with critical severity and their fixed summaries. At 90 seconds the profile restores retry 1, exclusion 0, and export lag 20; all three alerts have explicit empty results.
- Actual behavior: sustained finite contradictions progress from pending to firing at the one-minute hold and clear on the first canonical evaluation, without losing alert identity, severity, or fixed-cardinality annotations.
- Compatibility/performance impact: additive native test evidence only; production alerts and holds, gauges, REST/RPC/WebSocket contracts, ingestion, persistence, provider configuration, migrations, and runtime behavior remain unchanged.
- Acceptance criteria: one continuous finite-contradiction timeline; three exact one-minute critical outcomes; canonical recovery at 90 seconds; three exact clear outcomes; retained pending, transient, absent, canonical, and partial controls. All criteria are met.
...
## UPSTREAM-PROMTOOL-MISSING-SERIES-RECOVERY-077

- Severity: `PASS` (delivered by `0a20b32`)
- Owner: `DEV`
- Evidence: one continuous 30-second profile keeps retry canonicality, exclusion future-state, and source-tip samples absent through the one-minute boundary while index tip 100 and export lag 20 remain present. The exact three critical alerts fire at one minute. At 90 seconds the missing series return with canonical values 1, 0, and 120; all three alerts have explicit empty results.
- Actual behavior: sustained absence remains fail closed through the hold and clears only after complete canonical evidence returns, symmetrically with the finite-contradiction lifecycle and without treating partial scrape recovery as healthy.
- Compatibility/performance impact: additive native test evidence only; production alerts and holds, gauges, REST/RPC/WebSocket contracts, ingestion, persistence, provider configuration, migrations, and runtime behavior remain unchanged.
...
## UPSTREAM-HTTP-REQUIRED-ADMISSION-MATRIX-080

- Severity: `PASS` (delivered by `d6df82d`)
- Owner: `DEV`
- Evidence: discovery exposes exactly two routes with nonempty required-query partitions: pool quote requires `amountRaw` and `inputMint`, while executable depth requires `amountRaw`. Independent runtime probes omit each of the three required inputs under injected noncanonical decision state and receive stable redacted HTTP 400; complete required sets for both routes progress to the expected fail-closed HTTP 503 state gate.
- Actual behavior: missing required inputs are rejected before decision-state evaluation, discovery and runtime route sets agree, required and optional partitions are disjoint, and canonical positive-u64 minimum/maximum values remain accepted while zero and overflow reject.
- Compatibility/performance impact: regression and handoff evidence only; runtime admission order, discovery bytes and digest, product code, REST/RPC/WebSocket contracts, schemas, ingestion, persistence, provider configuration, migrations, and runtime behavior remain unchanged.
- Acceptance criteria: derive every required route and parameter from discovery; independently omit each required input; require HTTP 400 before unhealthy state; require complete inputs to reach HTTP 503; retain exact positive-u64 boundaries. All criteria are met.
...
## UPSTREAM-HTTP-VALUE-CONSTRAINT-MATRIX-081

- Severity: `PASS` (delivered by `20618cb`)
- Owner: `DEV`
- Evidence: discovery maps candle `interval` to its exact six-value enum and bot-readiness `pool` to a 1–64 UTF-16-code-unit, control-free collection filter. Independent validation accepts exact minimum/maximum interval and filter boundaries, rejects decimal, exponent, leading-zero, empty, newline-bearing, and overlength forms, and confirms five representative invalid live requests return HTTP 400.
- Actual behavior: discovery-generated clients can reject decision-relevant invalid candle aliases and bot filters before request execution, while every published valid boundary remains accepted. Route-to-profile mappings and the contract digest remain stable.
- Compatibility/performance impact: regression and handoff evidence only; current runtime validation, discovery bytes and digest, product code, REST/RPC/WebSocket contracts, schemas, ingestion, persistence, provider configuration, migrations, and runtime behavior remain unchanged.
...
## UPSTREAM-PREPARATION-REQUEST-SCHEMAS-091

- Severity: `PASS` (delivered by `3e61517`)
- Owner: `DEV`
- Reproduction: resolve `pool_prepare_swap_request_v1` and `token_prepare_swap_request_v1` from the closed request-schema catalog; evaluate their required, optional, type, pattern, enumeration, safe-integer, and extension rules; resolve every route through `request_body_schema_binding`; then send admitted and malformed bodies to both real preparation handlers under canonical injected decision-state controls.
- Evidence: pool discovery requires decimal-string `amountRaw` and nonempty `inputMint`, optionally admits a safe-integer `limitTick`, and preserves extension fields. Token discovery requires decimal-string `amountRaw`, optionally admits `buy|sell`, defaults omission at runtime, and preserves extension fields. Independent evaluation accepts 4/4 pool and 4/4 token positive families; rejects 8/8 pool and 6/6 token structural negatives; validates all 54 route bindings; rejects 3/3 cross-schema identities; and proves ten live malformed envelopes return HTTP 400 with exact `invalid_prepare_parameters`. Four admitted live envelopes proceed beyond structural admission to bounded quote/construction outcomes. Token zero amount is correctly distinguished as structurally admitted and later rejected by quote semantics with `invalid_amount`.
- Affected contracts: both unsigned preparation POST routes, route request-body discovery, closed request-schema catalog/bootstrap, generated client validators, safe-integer tick transport, token-side defaults, extension construction fields, contract digest/ETag, and support diagnostics.
- Expected versus actual behavior: generated clients can validate the route-level envelope without falsely closing protocol-specific construction fields; malformed route envelopes fail before quote/construction logic, while admitted envelopes remain subject to venue and positive-amount semantics. Expected and actual match.
- Acceptance criteria: publish distinct pool/token identities; preserve runtime-compatible extension fields; accept zero/positive decimal strings, side omission/buy/sell, and safe-integer tick bounds; reject primitive/array/missing/decimal-alias/empty-mint/fractional-or-unsafe-tick/unknown-side inputs; bind exact routes and bootstrap catalog; verify live structural error separation. All criteria are met.
- Validation results: independent schema/runtime harness PASS across 22 schema cases, 54 real bindings, three cross-binding negatives, four live admitted envelopes, and ten live structural rejections; focused current regressions 6/6, full 475/475, syntax 87/87, replay invariants, and operational fail-closed checks pass.
...
- Severity: `PASS` (delivered by `5698e93`)
- Owner: `DEV`
...
- Evidence: both published optional properties are non-null when present (`limitTick` is `integer`; `side` is the `buy|sell` string enum). `5698e93` replaces nullish omission tests with `Object.hasOwn` checks. Both explicit-null requests now return HTTP 400 `invalid_prepare_parameters`; omitted controls advance to exact pool/token HTTP 404 resource errors, preserving the distinction between absence and an invalid present value.
...
- Owner: `DEV`
- Reproduction: resolve the single and batch-item RPC request schemas, mutate their closure, required/optional partitions, property catalog, and batch maximum, then send standard requests, credential- and policy-looking unknown members, and a mixed real batch through `/rpc`.
- Evidence: both schema positions require exactly `jsonrpc`, `id`, and `method`, allow only optional `params`, and set `additionalProperties:false`; malformed variants reject 8/8. Five standard single requests retain their result and correlation identity. Seven distinct unknown members return exact nonretryable `-32600 Invalid Request`; a six-item batch retains request order, isolates valid results, invalid envelopes, and an unknown method, and nulls only the unsafe correlation ID. Index state is byte-stable across the harness.
- Affected contracts: JSON-RPC request admission, batch isolation and correlation, generated clients, credential-leak prevention, protocol-error classification, contract digest/ETag, audit safety, and commercial supportability.
- Expected versus actual behavior: dispatch considers only the exact standard envelope and rejects every ignored top-level extension before method lookup. Expected and actual match.
- Acceptance criteria: close both schema positions; retain standard single and batch forms; reject malformed discovery variants and unknown runtime members with exact protocol errors; preserve request order, per-item isolation, correlation behavior, and state immutability. All criteria are met.
- Validation results: two schema positions PASS; malformed variants reject 8/8; compatible singles pass 5/5; unknown-member rejections pass 7/7; mixed-batch bindings pass 6/6; focused committed coverage PASS. Full suite 490/490, syntax 87/87, replay invariants, public RPC health, and fail-closed operational checks pass.
- Compatibility/performance impact: valid JSON-RPC 2.0 envelopes are unchanged; clients depending on silently ignored extensions must remove them. Admission scans an already body-bounded object; no persistence, provider, REST, WebSocket, migration, or configuration change exists.
- Blockers: none; this finding is closed.

## UPSTREAM-PREPARATION-BODY-CLOSURE-111

- Severity: `PASS` (delivered by `f7c2093`)
- Owner: `DEV`
- Reproduction: resolve the pool and token preparation request schemas, compare each required/optional union with its property catalog, mutate closure and membership metadata, then submit every published field and credential-, signer-, trace-, retry-, and policy-looking unknown fields to missing-resource preparation routes.
- Evidence: both preparation schemas set `additionalProperties:false`; their disjoint required/optional partitions exactly cover 36 published properties spanning the existing Raydium, Orca, Meteora, Phoenix, OpenBook, PumpSwap, and Pump constructors. Malformed variants reject 8/8. All 36 property bindings pass envelope admission and reach the expected missing-resource 404 boundary. Seven unknown names on each route reject 14/14 with exact HTTP 400 `invalid_prepare_parameters` before lookup. Index state remains byte-stable.
- Affected contracts: unsigned swap-preparation admission, all supported venue constructors, generated request models, signer/credential/policy leak prevention, request diagnostics, contract digest/ETag, and trading-safety boundaries.
- Expected versus actual behavior: every consumed cross-protocol preparation member remains admitted, while unmodeled top-level members fail before state lookup or construction. Expected and actual match.
- Acceptance criteria: publish exact closed pool/token schemas; preserve every consumed field; reject malformed schema variants and unknown members on both routes before resource lookup; retain fail-closed diagnostics and no state mutation. All criteria are met.
- Validation results: two schemas PASS; malformed variants reject 8/8; published property bindings pass 36/36; unknown-member route bindings reject 14/14; focused committed coverage PASS. Full suite 490/490, syntax 87/87, replay invariants, public RPC health, and fail-closed operational checks pass.
- Compatibility/performance impact: existing modeled preparation bodies remain compatible; silently ignored extensions now return HTTP 400. Closed-key scans are bounded by the existing request-body ceiling, and no response, persistence, provider, RPC, WebSocket, migration, or configuration change exists.
- Blockers: none; this finding is closed.

## UPSTREAM-RPC-ID-STRING-BOUND-108

- Severity: `PASS` (delivered by `7a5c710`)
- Owner: `DEV`
- Reproduction: resolve the ID schema at the request item, single success, single failure, batch success, and batch failure positions; mutate its maximum, type, numeric bounds, and member set. Send empty, one-unit, 256-unit ASCII, 256-unit surrogate-pair, safe numeric, null, and overlength ASCII/unicode IDs through the real dispatcher and a mixed batch.
- Evidence: all five schema positions publish the retained inclusive safe-integer bounds plus `maximumLength:256` for strings. Runtime uses JavaScript UTF-16 code-unit length before echoing IDs. Empty and bounded strings, safe integers, and null preserve identity; 257-unit ASCII and 258-unit surrogate-pair IDs return exact nonretryable `-32600` with null correlation, including isolated batch items.
- Affected contracts: JSON-RPC request and response identity, response amplification, batch correlation, cache/audit boundedness, generated validators, protocol errors, contract digest/ETag, and commercial supportability.
- Expected versus actual behavior: string correlation identifiers remain bounded before dispatch or echo while existing empty/bounded strings, safe numeric IDs, and null remain compatible. Expected and actual match.
- Acceptance criteria: publish the same maximum in all five schema positions; preserve numeric bounds and existing types; reject malformed schema variants; prove ASCII and surrogate-pair boundaries, compatible forms, null-id failure, and batch isolation. All criteria are met.
- Validation results: five schema positions PASS; malformed schemas reject 7/7; compatible runtime forms pass 6/6; overlength forms reject 2/2; mixed-batch bindings pass 5/5; focused committed coverage PASS. Full suite 488/488, syntax 87/87, replay invariants, public RPC health, and fail-closed operational checks pass.
- Compatibility/performance impact: string IDs longer than 256 UTF-16 units now fail closed; bounded strings, safe numbers, and null are unchanged. No persistence, provider, REST, WebSocket, migration, or configuration changes exist; validation is constant-time and below the existing body ceiling.
- Blockers: none; this finding is closed.

## UPSTREAM-RPC-METHOD-NAME-BOUND-109

- Severity: `PASS` (delivered by `d086cde`)
- Owner: `DEV`
- Reproduction: resolve the method schema in single and batch request items; mutate minimum, maximum, type, and member closure. Invoke all 12 known methods; send 1-unit, 128-unit ASCII, 128-unit surrogate-pair, empty, 129-unit ASCII, 130-unit surrogate-pair, null, numeric, and object method values; retain a mixed batch.
- Evidence: both request positions publish exact string length bounds 1–128 UTF-16 code units. All known methods advance beyond envelope/unknown-method errors. Bounded unknowns return exact `-32601 Method not found`; empty, overlong, and wrong-type values return exact envelope `-32600 Invalid Request` while preserving valid IDs. Mixed batches isolate and order each result.
- Affected contracts: JSON-RPC request admission, method dispatch, protocol-error classification, audit/request boundedness, batch isolation, generated clients, contract digest/ETag, and deterministic diagnostics.
- Expected versus actual behavior: structurally invalid names fail at envelope admission, while syntactically valid unknown extension names reach method lookup. Expected and actual match.
- Acceptance criteria: publish and enforce exact nonempty 128-unit bounds in single and batch schemas; reject malformed metadata; admit all known methods; distinguish bounded unknowns from invalid names; preserve valid correlation and batch isolation. All criteria are met.
- Validation results: two schema positions PASS; malformed schemas reject 7/7; known methods pass 12/12; bounded unknown controls pass 3/3; invalid names reject 6/6; mixed-batch bindings pass 5/5; focused committed coverage PASS. Full suite 488/488, syntax 87/87, replay invariants, public RPC health, and fail-closed operational checks pass.
- Compatibility/performance impact: empty or overlong method names now fail earlier with `-32600`; all known and bounded extension names remain compatible. Runtime work remains bounded and no persistence, provider, REST, WebSocket, migration, or configuration change exists.
- Blockers: none; this finding is closed.
...
- Owner: `DEV`
- Reproduction: resolve `rpc_parameter_null_binding`; enumerate all required and optional RPC parameter schemas; for every optional member compare explicit null with omission in both named and positional requests, then send null for every required member in both request styles.
- Evidence: all 11 optional members across five methods explicitly include null in their schemas. Named and positional explicit-null responses equal omission 22/22. Every required schema excludes null, and all 18 real required-null requests return exact JSON-RPC `-32602 Invalid params`.
- Affected contracts: all parameterized read-only RPC methods, pagination and filter omission, generated named and positional clients, deterministic diagnostics, contract digest/ETag, and support tooling.
- Expected versus actual behavior: required parameters reject null; optional null means omission only when the published schema is nullable. Expected and actual match.
- Acceptance criteria: publish exact relationship fields and policies; cover all 11 optional members and every required member in both request styles; retain presence, default, shape, format, cursor, and zero-parameter behavior. All criteria are met.
- Validation results: optional null/omission parity 22/22 PASS and required-null rejection 18/18 PASS; focused current tests 12/12, full 482/482, syntax 87/87, replay invariants, public RPC health, and fail-closed operational checks pass.
- Compatibility/performance impact: additive discovery metadata changes the digest/ETag and generated validator requirements only. RPC runtime admission and result bytes, REST, WebSocket, ingestion, persistence, providers, migrations, and configuration remain unchanged. Validation is bounded by 11 optional members.
- Blockers: none; this finding is closed.

## UPSTREAM-RPC-METHOD-RESULT-BINDING-103

- Severity: `PASS` (delivered by `bab5505`)
- Owner: `DEV`
- Reproduction: resolve `rpc_method_result_binding`; compare the 12 method references with `rpcResultSchemas`; then introduce an unknown reference, duplicate a reference, remove a catalog entry, and add an unreferenced entry. Invoke every method with valid required parameters and validate each successful result against the resolved top-level schema variant.
- Evidence: method references and catalog keys form an exact unique 12-way set. The baseline accepts and all four isolated catalog contradictions reject. All 12 live methods produce a result whose null/object top-level shape is admitted by its resolved schema, while the existing suite verifies the deeper closed producer variants.
- Affected contracts: all 12 read-only RPC results, generated decoders, result schema evolution, support diagnostics, contract digest/ETag, and commercial response safety.
- Expected versus actual behavior: each method resolves uniquely through the exact result catalog and successful values validate against the resolved schema. Expected and actual match.
- Acceptance criteria: publish catalog, method, reference, exact-key, and success-validation policies; accept the complete real catalog; reject unknown, duplicate, missing, and extra identities; validate every live result family. All criteria are met.
- Validation results: exact 12-way bijection PASS; isolated contradictions 4/4 reject; live top-level result validation 12/12 PASS; focused current tests 12/12, full 482/482, syntax 87/87, replay invariants, public RPC health, and fail-closed operational checks pass.
- Compatibility/performance impact: additive discovery metadata changes the digest/ETag and generated validator requirements only. RPC request/result bytes, REST, WebSocket, ingestion, persistence, providers, migrations, and configuration remain unchanged. Resolution is bounded by the fixed catalog.
- Blockers: none; this finding is closed.
...
- Owner: `DEV`
- Reproduction: resolve `rpc_parameter_default_binding` from query-contract discovery; enumerate every method default; then add an unknown key, default a required key, and mutate a limit below minimum, above maximum, to a fraction, and to a string.
- Evidence: all 12 methods and the five real defaults satisfy the declared optional-membership and value-schema policies. The independent baseline accepts and all six isolated contradictions reject. The fail-closed dialect declares the exact relationship kind; five focused discovery controls and the complete suite pass.
- Affected contracts: RPC discovery, omission behavior for the five defaulted pagination methods, generated validators and SDKs, request efficiency, contract digest/ETag, and support diagnostics.
- Expected versus actual behavior: a default may exist only for an optional parameter and must validate against that method's published parameter schema. Expected and actual match.
- Acceptance criteria: bind the RPC catalog, method list, parameter catalog, optional/default members, and parameter schemas; accept every real method; reject unknown, required, out-of-range, fractional, and wrong-type defaults; preserve existing runtime omission semantics. All criteria are met.
- Validation results: independent 12-method/five-default baseline PASS; isolated contradictions 6/6 reject; focused current tests 8/8, full 479/479, syntax 87/87, replay invariants, public RPC health, and fail-closed operational checks pass.
- Compatibility/performance impact: additive relationship metadata changes the discovery digest/ETag and generated validator requirements only. Existing defaults, RPC request/result bytes, REST, WebSocket, persistence, providers, migrations, and configuration remain unchanged. Evaluation is bounded by the fixed method catalog.
- Blockers: none; this finding is closed.

## UPSTREAM-RPC-PARAMETER-SHAPE-BINDING-100

- Severity: `PASS` (delivered by `05aed5e`)
- Owner: `DEV`
- Reproduction: resolve `rpc_parameter_shape_binding`; verify exact named and positional styles and unique required-plus-optional membership for all 12 methods; then remove/duplicate styles and duplicate required/optional names. Against the real local RPC handler, send one excess positional and one unknown named parameter to every parameterized method, plus the four admitted empty and five rejected nonempty shapes to each zero-parameter method.
- Evidence: the complete catalog satisfies the declared order, membership, and zero-parameter policies. Four isolated catalog contradictions reject. All 38 independent live probes behave exactly as declared across ten parameterized and two zero-parameter methods; the committed discovery/runtime regressions also pass.
- Affected contracts: all 12 read-only RPC methods, generated named and positional requests, zero-parameter admission, deterministic invalid-params diagnostics, request efficiency, contract digest/ETag, and support tooling.
- Expected versus actual behavior: positional arrays are bounded by catalog order, named objects contain catalog keys only, and zero-parameter methods admit only omission, null, empty array, or empty object. Expected and actual match.
- Acceptance criteria: publish exact member names and all three policies; retain both supported styles and unique catalog membership; reject isolated metadata contradictions; prove excess positional, unknown named, empty, and nonempty runtime behavior across every method. All criteria are met.
- Validation results: independent catalog mutations 4/4 reject and live request-shape probes 38/38 PASS; focused current tests 8/8, full 479/479, syntax 87/87, replay invariants, public RPC health, and fail-closed operational checks pass.
- Compatibility/performance impact: additive discovery metadata changes the digest/ETag and generated validator requirements only. Runtime admission and response bytes, REST, WebSocket, ingestion, persistence, providers, migrations, and configuration remain unchanged. Validation remains bounded by at most the cataloged parameters.
- Blockers: none; this finding is closed.
...
- Severity: `FAIL` / `HIGH`
- Owner: `DEV`
- Reproduction: inspect `meteora_dlmm_quote_v1.properties.binTraversal.items.properties.binArrayAddress` and the `meteora_finalized_bin_array_account` relationship, then inspect `verifyMeteoraDlmmQuoteFinalizedAccounts`. The schema accepts any nonempty string and the committed positives use `"array"` / `"bin-array-0"`. The verifier rejects only `source.slot < row.binArraySlot`, so evidence with `source.slot > row.binArraySlot` is explicitly admitted and reproduced while the quote still labels the earlier slot.
- Evidence: `7563c40` publishes address, slot, owner, decoder, pool/index, commitment, acquisition, and raw-hash metadata. `898f1b7` decodes 10,136-byte Meteora-owned account bytes and rejects missing/duplicate accounts, malformed data, pool/index/hash mismatches, coupled economic forgery, corrupt bytes, and any full-quote mismatch. The independent contradiction harness reports `addressSchema={type:string,minimumLength:1}`, `canonicalAddressBound=false`, `exactSlotRequired=false`, and `laterSlotExplicitlyAdmitted=true`. Consequently, schema-valid positive identities may be impossible to refetch and a later finalized account state can be attributed to the earlier advertised acquisition slot; this is independently fetched content but not immutable evidence for that stated slot.
...
- Expected versus actual behavior: every advertised source identity must be a canonical 32-byte Solana public key and independently fetched bytes must be bound to the exact advertised acquisition context, or to another immutable finalized reference/trusted signature. Actual discovery and verifier accept noncanonical account names and later-slot bytes for an earlier row slot.
- Acceptance criteria: require canonical base58 `binArrayAddress`; require each supplied finalized account's returned context slot to equal the advertised row/top-level acquisition slot, or replace the equality claim with an immutable block/account proof or trusted signature; reject the existing noncanonical positives and an otherwise-valid later-slot vector; retain owner, size, decoder, pool/index/hash, complete quote reproduction, real positive, preserved-digest, recomputed-digest, corrupt-byte, direction, transfer-fee, range, aggregate, path, capacity, and execution-gate controls.
- Validation results: committed focused Meteora tests 13/13 and full suite 484/484 PASS, including owner/size/decoder/pool/index/hash and exact quote reproduction. Independent contract/verifier inspection FAILS both the canonical address and exact acquisition-slot requirements. Syntax 87/87 and replay PASS at 5,026.18 blocks/s with 9,997,256-byte heap growth.
...
- Blockers: none; deterministic offline reproduction from the published schema and exported verifier.
...
- Validation results: prior closed boundaries, Meteora execution reproduction, instruction-evidence labeling, real-producer verification proof, complete dialect declaration, bootstrap schema, nested dialect closure, admission-order closure, admission-outcome closure, canonicalization closure, path-profile closure, all ten value-profile closures, WebSocket bootstrap closure, exact preparation catalog, response catalog, HTTP path uniqueness, set partition, path-template binding, path-method policy, parameter-profile mapping, response-outcome semantics, parameter-default mapping, executable-depth union, and route-local outcome identity PASS; strict protocol-specific schema closure remains 21/22 PASS and 1/22 FAIL (ten quote plus all eleven instruction variants PASS; Meteora finalized-content discovery parity fails). Focused contract/producer 5/5 and Meteora suites 19/19 PASS; full suite 442/442 PASS; syntax 86/86 PASS; replay PASS at 6,750.82 blocks/s with 10,018,048-byte heap growth.
...
- NEXT_DEV_ACTION: require canonical 32-byte base58 `binArrayAddress` values and bind every verifier account to the exact advertised finalized acquisition slot (or an immutable finalized proof/trusted signature), adding noncanonical-address and later-slot negatives while preserving owner, decoder, pool/index/hash, exact quote reproduction, real positive, preserved-digest, recomputed-digest, corrupt-byte, direction, transfer-fee, range, aggregate, path, capacity, and execution-gate controls.
