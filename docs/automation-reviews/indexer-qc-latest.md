...
- Run: `2026-08-28T19:35:16+07:00`
- Scope: `C:\Tuan\devApps\solana_indexer`
- Revision: `3dd2483488b8f8bb15aa85272469296051657ec7`
- Compared with QA baseline: `e3cfe73555b7dcaa6d794f9cfad01ce5066afb7c` (2 DEV commits, 3 changed files)
- Compared with `origin/main`: 10 ahead, 0 behind before this evidence report
- Latest DEV commits: `297987d` (quote semantic relationships) and `3dd2483` (CPMM output conservation)
- Overall result: 2 PASS, 0 FAIL, 0 BLOCKED, and 0 SKIP across the complete stable DEV delta. `UPSTREAM-QA-RAYDIUM-CLMM-QUOTE-SCHEMA-001` now matches constructor tick cardinality, exact-input conservation, gross/net output, transfer-fee mode, and component-slot semantics. `UPSTREAM-QA-RAYDIUM-CPMM-QUOTE-SCHEMA-001` now rejects all four prior transfer-evidence contradictions plus the isolated gross/net output-fee mismatch while preserving both real constructor forms. The parent HIGH finding narrows but remains open because sixteen catalog variants are still schema-null. Live qualification is independently blocked by absent fresh canonical evidence.
...
|---|---|---|---|
| `UPSTREAM-QA-RAYDIUM-CPMM-QUOTE-SCHEMA-001` | pool quote / CPMM catalog row | `PASS` | Six conditional relationships reject all 4/4 previously admitted legacy/finalized transfer-evidence contradictions, and `3dd2483` adds exact `grossOutputRaw = amountOutRaw + outputTransferFeeRaw` conservation. Both real forms pass and the isolated `10 != 7 + 2` mismatch rejects. |
| `UPSTREAM-QA-RAYDIUM-CLMM-QUOTE-SCHEMA-001` | pool quote / CLMM catalog row | `PASS` | `array_length` binds tick count; two `decimal_sum` rules bind exact-input and gross/net-output conservation; conditional fee rules bind no-fee mode; the committed positive is corrected to `1000 = 5 + 993 + 2 + 0`. All 4/4 prior contradictions now reject, real constants/closure/redaction and four slot inversions remain covered, and the new relationship kinds are declared fail closed. |

- Available DEV delta: two BA/PO-selected commits after `e3cfe73` repair two distinct stable quote-schema findings. HEAD advanced from `297987d` to `3dd2483` during the first validation/report attempt; mixed-state results were withheld, the complete delta was refreshed, and every tier was rerun against the stable result. No additional DEV commit or enhancement exists.
- Verification result: 2 PASS, 0 FAIL, 0 BLOCKED, 0 SKIP.
- Exact fix/enhancement shortfall: 18; the two commits close exactly two distinct protocol-schema outcomes, and splitting relationship descriptors or individual contradiction assertions would be padding.
- Validation: the independent generated-validator harness accepts real legacy, Token-2022, and CLMM controls and rejects all nine isolated semantic contradictions 9/9. Dialect relationship kinds remain 9/9 declared and fail closed. Strict protocol-specific closure advances to 6/22 PASS and 16/22 FAIL (CPMM and CLMM quotes plus four instruction-evidence variants PASS; nine quote variants and seven instruction variants remain null). Focused committed suite 7/7 PASS; 63 response schemas and digest `85612c940e1b4fd587b26bc73ea21bead6baa9a9980e91ee04cd472b683af4e8` are structurally coherent; full suite 423/423 PASS; syntax 86/86 PASS; replay invariants PASS at 8,095.49 blocks/s with 9,625,800-byte heap growth; operational health emitted all 20 ordered checks, retained nine blockers, denied production mutation, and exited 1 as designed. Format, lint, typecheck, and build are `SKIP` because the repository defines no such scripts.
...
| HTTP response schema registry structure | `PASS` | The registry publishes 63 schemas with stable identities and digest `85612c940e1b4fd587b26bc73ea21bead6baa9a9980e91ee04cd472b683af4e8`. Independent recomputation matches. Recursive enumeration confirms every live schema-node keyword and all nine relationship kinds are declared, including conditional types, array cardinality, canonical decimal sums, and fail-closed preparation catalog membership, while unknown future keywords remain fail closed. |
...
| Swap-preparation success schema | `FAIL` | Top-level, handoff/binding, preparation/transaction, simulation-policy, universal amount-effect, and catalog identity boundaries are closed. Four instruction-evidence variants plus the CPMM and CLMM quotes now have constructor parity. Nine quote variants and seven instruction-evidence variants remain schema-null. |
...
- Evidence: prior increments close handoff, transaction, policy, universal amounts, all three input/output effect bindings, catalog discovery, and type/protocol membership. Four instruction-evidence schemas match their constructors. `297987d` closes CLMM quote semantics and CPMM transfer-evidence parity; `3dd2483` closes CPMM gross/net output-fee conservation. Nine quote and seven instruction-evidence variants remain schema-null.
...
- Actual behavior: external authority, cataloged preparation identity, preparation, transaction, shared simulation policy, universal raw amounts, all three signer-to-effect relationships, four instruction-evidence variants, and both current CPMM/CLMM quotes are verifiable. Nine quotes and seven instruction-evidence variants remain schema-null. Generated validators still cannot establish complete quote/output provenance or protocol shape for sixteen variants.
...
- Validation results: prior closed boundaries remain PASS; strict protocol-specific schema coverage advances to 6/22 PASS and 16/22 FAIL (CPMM and CLMM quotes plus four instruction-evidence variants PASS; nine quotes and seven instruction variants remain null). All nine isolated current semantic contradictions reject 9/9. Focused suite 7/7 PASS; full suite 423/423 PASS; syntax 86/86 PASS; replay PASS at 8,095.49 blocks/s with 9,625,800-byte heap growth.
...
- NEXT_DEV_ACTION: add the next BA/PO-ranked exact closed quote schema with real-constructor parity and fail-closed empty, credential-bearing, wrong-program, unsafe-state, cross-object, and semantic-mismatch regressions while preserving CPMM/CLMM quote relationships and all four instruction-evidence slot bindings.
