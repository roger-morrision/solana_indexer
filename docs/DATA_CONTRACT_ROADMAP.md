...
| Streaming clients | persisted block, swap and lifecycle events | local canonical index | monotonic block sequence, cursor replay, token/pool/protocol/event-type filters, per-event finality provenance, filtered reorg tombstones, resync signal | authenticated upgrades share weighted local/Redis tenant quota admission and durable redacted usage metering; serialized ingestion, persistence-before-broadcast, explicit finality promotion and fork retraction, heartbeat, pre-write aggregate buffer admission, single-frame size rejection and slow-consumer eviction | `WS /ws` | quota/audit admission, end-to-end replay, oversized-frame/backpressure, filtered-topic, finality and reorg-retraction tests |
