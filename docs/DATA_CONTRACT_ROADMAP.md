...
pool creation, Meteora DLMM legacy/current pair creation, PumpSwap pool creation, and Pump bonding-curve creation plus
legacy/current migrations. Exact output-type cardinality is required, while
Orca initialization and Pump completion remain log-event contracts because their
normalized evidence is not derived from a corresponding retained instruction ABI.
