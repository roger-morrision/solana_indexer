...
AMM v4 OpenBook reserve reconciliation and exact-input analysis quoting are complete: the exact legacy 3,228-byte account layout, market/authority/flags, free/total balances, and finalized component slots are checked before publishing vault-plus-orders-minus-pending-PnL reserves; quotes use the on-chain pool's rational swap fee with ceiling rounding and constant-product output flooring. Unsigned AMM v4 construction and simulation remain fail-closed.
