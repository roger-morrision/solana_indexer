...
| Token page | token transfer and observed-holder history | successful SPL/Token-2022 parsed instructions plus transaction pre/post token balances | token account, owner, mint, exact pre/post raw amount, decimals, signature/slot | loaded-address aware; holder coverage explicitly partial until an account snapshot completes | mint, token-account and holders REST | parser, versioned-address, holder and reorg tests |
...
10. Add Yellowstone/Geyser as the primary bounded ingestion gateway with the current block PubSub path retained for reconciliation and backfill.
