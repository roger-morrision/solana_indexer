...
| Protocol swaps/lifecycle | partial | fixture-backed Raydium CPMM/CLMM, PumpSwap, Pump bonding curve; successful Raydium CPMM, PumpSwap and Pump `create_v2` instructions persist canonical pool/curve, mint, quote, vault, creator, metadata and mode evidence; Pump legacy/current migrations link source curves to destination PumpSwap pools and vaults; finalized CLMM PoolState/vault/TickArrayState snapshots include default and verified signed overflow bitmaps with exact initialized ticks | completion lifecycle events, registry entries and fixture-backed decoders |
