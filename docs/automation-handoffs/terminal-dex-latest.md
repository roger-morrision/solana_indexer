...
## UPSTREAM parameterized RPC admission closure

- Selected ID: `UPSTREAM-RPC-PARAMETERIZED-ADMISSION-051` (commercial API correctness, cache identity, replay safety, wallet/token integrity, and abuse-resistant request handling).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities; QC at `e451321` closed owner binding and stats-schema work but proved 20 shared admission contradictions, making strict parameter shape enforcement the highest-value dependency-ready repair.
- Contract: every cataloged parameterized RPC method now rejects positional arrays longer than its published required-plus-optional profile and named objects containing any property outside that profile with JSON-RPC `-32602`.
- Validation contract: the test derives all ten parameterized methods from discovery and proves both overflow positional and unknown named input reject, preventing method-list drift from silently reopening admission.
...
## UPSTREAM unavailable public-health state semantics

- Selected ID: `UPSTREAM-REST-INDEX-HEALTH-STATE-SEMANTICS-059` (alert correctness, AI/trading admission, replay gating, incident diagnosis, and commercial availability SLAs).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities; after closing unavailable nested evidence, `/api/health` 503 discovery still permitted arbitrary status strings and lacked the 13 cross-field timing/state rules already verified for the identical RPC health producer. This dependency-ready semantic parity gap was the highest-value offline increment.
- Contract: `index_health_unavailable_v1` now publishes the seven exact unhealthy states, the formatted non-null timestamp shape when present, and `indexer-health-state-semantics-v1` for boolean, reason, freshness threshold, timestamp presence, clock-skew, and deterministic-reason consistency.
- Validation contract: REST and RPC reference the same 13-rule relationship descriptor; public 503 discovery excludes `healthy`, and real empty plus malformed-evidence bodies retain valid declared states and nested profiles.
...
## UPSTREAM native missing-series recovery boundary

- Selected ID: `UPSTREAM-PROMTOOL-MISSING-SERIES-RECOVERY-077` (scrape-failure lifecycle correctness, incident trust, holder governance, ingestion safety, and commercial SLA stability).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected the symmetric missing-series lifecycle because absence is intentionally fail-closed but prior continuous recovery coverage exercised only finite contradictions. QC retained IDs 074–075 as verification input and did not cover this boundary.
- Contract: one 30-second native profile keeps retry canonicality, exclusion future-state, and source-tip series absent through the one-minute threshold, requires the exact three critical alerts there, restores complete canonical evidence at 90 seconds, and requires all three alerts to clear immediately.
- Validation contract: focused repository coverage requires the stable missing-series recovery profile, omitted-to-canonical sequences, three exact one-minute critical outcomes, three exact 90-second empty outcomes, and preservation of finite-contradiction, transient-recovery, canonical, and isolated-absence controls.
- Compatibility/migration/configuration: additive native test evidence only; production alert expressions and one-minute holds, gauges, REST/RPC/WebSocket contracts, ingestion, persistence, providers, migrations, and configuration remain unchanged. No migration or operator configuration is required.
- Remaining boundary: native execution remains SKIP until promtool is installed; offline absence/recovery controls do not establish live scrape cadence, rule reload, Alertmanager delivery, or canonical production evidence.
- NEXT_WEB_ACTION: distinguish missing-series firing from finite contradictions during the incident and clear either state only after the first complete canonical evaluation.

## UPSTREAM native progress-operand recovery isolation

- Selected ID: `UPSTREAM-PROMTOOL-PROGRESS-OPERAND-RECOVERY-078` (ingestion diagnosis, scrape recovery, discovery freshness, replay safety, and commercial SLA stability).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected operand-specific recovery because prior fixtures proved each progress operand's absence could fire and proved a combined source-tip absence lifecycle, but did not bind index tip, source tip, and export lag independently through firing and clearing. QC closed IDs 076–077 after selection and remained verification input only.
- Contract: three 30-second native profiles independently omit index tip, source tip, or export lag through the one-minute threshold while all unrelated gauges remain canonical, require only the exact critical progress alert, restore the missing operand at 90 seconds, and require immediate clearing.
- Validation contract: focused repository coverage requires all three stable lifecycle names, omitted-to-canonical sequences for each operand, three exact one-minute critical outcomes, three exact 90-second empty outcomes, and unchanged retry/exclusion alert counts.
- Compatibility/migration/configuration: additive native test evidence only; production alert expressions and one-minute holds, gauges, REST/RPC/WebSocket contracts, ingestion, persistence, providers, migrations, and configuration remain unchanged. No migration or operator configuration is required.
...
## UPSTREAM HTTP request-body discovery

- Selected ID: `UPSTREAM-HTTP-REQUEST-BODY-DISCOVERY-087` (commercial SDK generation, RPC safety, unsigned preparation safety, quota predictability, and supportability).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected runtime-accurate body transport discovery because the prior route-wide matrix proved admission behavior but clients still could not derive media, charset, content-encoding, or configured byte ceilings. QC was consulted after selection and independently closed ID 086.
- Contract: every one of the 54 route rows now carries a closed `requestBody` member. The 51 GET routes publish `null`; `/rpc` and both unsigned preparation routes publish required JSON, optional UTF-8 charset, forbidden content encoding, and the exact server-configured maximum byte count. The live discovery endpoint hashes its configured snapshot, so its ETag changes when either body ceiling changes.
- Validation contract: focused coverage requires exactly three non-null POST policies, 51 null GET policies, exact RPC/preparation ceiling separation, a closed bootstrap schema, and parity between the configured exported snapshot and live discovery response.
...
## UPSTREAM JSON-RPC request envelope discovery

- Selected ID: `UPSTREAM-RPC-REQUEST-SCHEMA-089` (commercial RPC SDK generation, batch safety, quota predictability, and supportability).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected the homogeneous JSON-RPC envelope before venue-specific preparation payloads. Runtime requires version, method, and string/integer/null ID, admits optional array/object/null params, accepts single calls or batches of 1–100, and currently tolerates extension members; discovery must mirror those facts rather than claim a stricter producer contract. QC was consulted after selection and retained response-envelope and method-catalog verification only.
- Contract: discovery now publishes `requestBodySchemas.rpc_request_v1`, binds `/rpc.requestBody.bodySchema` to it, and retains null schema identities on both preparation routes until their heterogeneous payload catalogs are modeled. The bootstrap closes the request-schema catalog to its sole current identity while the item schema explicitly preserves runtime-compatible extension members.
- Validation contract: focused coverage accepts single, extension-bearing, null-ID/null-params, one-item, and 100-item bodies; rejects primitive, missing-field, wrong-version, object-ID, empty-method, empty-batch, and 101-item bodies; and verifies route/catalog/bootstrap identity.
...
## UPSTREAM preparation request schema discovery

- Selected ID: `UPSTREAM-PREPARATION-REQUEST-SCHEMAS-091` (unsigned trade construction safety, generated-client correctness, RPC/HTTP discovery completeness, and commercial supportability).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected the two remaining POST request envelopes after QC independently passed ID 090. The pool handler admits an object with decimal-string `amountRaw`, nonempty `inputMint`, and venue-conditional safe-integer `limitTick`; the token handler admits decimal-string `amountRaw` and optional `buy|sell` side. Both handlers currently tolerate extension fields consumed by downstream venue constructors, so discovery must not falsely close those objects.
- Contract: `requestBodySchemas` now publishes `pool_prepare_swap_request_v1` and `token_prepare_swap_request_v1`; each preparation route names its exact schema through `request_body_schema_binding`. Pool admission requires `amountRaw` and `inputMint` and optionally types `limitTick`; token admission requires `amountRaw` and optionally types `side`. Extension members remain explicitly admitted for runtime parity.
- Validation contract: focused coverage accepts zero/positive decimal strings, omitted/default token side, both explicit sides, safe-integer tick boundaries, and extension-bearing bodies; it rejects primitives, arrays, missing required fields, decimal aliases, empty mint identity, fractional ticks, and unknown sides. Route bindings and the closed bootstrap catalog are verified.
- Compatibility/migration/configuration: additive discovery changes the digest/ETag only. Runtime preparation admission, unsigned construction, quotas, persistence, providers, RPC, WebSocket behavior, migrations, and configuration are unchanged. Generated clients may validate the route-level envelope but must retain protocol-specific downstream construction checks.
- Remaining boundary: pool `limitTick` conditionality depends on the resolved venue, and venue-specific construction members remain runtime-enforced; RPC method/parameter cross-binding and QC's Meteora finalized-content provenance finding remain evidence-backed increments. Live qualification remains blocked on canonical operational evidence.
- NEXT_WEB_ACTION: generate distinct pool and token preparation envelope validators, retain extension construction fields, and apply venue-specific requirements only after resolving the pool protocol.

## UPSTREAM preparation body admission ordering

- Selected ID: `UPSTREAM-PREPARATION-BODY-ADMISSION-ORDER-092` (trading-safety diagnostics, deterministic client behavior, quota predictability, and discovery/runtime parity).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected a current admission-order contradiction after QC passed ID 091 under canonical decision-state controls. The published HTTP order places body validation before weighted quota, structure quality, decision quality, and route lookup, but preparation envelope checks still ran after those state/resource gates. Malformed client input could therefore be misreported as retryable 503 or missing-resource 404.
- Contract: pool and token preparation envelopes are now validated immediately after bounded JSON decoding and before weighted quota or index-quality evaluation. Schema-level decimal-string, required-mint, safe-integer tick, and side-enum failures return the existing non-retryable 400 `invalid_prepare_parameters`; protocol-dependent tick required/forbidden checks remain after finalized pool resolution.
- Validation contract: focused coverage proves malformed pool and token bodies return exact 400 responses even with the default unhealthy/empty store, while valid route envelopes proceed beyond body admission. Existing media-type, malformed-JSON, method, quota, path, venue-dependent, and successful unsigned-construction tests remain intact.
- Compatibility/migration/configuration: this changes only error precedence for structurally invalid preparation requests, aligning runtime with already published admission order and schemas. Valid requests, quotas after body validation, unsigned construction, persistence, providers, RPC, WebSocket behavior, migrations, and configuration are unchanged.
- Remaining boundary: venue-specific construction fields remain downstream-enforced; QC's Meteora finalized-content provenance defect and RPC method/parameter cross-binding remain evidence-backed increments. Live qualification remains blocked on canonical operational evidence.
- NEXT_WEB_ACTION: treat preparation HTTP 400 as a non-retryable envelope construction error and reserve 404/503 handling for schema-valid requests that reach resource or decision-state evaluation.

## UPSTREAM RPC request method binding

- Selected ID: `UPSTREAM-RPC-METHOD-PARAM-BINDING-093` (generated-client safety, commercial RPC integrity, deterministic unknown-method handling, and supportability).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected the dependency-ready relationship between the published RPC envelope and method catalog. Discovery exposed both structures but did not state that known methods validate against their catalog parameter declarations while structurally valid unknown methods reach JSON-RPC method-not-found handling. QC was consulted after selection; its independent pass for ID 091 and separate high-priority Meteora provenance defect do not contradict this bounded increment.
- Contract: the root query-contract schema now declares `rpc_request_method_binding`, linking `requestBodySchemas.rpc_request_v1` method and params properties to `rpc.methods`; known methods use catalog parameter validation and unknown names preserve `method_not_found` semantics.
- Validation contract: all 12 current method identities are unique and publish positional and named parameter styles; the envelope exposes method and params, while missing envelopes, missing params properties, duplicate catalog identities, and incomplete styles reject. Existing real-handler coverage preserves exact JSON-RPC `-32601` behavior for unknown methods.
- Compatibility/migration/configuration: additive fail-closed discovery vocabulary changes the digest/ETag only. Runtime RPC admission and results, HTTP behavior, ingestion, persistence, providers, WebSocket behavior, migrations, and configuration are unchanged. Generated validators must implement the relationship before accepting the new document.
- Remaining boundary: parameter-member types remain described by catalog field names and runtime method logic; QC's Meteora finalized-content provenance defect remains a larger dependency chain. Live qualification remains blocked on canonical operational evidence.
- NEXT_WEB_ACTION: resolve known JSON-RPC methods through the published catalog before transport, validate named or positional params from the selected entry, and preserve unknown methods for the indexer's method-not-found response.

## UPSTREAM preparation optional-null admission repair

- Selected ID: `UPSTREAM-PREPARATION-OPTIONAL-NULL-ADMISSION-094` (unsigned trading safety, deterministic diagnostics, discovery/runtime parity, and supportability).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected a newly verified contract contradiction. Independent QC proved that explicit `limitTick:null` and `side:null` values violate the published non-null optional schemas but passed the early validators and reached resource lookup. This current, dependency-ready admission defect outranked the larger Meteora finalized-content provenance chain.
- Contract: preparation envelope validation now distinguishes omission from explicit null. Omitted `limitTick` and `side` retain their documented optional/default behavior; present `limitTick` must be a safe integer and present `side` must be `buy` or `sell`.
- Validation contract: discovery-schema checks reject both explicit-null forms, and real HTTP admission returns exact non-retryable 400 `invalid_prepare_parameters` for both routes before missing-resource or decision-state evaluation. Existing fractional-tick, unknown-side, omission, and valid-value boundaries remain covered.
...
## UPSTREAM canonical RPC cursor parity

- Selected ID: `UPSTREAM-QA-RPC-CURSOR-PARAMETER-SCHEMA-001` (commercial pagination correctness, generated-client parity, cache identity, and deterministic diagnostics).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected the verified dependency-ready cursor contradiction after independently reviewing the runtime codec and roadmap requirement for canonical opaque version-1 cursors. The defect affects every cursor-bearing RPC method and directly undermines IDs 096/097; it therefore outranks unblocked additive discovery work. Authenticated Meteora provenance remains the higher-severity but design-dependent item.
- Contract: explicit empty-string cursors now fail with JSON-RPC `-32602`, while null and omission retain first-page semantics. Published cursor schemas name the bounded `canonical-cursor-v1` format in addition to their nonempty, 1,024-character, base64url lexical constraints, requiring decoded canonical JSON with exact version, key, and scope semantics.
- Validation contract: focused discovery checks the named canonical format; the real RPC handler rejects empty, one-character versionless, and malformed cursors, accepts null and omission, and continues accepting a producer-generated canonical cursor in positional form.
...
## UPSTREAM RPC parameter shape binding

- Selected ID: `UPSTREAM-RPC-PARAMETER-SHAPE-BINDING-100` (generated-client correctness, deterministic diagnostics, commercial RPC integrity, and request efficiency).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected the dependency-ready request-shape boundary after the default binding. Runtime already rejects excess positional values, unknown named keys, and nonempty input for zero-parameter methods across all 12 methods, but discovery did not explicitly bind those rules to catalog order and membership. QC was consulted after selection; its Meteora finalized-content provenance defect remains open and requires independently authenticated evidence rather than a self-consistency label.
- Contract: query-contract discovery now declares `rpc_parameter_shape_binding`: positional arrays may contain at most the cataloged required-plus-optional members in catalog order, named objects may contain only cataloged keys, and zero-parameter methods admit only omitted, null, empty-array, or empty-object params.
- Validation contract: focused discovery verifies the exact relationship, both supported styles, unique parameter membership, and isolated missing-style and duplicate-name contradictions. Existing live route coverage rejects excess positional and unknown named input for all ten parameterized methods and rejects nonempty scalar, array, and object input for both zero-parameter methods.
- Compatibility/migration/configuration: additive relationship metadata changes only the query-contract digest/ETag. RPC request/result bytes and runtime admission, REST, WebSocket, ingestion, persistence, providers, migrations, and configuration are unchanged. Generated validators must implement the new relationship before accepting the document.
- Remaining boundary: required-value and method-specific cursor-scope semantics remain runtime-enforced; authenticated Meteora quote provenance and fresh canonical operational qualification remain open.
- NEXT_WEB_ACTION: generate positional arrays in catalog order, reject excess elements and unknown named keys before transport, and send no parameters for zero-parameter methods.

## UPSTREAM RPC required-parameter presence binding

- Selected ID: `UPSTREAM-RPC-PARAMETER-PRESENCE-BINDING-101` (generated-client correctness, deterministic diagnostics, commercial RPC integrity, and avoidable request reduction).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected the remaining dependency-ready RPC request boundary after shape closure. Discovery names required members but did not explicitly state that named requests must contain every required key or that positional arrays must contain the complete required prefix. QC was consulted after selection and independently passed IDs 099–100; its Meteora finalized-content provenance defect remains open and requires authenticated evidence design.
- Contract: query-contract discovery now declares `rpc_parameter_presence_binding`: named parameter objects contain all cataloged required keys, positional arrays contain the complete required prefix in catalog order, and optional parameters may be omitted.
- Validation contract: focused coverage verifies the exact relationship and all nine required-parameter methods through the real RPC handler. Empty named objects and empty positional arrays return exact JSON-RPC `-32602` for each method, while existing value-schema, optional-default, shape, and zero-parameter coverage remains intact.
...
- NEXT_WEB_ACTION: require every named required key and the complete positional required prefix before sending a known RPC method, while continuing to omit optional members when unset.

## UPSTREAM RPC optional-null binding

- Selected ID: `UPSTREAM-RPC-PARAMETER-NULL-BINDING-102` (generated-client consistency, pagination correctness, deterministic diagnostics, and commercial RPC supportability).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected the remaining dependency-ready null/omission boundary after required-presence closure. All current optional RPC members publish nullable schemas and runtime uses null as omission, while every required schema excludes null, but discovery did not state that semantic relationship. QC was consulted after selection and independently passed ID 101; its Meteora finalized-content provenance defect remains open and requires authenticated evidence design.
- Contract: query-contract discovery now declares `rpc_parameter_null_binding`: required parameters reject null, while an optional parameter may use null as omission only when its published schema explicitly includes null.
- Validation contract: focused discovery verifies the exact relationship and nullable schemas for all 11 optional members across five methods. Real RPC calls prove each explicit-null optional member is response-equivalent to omission while existing required-null, presence, default, shape, format, and cursor-invalidity coverage remains intact.
- Compatibility/migration/configuration: additive relationship metadata changes only the query-contract digest/ETag. Runtime admission and RPC result bytes, REST, WebSocket, ingestion, persistence, providers, migrations, and configuration are unchanged. Generated validators must implement the relationship before accepting the document.
- Remaining boundary: method-specific cursor scope and retained-record semantics remain runtime-enforced; authenticated Meteora quote provenance and fresh canonical operational qualification remain open.
...
## UPSTREAM finalized transfer-hook request evidence

- Selected ID: `UPSTREAM-PREPARATION-TRANSFER-HOOK-MAP-115` (Token-2022 execution safety, generated-client parity, evidence integrity, and bounded request admission).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected the remaining open nested preparation input. Meteora already rejected malformed finalized source-account bytes during construction, but discovery described the address map only as nullable object and pre-lookup admission accepted arbitrary nested content.
- Contract: `transferHookAccountData` is now a nullable map of at most 512 canonical Solana addresses to closed finalized evidence records. Each record binds schema version, matching address, finalized commitment, safe slot, canonical owner, at most 65,536 lowercase raw bytes, exact byte length, and SHA-256 payload hash; runtime enforces the same shape, key equality, length, and digest before state lookup.
- Compatibility/migration/configuration: valid Meteora route evidence and null/omission remain compatible. Malformed, credential-bearing, address-mismatched, non-finalized, oversized, or hash-inconsistent maps now receive deterministic `400 invalid_prepare_parameters`; response bytes, persistence, providers, RPC/WebSocket behavior, migrations, and configuration are unchanged, while discovery digest/ETag changes.
- NEXT_WEB_ACTION: generate transfer-hook account maps from the published closed schema and verify address-key equality plus raw-byte length and SHA-256 before transport.

## UPSTREAM finalized Meteora bin-array identity

- Selected ID: `UPSTREAM-METEORA-FINALIZED-BIN-IDENTITY-116` (quote provenance, AI/trading-safety verification, and commercial schema completeness).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected the dependency-ready trust-anchor increment under the remaining HIGH Meteora quote finding. Capacity commitments named a raw payload hash but did not tell schema consumers which finalized account and context independently supplied those bytes.
- Contract: every Meteora traversal row now carries its source bin-array address and finalized acquisition slot. Discovery publishes a fail-closed `meteora_finalized_bin_array_account` relationship requiring account refetch at that context, Meteora program ownership, pool/index decoding through `meteora_dlmm_bin_array_v1`, and SHA-256 equality with `binArrayPayloadHash` before accepting the committed row economics.
- Compatibility/migration/configuration: two fields are added to each Meteora traversal row and the discovery digest/ETag changes. Existing quote inputs, execution reproduction, REST status behavior, persistence, providers, RPC/WebSocket contracts, migrations, and configuration are unchanged; generated consumers must regenerate and verify the new evidence relationship.
- NEXT_WEB_ACTION: refetch every advertised Meteora bin-array account at finalized commitment with the published minimum context slot, then verify owner, decoded pool/index, and raw SHA-256 before accepting traversal economics.

## UPSTREAM finalized Meteora quote verification

- Selected ID: `UPSTREAM-METEORA-FINALIZED-BIN-VERIFIER-117` (independent quote reproduction, trading-safety enforcement, and commercial consumer readiness).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected the dependency-ready verifier behind the remaining HIGH Meteora quote risk. The prior contract made finalized accounts independently refetchable, but each consumer still had to implement the owner, context, decoder, identity, and full-quote comparison sequence correctly.
...
## UPSTREAM Meteora pool-evidence receipt binding

- Selected ID: `UPSTREAM-METEORA-VERIFICATION-RECEIPT-122` (quote/pool provenance integrity and safe preparation handoff).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected the dependency-ready QC failure ahead of receipt integration. The prior receipt admitted same-address mutation of quote-critical pool state because it bound only `pool.address`.
- Contract: receipt creation now commits to a deterministic SHA-256 digest of the complete canonicalized pool evidence. Receipt verification recomputes that digest and rejects same-address mutation of mints, fee parameters, slots, finalized account projections, or other pool evidence, while retaining quote, account, owner, slot, payload, policy, and receipt-hash checks.
- Compatibility/migration/configuration: newly created receipts add `poolEvidenceHash` and use the strengthened `owner_decoder_pool_evidence_index_payload_hash_exact_quote` policy. Prior under-bound receipts intentionally fail verification and must be regenerated from the original verified quote, pool, and finalized account evidence. Quote, preparation, persistence, provider configuration, REST/RPC/WebSocket shapes, and migrations are otherwise unchanged. The receipt remains tamper-evident, not a trusted signature.
- NEXT_WEB_ACTION: regenerate stored Meteora verification receipts with `poolEvidenceHash` before attaching them to unsigned preparations.

## UPSTREAM verified Meteora preparation boundary

- Selected ID: `UPSTREAM-METEORA-VERIFIED-PREPARATION-123` (simulation safety, auditable quote provenance, and downstream integration readiness).
- BA/PO decision: fresh inspection retained 22 evidence-backed opportunities and selected the dependency-ready handoff gap after strengthening receipt content. Unsigned preparation remained possible without recording verification, and local simulation did not require the receipt to be rechecked against the quote and pool.
- Contract: `bindMeteoraDlmmVerificationReceiptToPreparation` validates the original preparation hash and quote economics identity, revalidates the strengthened receipt against the exact quote and pool, embeds it, and recomputes the preparation hash. `simulatePreparedMeteoraDlmmSwap` now rejects unbound preparations and revalidates the embedded receipt before contacting local RPC.
- Compatibility/migration/configuration: unsigned construction remains available and non-authorizing, but simulation callers must bind a current strengthened receipt and provide its quote/pool evidence. Existing unbound preparations intentionally fail before RPC and must be rebound. No signing, submission, provider, persistence, REST/RPC/WebSocket, database, or configuration changes are introduced.
- NEXT_WEB_ACTION: call `bindMeteoraDlmmVerificationReceiptToPreparation` after finalized account verification and persist only the returned bound preparation for simulation.

## UPSTREAM simulation-time semantic rebinding

- Selected ID: `UPSTREAM-QA-METEORA-PREPARATION-RECEIPT-002`. Source baseline: `b92c9cf`. Product decision: pre-simulation semantic integrity outranks acquisition orchestration because the latter would distribute an artifact still vulnerable to cross-quote substitution. This is one defect, not ten findings for its ten regression vectors.
- Fresh BA/PO reconciliation below covers 20 distinct product opportunities or retained safeguards. `R` means `DATA_CONTRACT_ROADMAP.md`; `T` means `test/indexer.test.js`. Retained safeguards are not new deliverables. Prior handoff IDs are reused where applicable; roadmap review IDs identify distinct unfinished product capabilities, not asserted defects.

| Stable UPSTREAM ID | User/business value and contract | Evidence | Priority; dependency; compatibility | Acceptance and validation | Status |
|---|---|---|---|---|---|
| UPSTREAM-QA-METEORA-PREPARATION-RECEIPT-002 | Prevent cross-quote simulation; Meteora helper | QC coupled metadata drift bypasses binder; wire reconstruction regression | P0; canonical builder ready; internal rejection tightening | Rehashed metadata, wire bytes, accounts and effects must agree before RPC; focused mutation matrix | Implemented; independent QC pending |
| UPSTREAM-METEORA-ORCHESTRATION-124 | Fewer integration errors; account-to-preparation helpers | separate acquisition, creation, binding exports | P1; semantic fix first; additive | Exact-context positive and zero-call preflight tests | Deferred dependency |
| UPSTREAM-METEORA-VERIFICATION-RECEIPT-122 | Same-address evidence integrity; receipt | poolEvidenceHash and receipt regression | P1; canonical pool; old receipts rejected | Original/reordered pool accepted, changed pool rejected | Retained |
| UPSTREAM-METEORA-ACQUISITION-PREFLIGHT-120 | Bounded provider usage; account RPC acquisition | bounded traversal and exact-context helper | P1; canonical quote; valid calls unchanged | Malformed identity/slots never call RPC | Retained |
| UPSTREAM-ROADMAP-LIVE-INGESTION-001 | Current discovery/activity; canonical feed | R known limits; operational health | P1; operator provider/mainnet evidence; no gate relaxation | Fresh canonical events and transactions; health/isolated qualification | Operator blocked |
| UPSTREAM-ROADMAP-TRENDING-USD-001 | Meaningful ranking; trending | R trending explicitly lacks USD volume claim | P2; complete trusted USD reference; additive future design | Units/coverage explicit; window and reference tests | Deferred |
| UPSTREAM-TOKEN-ACCOUNT-SUCCESS-SCHEMA-001 | Wallet balance correctness; token detail | T exact raw/withheld projection tests | P1; finalized snapshot; unchanged | No withheld/base conflation or credential escape; full suite | Retained |
| UPSTREAM-HOLDERS-SUCCESS-PARITY-001 | Holder/whale concentration; holder REST | shared closed holder schema and R exclusions | P1; complete reviewed exclusions; unchanged | Incomplete exclusions cannot assess concentration; full suite | Retained |
| UPSTREAM-ROADMAP-WALLET-HISTORY-001 | Trustworthy trader PnL; wallet profile | store.walletProfile missing complete_wallet_history | P2; retained-history acquisition; explicit partial coverage | No global-history claim from tracked swaps; wallet tests | Deferred |
| UPSTREAM-ROADMAP-ENTITY-LABELS-001 | Explainable wallet intelligence; profile/funding | store.walletProfile missing validated_entity_labels | P2; governed labeled evidence; never infer identities | Provenance/expiry before labels affect analysis | Evidence blocked |
| UPSTREAM-ROADMAP-SYBIL-001 | Safer wallet analysis; funding cluster | store.walletProfile missing sybil_cluster_analysis | P2; validated methodology/data; preserve smartMoney:false | Shared funder alone cannot classify an entity; funding tests | Evidence blocked |
| UPSTREAM-ROADMAP-ORACLE-PROVIDERS-001 | More usable pool depth; OpenBook quote | R unsupported legacy oracle forms | P2; pinned provider ABI/vectors; remain non-executable | Supported decoder plus freshness/confidence negatives | Deferred |
| UPSTREAM-ROADMAP-PUMPSWAP-MODES-001 | Broader pool trading analysis; PumpSwap | R cashback/SOL-buyback modes fail closed | P2; verified accounting vectors; no inferred fees | Exact mode-specific conservation and route tests | Deferred |
| UPSTREAM-QA-GAP-FEED-SUCCESS-PROJECTION-001 | Safe cache invalidation; replay/reorg consumers | gap schema identity/order tests | P1; canonical exporter; unchanged | Divergent skipped slots/reorg evidence reject; full suite | Retained |
| UPSTREAM-ROADMAP-WS-ACK-001 | Reliable commercial streaming; WS replay | T acknowledgement and expiry tests | P1; persisted sequence; unchanged | Invalid acknowledgements rejected and stalled peers evicted | Retained |
| UPSTREAM-ROADMAP-WAREHOUSE-QUALIFICATION-001 | Durable commercial history; multi-sink contract | R phases 9/11; unavailable checkpoint | P1; isolated configured sinks; no production mutation | Exact source/sink content reconciliation before retirement | Operator blocked |
| UPSTREAM-ROADMAP-RECOVERY-QUALIFICATION-001 | Recoverable customer history; recovery receipt | R isolated drill requirement; recovery tests | P1; backup and isolated stores; explicit approval boundary | Content identity, RTO, no live-root writes | Operator blocked |
| UPSTREAM-ROADMAP-COMMERCIAL-QUOTAS-001 | Multi-instance fair admission; REST/RPC/WS | T atomic Redis quota and metering tests | P1; configured Redis; fail closed unchanged | Weighted batches and upgrades cannot evade quotas | Retained |
| UPSTREAM-ROADMAP-GEYSER-ABI-001 | Sustained ingestion throughput; gateway | R phase 10 incompatible deployment pins | P2; operator-compatible binaries and sustained evidence | 100k finalized blocks/24h, zero divergence; ABI preflight | Operator blocked |
| UPSTREAM-ROADMAP-REPLAY-PERFORMANCE-001 | Predictable scaling; replay/load contract | replay-load-validation.js bounded-heap invariant | P2; representative workload for production claims; offline only | Duplicate/replacement invariants and bounded heap; replay CLI | Offline safeguard retained |

- Implemented: simulation reuses the existing preparation/receipt/quote/pool binder after receipt validation, before local RPC. The original regression fails against the baseline by reaching RPC, then passes with ten isolated cross-quote substitutions rejected locally. Valid bound simulation and unbound/pool-drift/hash controls remain covered. Hashes remain content checks, not signatures or authorization.
- Validation: focused 9/9 PASS; full suite 497/497 PASS; syntax 87/87 PASS; 1,000-block replay invariants PASS (7,373.04 blocks/s, 9,689,368-byte heap delta). Operational health remains BLOCKED with nine unavailable/invalid provider, indexed-data, exporter, warehouse, backup and recovery prerequisites; monitoring SKIP because promtool is unavailable. Format/lint/typecheck/build scripts are absent and SKIP. No live service qualification is claimed.
- Compatibility/migration/configuration: no new fields, migrations, configuration, HTTP/RPC/WebSocket response changes, or provider calls. Existing valid bound preparations remain compatible. Rehashed cross-quote artifacts now fail at semantic admission; no signing or submission was added.
- NEXT_WEB_ACTION: handle semantic-binding rejection as an invalid preparation and rebuild from one unchanged quote, pool, and verified receipt rather than retrying simulation.

## UPSTREAM wire-level simulation binding

- Selected ID: `UPSTREAM-QA-METEORA-PREPARATION-RECEIPT-002`, continuing the same defect against baseline `ff8fb64`. The 20 opportunities above remain reconciled; the other 19 retain their evidence, dependencies, acceptance criteria and status. Coupled quote/metadata mutation outranks orchestration because the existing binder does not inspect serialized instruction semantics. `UPSTREAM-METEORA-ORCHESTRATION-124` remains deferred pending independent verification.
- Implemented: simulation decodes the unsigned legacy transaction, reconstructs the canonical Meteora instruction from quote/pool evidence and decoded route inputs, and compares wire bytes, normalized account roles, instruction metadata, transaction hashes, effect policies and minimum context before RPC. The baseline coupled amount mutation reached a local stub; the regression now rejects it with zero calls, alongside altered vault/input wire instructions and effect/identity/context mutations. Existing legacy controls and fee/static-hook/data-hook swap2 admission remain covered. Receipt hashes remain content checks, not trusted signatures.
- Compatibility/migration/configuration: no REST/RPC/WebSocket response, storage, migration or configuration changes. Data-derived transfer-hook simulation callers must now pass the original finalized `transferHookAccountData` map again so account resolution can be reconstructed; missing evidence fails locally. Legacy, fee-only and static-hook callers need no new argument. No signing, submission or live provider calls were added.
- Validation: focused 11/11 PASS; full suite PASS; syntax 87/87 PASS; synthetic 1,000-block replay PASS with all invariants, 9,161.52 blocks/s and 9,139,704-byte heap delta. Health BLOCKED by nine operator-owned provider/index/exporter/warehouse/backup/recovery prerequisites. Monitoring SKIP because promtool is unavailable; format/lint/typecheck/build SKIP because scripts are absent. Live smoke/production performance qualification SKIP; swap2 positive admission uses local stubs, not real simulation.
- NEXT_WEB_ACTION: supply original finalized transfer-hook account bytes for data-derived routes and rebuild rejected preparations from unchanged verified quote/pool evidence before simulation.
